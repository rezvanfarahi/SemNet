...
#sys.path.insert(1,'/imaging/local/software/mne_python/v0.9full')
...
#import operator
from mne.minimum_norm import (psf_ctf_28Oct15,read_inverse_operator, apply_inverse_epochs)
...
#        print conind
    ...
    fname_inv = data_path + meg + 'InvOp_ico5newreg_filtnew_pnt1_48_clean_ica_EMEG-inv.fif'#'InvOp_ico5diagreg_fft_1_48_clean_ica_EMEG-inv.fif'#
    # Load data
...
#        ntimes=this_stc.data.shape[1]
    ...
    mode = 'svd'
    n_svd_comp = 1
    labellist_temp=copy.deepcopy(labels)
    labels = [labellist_temp[jj].morph(subject_from='fsaverage', subject_to=subjects[subject_no], subjects_dir=data_path) for jj in range(len(labels))]
...
#    
#    for which_parc in np.array([3,4]):#range(len(parc_name)):#np.array([2,3]):#
#        
#        labels_pre = mne.read_labels_from_annot('fsaverage', parc=parc_name[which_parc], subjects_dir=subjects_dir)
#        if which_parc==0:
#            labels_pre.remove(labels_pre[-1])
...
#            outparc=['aparc','aparc_mod','aparc2009','aparc2009_mod','RG']
#            out_path=data_path+meg+'pacel_connmat_'+event_names[evcnt]+'_'+outparc[which_parc]+'.mat'
