...
#            MT[:,uvcnt]= mne.stats.ttest_1samp_no_p(signmat*thisX[:,:,uvcnt], sigma=1e-3, method='relative')
    ...
    if len(pval_sig)>0:
        for pvs,pvalsig in enumerate(pval_sig):
            this_MTpos=MTpos.copy()
            this_MTpos[np.where(comps_pos!=cu_pos[pvalsig])[0],:]=0
            out_fname=out_path+'NBSnetwork_interaction_pos_cluster'+str(pvs)+'_'+parc_names[wparc]+'_'+event_names[thisev]+'_'+event_names[refev]+'_'+freq_names[thisfreq]+'_'+timesmin[thist]+'_'+timesmax[thist]+'_'+str(p_threshold)[2:]+'.mat'
#            scipy.io.savemat(out_fname,{'this_MTpos':this_MTpos})
    ...
    plot_connectivity_circle(this_MTpos, label_names,colormap='cool',n_lines=None,
