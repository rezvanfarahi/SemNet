...
for cnt, meg in enumerate(ll):
	...
	print meg
	fname = data_path + meg + 'block_localisers_tsss_filt_pnt1_48_raw.fif'
#     event_fname = event_path + meg + 'block_localisers_tsss_ds_raw-eve.fif'
         ...
         if 1>0:#ch_type is 'eeg': #
             ica, _ = compute_ica(raw, picks=picks,
		                 ...
		                 n_max_ecg=n_max_ecg, n_max_eog=n_max_eog,
                           ecg_tmin=-0.5, ecg_tmax=0.5, 
                           eog_tmin=-0.5,eog_tmax=0.5,
		                 reject=ica_reject,random_state=42,artifact_stats=artifact_stats,
		                 decim=decim, report=report, show=False, img_scale=img_scale)
             out_path=data_path+meg+'localisers_{}-ica.fif'.format(ch_type)
	...
	for k, v in artifact_stats.items():
         print(k, v)    
	report.save(op.join(results_dir, run_id,'preprocessing_localisers-report-{}.html'.format(subject)),open_browser=True, overwrite=True)
     
	ica_path=data_path+meg+'localisers_meg-ica.fif'
	ica=read_ica(ica_path)
	raw=ica.apply(raw)
	ica_path=data_path+meg+'localisers_eeg-ica.fif'
	ica=read_ica(ica_path)
	raw=ica.apply(raw)
	data_out_path=data_path+meg+'block_localisers_tsss_filt_pnt1_48_ica_raw.fif'
