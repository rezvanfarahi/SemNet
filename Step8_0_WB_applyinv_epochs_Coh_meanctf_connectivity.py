...
#            cwt_n_cycles = cwt_frequencies / float(3)#np.array([2,3,5,7])#
    ...
    # get 2 frequency bins
            ...
            coh_even=np.zeros((indices[0].shape[0],4,4))
            for actc,tc in enumerate(range(50,351,100)): #time count
                print actc
                tminw=tc/1000.
                tmaxw=(tc+200)/1000.
                coh_even[:,:,actc], freqs1, times1, n_epochs1, _ = spectral_connectivity(comb_ts, method='coh', mode='multitaper', indices=indices,sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=True, tmin=tminw, tmax=tmaxw, n_jobs=4)
...
# Note: We use a hack to save the frequency axis as time
            ...
            data_out = data_path + meg +  'Morphed_ico_SemDec_evenodd_ica_equalized_'+event_names[evcnt]+'_Mlttap_new_ctf_Coh_Theta_150_450_200ms_' + thislabelname
            print 'theta'
            ...
            data_out = data_path + meg +  'Morphed_ico_SemDec_evenodd_ica_equalized_'+event_names[evcnt]+'_Mlttap_new_ctf_Coh_Alpha_150_450_200ms_' + thislabelname
            print 'alpha'
            coh_stc.save(data_out)
            
            coh_stc = mne.SourceEstimate(coh_even[:,2,:], vertices=vertices_to, tmin= tmin1, tstep= tstep1, subject='fsaverage')
            data_out = data_path + meg +  'Morphed_ico_SemDec_evenodd_ica_equalized_'+event_names[evcnt]+'_Mlttap_new_ctf_Coh_Beta_150_450_200ms_' + thislabelname
            print 'beta'
            coh_stc.save(data_out)
            
            coh_stc = mne.SourceEstimate(coh_even[:,3,:], vertices=vertices_to, tmin= tmin1, tstep= tstep1, subject='fsaverage')
            data_out = data_path + meg +  'Morphed_ico_SemDec_evenodd_ica_equalized_'+event_names[evcnt]+'_Mlttap_new_ctf_Coh_Gamma_150_450_200ms_' + thislabelname
            print 'gamma'
            coh_stc.save(data_out)
