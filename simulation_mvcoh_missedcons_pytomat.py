...
for connperc in [1]:
    print connperc
    for nseeds in np.array([5]): #,10,15,20,5,10,15
        missedcons=np.zeros((36,1))
        missednodes=np.zeros((36,1))
        ovnodes=np.zeros((36,1))
...
#                missedcons[fcnt,:,1]=thisvect.copy()
#        else:
        in_path='/imaging/rf02/Semnet/simulations/funcseeds_avgsims36_offset0_seeds5_conns_snr3_extglasser/'
        for(dirpath,dirnames,filenames) in walk(in_path):
