...
for nseeds in [5]:
    ...
    for nconns in [1]:
        print nconns
        concnt=concnt+1;
        for which_parc in np.array([1]):#range(5):
#            if nseeds==10 and nconns==1:
#                nsims=32
#            elif nseeds==15 and nconns==1:    
#                nsims=32
#            else:
            ...
            if which_parc==0:
                    nnodes=68
                    inpath='/imaging/rf02/parcellation/extended_simulations/funcseeds_avgsims70_offset0_seeds'+str(nseeds)+'_conns'+str(nconns)+'_bothsnrs_extglasser/aparc/'
                    filelist0=list()
                    ...
                    filelist.sort()
#                    inpath2='/imaging/rf02/parcellation/extended_simulations/avgsims70_offset0_bothsnrs_extglasser/aparc/'
#                    filelist_null0=list()
...
#                    aa=dir ('*parc_connmat*.mat');
                    ...
                    nnodes=74;
                    inpath='/imaging/rf02/Semnet/simulations/funcseeds_avgsims36_offset0_seeds5_conns_snr3_extglasser/aparc_mod/'
                    filelist0=list()
                    for(dirpath,dirnames,filenames) in walk(inpath):
                        filelist0.extend(filenames)
                    filelist=[fn for fn in filelist0 if fn[:12]=='parc_connmat'] 
                    filelist.sort()
#                    inpath2='/imaging/rf02/parcellation/extended_simulations/avgsims70_offset0_bothsnrs_extglasser/aparc_mod/'
#                    filelist_null0=list()
#                    for(dirpath2,dirnames2,filenames2) in walk(inpath2):
#                        filelist_null0.extend(filenames2)
#                    filelist_null=[fn for fn in filelist_null0 if fn[:12]=='parc_connmat']
#                    filelist_null.sort()
                    ...
                    nnodes=148;
                    inpath='/imaging/rf02/parcellation/extended_simulations/funcseeds_avgsims70_offset0_seeds'+str(nseeds)+'_conns'+str(nconns)+'_bothsnrs_extglasser/aparc2009/'
                    filelist0=list()
                    for(dirpath,dirnames,filenames) in walk(inpath):
                        filelist0.extend(filenames)
                    filelist=[fn for fn in filelist0 if fn[:12]=='parc_connmat']
                    filelist.sort()
#                    inpath2='/imaging/rf02/parcellation/extended_simulations/avgsims70_offset0_bothsnrs_extglasser/aparc2009/'
#                    filelist_null0=list()
#                    for(dirpath2,dirnames2,filenames2) in walk(inpath2):
#                        filelist_null0.extend(filenames2)
#                    filelist_null=[fn for fn in filelist_null0 if fn[:12]=='parc_connmat']
#                    filelist_null.sort()
                    ...
                    nnodes=74;
                    inpath='/imaging/rf02/parcellation/extended_simulations/funcseeds_avgsims70_offset0_seeds'+str(nseeds)+'_conns'+str(nconns)+'_bothsnrs_extglasser/aparc2009_mod/'
                    filelist0=list()
                    for(dirpath,dirnames,filenames) in walk(inpath):
                        filelist0.extend(filenames)
                    filelist=[fn for fn in filelist0 if fn[:12]=='parc_connmat']
                    filelist.sort()
#                    inpath2='/imaging/rf02/parcellation/extended_simulations/avgsims70_offset0_bothsnrs_extglasser/aparc2009_mod/'
#                    filelist_null0=list()
#                    for(dirpath2,dirnames2,filenames2) in walk(inpath2):
#                        filelist_null0.extend(filenames2)
#                    filelist_null=[fn for fn in filelist_null0 if fn[:12]=='parc_connmat']
#                    filelist_null.sort()
                    ...
                    nnodes=70;
                    inpath='/imaging/rf02/parcellation/extended_simulations/funcseeds_avgsims70_offset0_seeds'+str(nseeds)+'_conns'+str(nconns)+'_bothsnrs_extglasser/RG/'
                    filelist0=list()
                    for(dirpath,dirnames,filenames) in walk(inpath):
                        filelist0.extend(filenames)
                    filelist=[fn for fn in filelist0 if fn[:12]=='parc_connmat']
                    filelist.sort()
#                    inpath2='/imaging/rf02/parcellation/extended_simulations/avgsims70_offset0_bothsnrs_extglasser/RG/'
#                    filelist_null0=list()
#                    for(dirpath2,dirnames2,filenames2) in walk(inpath2):
#                        filelist_null0.extend(filenames2)
#                    filelist_null=[fn for fn in filelist_null0 if fn[:12]=='parc_connmat']
...
#            nullconnmat=np.zeros((nnodes,nnodes,7,nsubs,nsims));
#            for fcnt,filename_null in enumerate(filelist_null):#=1:nsims:
#                filepathnull=inpath2+filename_null
#                conpre=scipy.io.loadmat(filepathnull,mat_dtype=True); 
#                nullconnmat[:,:,:,:,fcnt]=np.squeeze(conpre['parc'+str(which_parc)+'_connmat'])
#            ncm_vect=np.zeros((nsubs,))
#            ncm_nullid=np.squeeze(nullconnmat[:,:,1,:,:])
#            for subcnt in range(nsubs):
#                thisncm=np.squeeze(ncm_nullid[:,:,subcnt,:])
#                ncm_sim=np.zeros((nsims,))
#                for simcntt in range(nsims):
#                    ncm_sim[simcntt]=np.max(np.abs(thisncm[:,:,simcntt]))
#                ncm_vect[subcnt]=np.percentile(ncm_sim,99.999)
                
            connmat=np.zeros((nnodes,nnodes,7,nsubs,nsims))#;%leakageSNR3,noiseSNR3,ideal,leakageSNR1,noiseSNR1 (COH 1st, imCOH2nd)
            print filelist
#            cc=-1;
...
#            base_connmat=repmat(nullconnmat(:,:,[2,1,1,4,3,6,5,5,8,7],:),[1,1,1,1,nsims]);
            ...
            basecorr_connmat_new=np.zeros((nnodes,nnodes,5,nsubs,nsims))
            basecorr_connmat_new[:,:,0,:,:]=basecorr_connmat[:,:,0,:,:].copy()#-basecorr_connmat[:,:,1,:,:]
            basecorr_connmat_new[:,:,2,:,:]=basecorr_connmat[:,:,3,:,:].copy()#-basecorr_connmat[:,:,4,:,:]
            basecorr_connmat_new[:,:,1,:,:]=basecorr_connmat[:,:,2,:,:].copy()
            basecorr_connmat_new[:,:,3,:,:]=basecorr_connmat[:,:,5,:,:].copy()
            basecorr_connmat_new[:,:,4,:,:]=basecorr_connmat[:,:,6,:,:].copy()
            
            allratios=np.arange(-0.05,1.06,0.05)
            basecorr_connmat_ROC=np.zeros((nnodes,nnodes,5,nsubs,nsims,len(allratios)))
#            basecorr_connmat_new=np.abs(basecorr_connmat_new)
            for thisratc,thisrat in enumerate(allratios):
                for cc1 in np.array([0,1,2,3]):#range(4):
                    for cc2 in range(nsubs):
                        for cc3 in range(nsims):
                            this_connmat=basecorr_connmat_new[:,:,cc1,cc2,cc3].copy()+np.triu(1000*np.ones((nnodes,nnodes)))
                            connmat_sort=np.sort(this_connmat[this_connmat<1000])
                            if thisrat<0:
                                tcm=0
                            elif thisrat>1:
                                tcm=1
                            elif thisrat==1:
                                tcm=connmat_sort[-1].copy()
                            else:
                                tcm=connmat_sort[int(thisrat*len(connmat_sort))].copy()#np.mean(this_connmat[this_connmat<1000])
#                            print tcm
                            #this_connmat=this_connmat-np.triu(1000*np.ones((nnodes,nnodes)))
                            this_connmat=this_connmat-tcm-np.triu((1000-tcm)*np.ones((nnodes,nnodes)))
                            basecorr_connmat_ROC[:,:,cc1,cc2,cc3,thisratc]=this_connmat.copy()
            for rocc in range(len(allratios)):
                basecorr_connmat_ROC[:,:,4,:,:,rocc]=basecorr_connmat_new[:,:,4,:,:].copy()
            pval=np.zeros((nnodes,nnodes,5,nsims,len(allratios)));
            print 'ttest'
#            for cc1 in range(4):
#                print cc1
#                
#                for cc2 in range(nsims):
#                    this_grandmat=np.squeeze(basecorr_connmat_new[:,:,cc1,:,cc2])
...
#            pval[:,:,4,:]=np.mean(np.squeeze(basecorr_connmat_new[:,:,4,:,:]),2)
            for thisratc,thisrat in enumerate(allratios):
                print thisratc
                for cc1 in range(nnodes): 
#                    print cc1
                    for cc2 in range(nnodes):
                        for cc3 in range(4):
                            ntests=nnodes*(nnodes-1)/2                   
                            thisvect=np.squeeze(basecorr_connmat_ROC[cc1,cc2,cc3,:,:,thisratc]);
                            ...
                            for ttcnt,thist in enumerate(this_tval):
                                pval[cc1,cc2,cc3,ttcnt,thisratc]=scistats.t.sf(np.abs(thist),nsubs-1) 
            pval[:,:,4,:,:]=np.mean(np.squeeze(basecorr_connmat_ROC[:,:,4,:,:,:]),2)
            out_name=inpath+'roc_ttest_pval_parc_'+str(which_parc)+'.mat'
            #parc_nullb is when I baseline corrected using pure noise with no leakage, over emphasises any effect of leakage to distant areas; i.e. not ideal
