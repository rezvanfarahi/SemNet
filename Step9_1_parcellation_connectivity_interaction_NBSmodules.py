...
#nnodes=3
#plt.close("all")
module_path='/imaging/rf02/Semnet/graph_analysis/'
thresh_connmat_modules_file=module_path+'grand_modconnmat_mi_maxctf_thresh_modules3conds_3bands.mat'
ltc=scipy.io.loadmat(thresh_connmat_modules_file,mat_dtype=True); thresh_connmat_modules=np.squeeze(ltc['thresh_connmat_modules'])
file_modules=module_path+'finalmodules_mi_maxctf_3conds_3bands.mat'
ltc=scipy.io.loadmat(file_modules,mat_dtype=True); finalmodules=np.squeeze(ltc['finalmodules'])
uniquemodules=np.unique(finalmodules).astype(int)

nnodes=len(uniquemodules)#int(parc_len[wparc])
...
#grand_connmat=np.zeros((nnodes,nnodes,4,3,len(event_names),nsubs))

grand_connmat1=np.transpose(thresh_connmat_modules,(1,2,3,4,5,0))
grand_connmat=grand_connmat1[:,:,:,timearray,:,:].copy()
for freq1,freq2 in zip(np.array([1]),np.array([3])):
    
    thisX=np.zeros((nnodes,nnodes,ntimes,n_subjects,nlevels))#grand_connmat[:,:,thisfreq,thist,thisev,:]-grand_connmat[:,:,thisfreq,thist,refev,:]
    thisX[:,:,:,:,0]=grand_connmat[:,:,freq1,:,thisev,:].copy()
    thisX[:,:,:,:,1]=grand_connmat[:,:,freq2,:,thisev,:].copy()
    thisX[:,:,:,:,2]=grand_connmat[:,:,freq1,:,refev,:].copy()
    thisX[:,:,:,:,3]=grand_connmat[:,:,freq2,:,refev,:].copy()
    thisX=np.transpose(thisX,(3,4,0,1,2))#sub x level x node x node x time
    MT=np.zeros((nnodes,nnodes,ntimes))    
    for tcnt in range(ntimes):
        for uvcnt in range(nnodes):
            #        print uvcnt
            #        MT[:,uvcnt]= mne.stats.ttest_1samp_no_p(thisX[:,:,uvcnt], sigma=1e-3, method='relative')
            MT[:,uvcnt,tcnt], p= mne.stats.f_mway_rm(thisX[:,:,:,uvcnt,tcnt], factor_levels=factor_levels,effects=effects, return_pvals=return_pvals,alpha=p_threshold)
   
#    for ttcnt in range(ntimes):
#        np.fill_diagonal(MT[:,:,ttcnt], 0)
    MTpos=MT.copy()
    MTpos[np.abs(MTpos)<t_threshold]=0
    MTpos[np.abs(MTpos)>=t_threshold]=1
    
    MTpos_1side=MT.copy()
    MTpos_1side[np.abs(MTpos_1side)<t_threshold]=0

    Tpos_sum=np.sum(np.sum(np.abs(MTpos_1side),2),1)
...
#        print pcnt
        ...
        thisX[:,:,:,thisc,0]=np.transpose(grand_connmat[:,:,freq1,:,thisev,thisc].copy(),(1,2,3,0))#np.reshape(grand_connmat[:,:,freq1,:,thisev,thisc].copy(),(nnodes,nnodes,ntimes,len(thisc)))
        thisX[:,:,:,refc,0]=np.transpose(grand_connmat[:,:,freq1,:,refev,refc].copy(),(1,2,3,0))#np.reshape(grand_connmat[:,:,freq1,:,refev,refc].copy(),(nnodes,nnodes,ntimes,len(refc)))
        thisX[:,:,:,thisc,1]=np.transpose(grand_connmat[:,:,freq2,:,thisev,thisc].copy(),(1,2,3,0))#np.reshape(grand_connmat[:,:,freq2,:,thisev,thisc].copy(),(nnodes,nnodes,ntimes,len(thisc)))
        thisX[:,:,:,refc,1]=np.transpose(grand_connmat[:,:,freq2,:,refev,refc].copy(),(1,2,3,0))#np.reshape(grand_connmat[:,:,freq2,:,refev,refc].copy(),(nnodes,nnodes,ntimes,len(refc)))
        
        thisX[:,:,:,thisc,2]=np.transpose(grand_connmat[:,:,freq1,:,refev,thisc].copy(),(1,2,3,0))#np.reshape(grand_connmat[:,:,freq1,:,refev,thisc].copy(),(nnodes,nnodes,ntimes,len(thisc)))
        thisX[:,:,:,refc,2]=np.transpose(grand_connmat[:,:,freq1,:,thisev,refc].copy(),(1,2,3,0))#np.reshape(grand_connmat[:,:,freq1,:,thisev,refc].copy(),(nnodes,nnodes,ntimes,len(refc)))
        thisX[:,:,:,thisc,3]=np.transpose(grand_connmat[:,:,freq2,:,refev,thisc].copy(),(1,2,3,0))#np.reshape(grand_connmat[:,:,freq2,:,refev,thisc].copy(),(nnodes,nnodes,ntimes,len(thisc)))
        thisX[:,:,:,refc,3]=np.transpose(grand_connmat[:,:,freq2,:,thisev,refc].copy(),(1,2,3,0))#np.reshape(grand_connmat[:,:,freq2,:,thisev,refc].copy(),(nnodes,nnodes,ntimes,len(refc)))
        thisX=np.transpose(thisX,(3,4,0,1,2))#sub x level x node x node x time
        MT=np.zeros((nnodes,nnodes,ntimes))    
        for tcnt in range(ntimes):
            for uvcnt in range(nnodes):
...
#            np.fill_diagonal(MT[:,:,ttcnt], 0)
        MTpos_perm=MT.copy()
        MTpos_perm[np.abs(MTpos_perm)<t_threshold]=0
        MTpos_perm[np.abs(MTpos_perm)>=t_threshold]=1
        #MTpos=MTpos-np.eye(MTpos.shape[0])
        MTpos_1side=MT.copy()
        MTpos_1side[np.abs(MTpos_1side)<t_threshold]=0
        
        Tpos_sum_perm=np.sum(np.sum(np.abs(MTpos_1side),2),1)
...
#            Tpos_sum_perm[tci]=np.sum(np.sum(thisMT,1))/2.
        Tpos_max_perm[pcnt]=Tpos_sum_perm[np.argmax(np.abs(Tpos_sum_perm))]
#        print Tpos_max_perm[pcnt]
    pvals=np.zeros((len(Tpos_sum),))
    for pvcnt in range(len(pvals)):
        pvals[pvcnt]=np.sum(np.abs(Tpos_max_perm)>=np.abs(Tpos_sum[pvcnt]))/np.float(n_permutations)
    print pvals
...
#        this_MTpos[np.where(comps_pos!=cu_pos[pvalsig])[0],:]=0
        out_fname=out_path+'modules_interaction_3D_overtime'+parc_names[wparc]+'_'+event_names[thisev]+'_'+event_names[refev]+'_'+str(p_threshold)[2:]+'.mat'#'_'+str(thisp)[2:6]+
...
#    MTpos[np.abs(MT)>=t_threshold]=1
#    ppos=p.copy()
#    ppos[ppos>=p_threshold]=1#[np.abs(MT)<t_threshold]=1
#    psum=np.sum(ppos<1)
#    print psum
...
#    ppos=1-ppos
    ...
    plt.imshow(ppos,interpolation='none')
    """
#    print ppos_corr.max()
