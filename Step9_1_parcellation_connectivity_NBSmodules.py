...
tail=0
timearray=np.array([0,1,2])
ntimes=len(timearray)
#grand_connmat=np.zeros((nnodes,nnodes,4,3,len(event_names),nsubs))
grand_connmat=np.transpose(thresh_connmat_modules,(1,2,3,4,5,0))
for thisfreq in np.array([0,1,2,3]):
    
    thisX=grand_connmat[:,:,thisfreq,timearray,thisev,:]-grand_connmat[:,:,thisfreq,timearray,refev,:]
    thisX=np.transpose(thisX,(3,0,1,2))
    MTorig=np.zeros((nnodes,nnodes,ntimes))    
    for uvcnt1 in range(thisX.shape[2]):#node
        for uvcnt2 in range(thisX.shape[3]):#time
#        print uvcnt
            MTorig[:,uvcnt1,uvcnt2]= mne.stats.ttest_1samp_no_p(thisX[:,:,uvcnt1,uvcnt2], method='relative')
...
#        print pcnt
        ...
        for uvcnt1 in range(thisX.shape[2]):
            for uvcnt2 in range(thisX.shape[3]):
    #            print uvcnt
                MT[:,uvcnt1,uvcnt2]= mne.stats.ttest_1samp_no_p(signmat*thisX[:,:,uvcnt1,uvcnt2], method='relative')
#        for ttcnt in range(ntimes):
#            np.fill_diagonal(MT[:,:,ttcnt], 0)
        MTpos_perm=MT.copy()
...
#        thisp=pvals[pvalsig]
        this_MTpos[pval_sig,:,:]=MTorig[pval_sig,:,:].copy()
        this_MTpos[np.abs(this_MTpos)<t_threshold]=0
        print np.where(this_MTpos!=0)
#        this_MTpos[np.where(comps_pos!=cu_pos[pvalsig])[0],:]=0
        out_fname=out_path+'modules_3D_overtime'+parc_names[wparc]+'_'+event_names[thisev]+'_'+event_names[refev]+'_'+freq_names[thisfreq]+'_'+str(p_threshold)[2:]+'.mat'#'_'+str(thisp)[2:6]+
