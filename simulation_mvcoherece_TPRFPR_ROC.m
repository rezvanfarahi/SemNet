...
nparcs=1;
nmeas=4;%ideal coherence
TP_grand=squeeze(zeros(nparcs,4,nsims));%the 9: COH:leak3,noise3,leak1,noise1, imCOH:leak3,noise3,ideal,leak1,noise1,
FP_grand=squeeze(zeros(nparcs,4,nsims));
TN_grand=squeeze(zeros(nparcs,4,nsims));
FN_grand=squeeze(zeros(nparcs,4,nsims));
seedcnt=0;
mcc=0;
...
% %                     load ('ideal_pval_parc0.mat');
                case 2
                    ...
                    cd(inpath)
                    load ('roc_ttest_pval_parc_1.mat');
...
%                     load ('ideal_pval_parc4.mat');
            ...
            load(['/imaging/rf02/Semnet/simulations/funcseeds_avgsims36_offset0_seeds5_conns_snr3_extglasser/allmissedcons.mat'])
...
%             bcm=squeeze(mean(basecorr_connmat(:,:,3,:,:),4));
           for cc0=1:nrats
            for cc1=1:5
                ...
                for cc2=1:nsims
                    %                         cc2
                    thismat=squeeze(pfdrmat(:,:,cc1,cc2,cc0));
%                     qq=bcm(:,:,cc2)+triu(1000*ones(nnodes,nnodes));
                    
                    if cc1==5 %|| cc1==5 || cc1==7 || cc1==9 %ideal cases
                        g=0;%thismat=tril(thismat,-1);%+1;
                    else
...
%                     end
                    ...
                    thismat=tril(thismat,-1);
                    pfdrmat_final(:,:,cc1,cc2,cc0)=thismat;
                end
            end
           end
...
%                 end
%             end
            eval_mat=pfdrmat_final(:,:,1:4,:,:)- pfdrmat_final(:,:,[5,5,5,5],:,:);
            TP=zeros(nmeas,nsims,nrats);
            TN=zeros(nmeas,nsims,nrats);
            FP=zeros(nmeas,nsims,nrats);
            FN=zeros(nmeas,nsims,nrats);
            for cc0=1:nrats
            for cc1=1:nmeas
                refs=[5,5,5,5];
                for cc2=1:nsims
                    thiseval=tril(eval_mat(:,:,cc1,cc2,cc0),-1)+triu(2*ones(nnodes,nnodes));
                    refvect=tril(pfdrmat_final(:,:,refs(cc1),cc2,cc0),-1)+triu(2*ones(nnodes,nnodes));
                    msdcn=missedcons(cc2);
                    refpos=max([sum(sum(refvect==1))+msdcn,1]);
%                     mc_gmat(cc2,which_parc,mcc)=msdcn/refpos;
                    refneg=max([sum(sum(refvect==0)),1]);%-msdcn;
                    TP(cc1,cc2,cc0)=sum(sum(thiseval==3))/refpos;
                    FP(cc1,cc2,cc0)=sum(sum(thiseval==4));%/refneg;%/refpos;
                    TN(cc1,cc2,cc0)=sum(sum(thiseval==4))/refneg;%(sum(sum(thiseval==2))-msdcn)/refneg;
                    FN(cc1,cc2,cc0)=refneg;%(sum(sum(thiseval==1))+msdcn)/refpos;
                end
            end
            end
