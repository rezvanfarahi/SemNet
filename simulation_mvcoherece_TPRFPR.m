...
%             end
            ...
            TP=zeros(nmeas,nsims);
            TN=zeros(nmeas,nsims);
            FP=zeros(nmeas,nsims);
            FN=zeros(nmeas,nsims);
            for cc1=1:nmeas
                refs=[5,5,5,5];
                for cc2=1:nsims
                    thiseval=eval_mat(:,:,cc1,cc2);
                    refvect=pfdrmat_final(:,:,refs(cc1),cc2)+triu(1000*ones(nnodes,nnodes));
                    msdcn=0;%missedcons(cc2);
