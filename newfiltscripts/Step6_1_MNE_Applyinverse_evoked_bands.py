...
for ii, meg in enumerate(ll):
    ...
    event_fname = orig_path + meg + 'SemDec_blocks_tsss_filtnew_pnt1_48_ica_raw-eve.fif'
...
#    events[:,0] = events[:,0]+np.round( raw.info['sfreq']*stim_delay )
    ...
    print "Epoching"
    if subject_inds[0] in np.array([2,3,5,18]):#np.array([4,8,9]):
...
#        raw.plot_psd(fmax=50,picks=picks)#,average=False)
        ...
        print len(epochs)/900.#    for eegcnt in range(71):
#        if eegcnt<10:
...
#                event_id = eid+1  
	...
	###############################################################################
        ...
        fname_inv = data_path + meg + 'InvOp2_ico5newreg_filtnew_pnt1_48_clean_ica_'+b+'_EMEG-inv.fif'
...
#            fig=this_evoked.plot(show=False)
#            figname_out='/imaging/rf02/Semnet/results/evoked/all_noisecov/subj'+str(subject_inds[0])+'.jpg'
...
#            ediv=np.sqrt(edv)
    ...
    #        this_stc.resample(200)
            data_out = data_path + meg + 'firstMorphed_SemDec_filtnew_pnt1_48ica_'+event_names[evcnt]+'_Source_signedEvoked_m300_600_'+b
