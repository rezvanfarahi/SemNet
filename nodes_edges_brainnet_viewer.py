...
brainnet_path='/imaging/rf02/Semnet/BrainNetViewer_20170403/rezvan/'
...
for nseeds in [5]:#,5,10,15]:
    ...
    for nconns in [1]:#,0.5,0.25]:
        ...
        for which_parc in range(5):
            ...
            inpath='/imaging/rf02/parcellation/extended_simulations/funcseeds_avgsims70_offset0_seeds'+str(nseeds)+'_conns'+str(nconns)+'_bothsnrs_extglasser/'+short_parc_names[which_parc]+'/'
            ...
            for lbl in labels:
                lbl.values.fill(1.0)  
            centre_pos=np.zeros((len(labels),6))
            for thislblii,thislbl in enumerate(labels):
                centre_pos[thislblii,3]=thislblii/2#color
                centre_pos[thislblii,5]=np.copy(thislblii)#name
                if thislbl.hemi=='lh':
                    centre_pos[thislblii,:3]=mne.vertex_to_mni(thislbl.center_of_mass('fsaverage'),0,'fsaverage')
                else:
                    centre_pos[thislblii,:3]=mne.vertex_to_mni(thislbl.center_of_mass('fsaverage'),1,'fsaverage')
            centre_pos[:,4]=1#degree
            out_name=brainnet_path+'nodeinfo_parc'+str(which_parc)+'.mat'
            scipy.io.savemat(out_name,{'centre_pos':centre_pos})
#            in_name=inpath+'perm_pval_parc'+str(which_parc)+'.mat'
...
#            null_pvalm=nullmat_pval[:,:,:,whichsim]#np.mean(nullmat_pval,3)
#            conv_pval=1-null_pvalm
#            thispval=conv_pval[:,:,7]#conv_pval[:,:,2]
#            
#            
...
#outname='/imaging/rf02/parcellation/extended_simulations/results/'+short_parc_names[which_parc]+'_seeds'+str(nseeds)+'_conns'+str(nconns)+'_sim'+str(whichsim)+'_imcoh1.jpg'                
