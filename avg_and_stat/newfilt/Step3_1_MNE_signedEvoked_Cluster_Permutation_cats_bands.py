...
out_path = '/imaging/rf02/Semnet/stc/permutation/evoked/bands/newfilt/newregsigned/'+b+'_cats/19subs/' # root
uvttest_path = '/imaging/rf02/Semnet/stc/uvttest/evoked/bands/newfilt/newregsigned/'+b+'_cats/19subs/' # root
...
for p_threshold in ll:
    ...
    for ii, meg in enumerate(list_all):
        ...
        for evcnt, event_name in enumerate(event_names):                
            ...
            wcnt=-1
            for wcnt1,wcnt2 in zip(list(np.arange(350,751,25)),list(np.arange(375,776,25))):#zip(list(np.arange(350,751,10)),list(np.arange(360,761,10))):#range(nwins):
    #                print wcnt1,wcnt2
                wcnt=wcnt+1
                X[ii,wcnt,:,evcnt]=np.mean(stc_cond.data[:,wcnt1:wcnt2],1)
