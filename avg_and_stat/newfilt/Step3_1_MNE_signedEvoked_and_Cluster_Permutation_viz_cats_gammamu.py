# -*- coding: utf-8 -*-
"""
Created on Fri Oct 27 00:10:14 2017
...
###############################################################################
# Set parameters
out_path = '/imaging/rf02/Semnet/stc/permutation/evoked/bands/newfilt/newregsigned/mugamma_newev/nsubs_19/'
#'/imaging/rf02/Semnet/stc/permutation/evoked/bands/newregsigned/taugamma_newev/' # root
...
#thiscond=3
refcond=0
out_path_img='/imaging/rf02/Semnet/jpg/permutation/evoked/bands/newfilt/newregsigned/mugamma_newev/19subs/'+event_names[thiscond]+'/'
...
for thispc,p_threshold in enumerate(ll):       
        
#    for thisp in [0.005,0.008,0.01,0.025,0.05]:
...
#    fname_in=out_path + 'ClusPer_abs_sw_icomorphed_newreg_clusterp'+str(p_threshold)[2:]+'_p'+str(p_thr)[2:]+'_19subj_SemDec_pnt1_30ica_'+event_names[thiscond]+'_'+event_names[refcond]+'_maxstep'+str(max_step)
    fname_in=out_path + 'ClusPer_sEvoked_sw_icomorphed_newreg_clusterp'+str(p_threshold)[2:]+'_p'+str(p_thr)[2:]+'_20subj_SemDec_pnt1_30ica_'+event_names[refcond]+'_'+event_names[thiscond]+'gammamu_maxstep'+str(max_step)

    stc[thispc] = read_source_estimate(fname_in)
...
plt.figure()
times_list=list(np.arange(50,451,25))
this_view='med'
this_surf='inflated'
hemi='rh'
for this_t in times_list:
	thisbrain=stc_sum.plot(hemi=hemi, 	views=this_view,subjects_dir=data_path,subject='fsaverage',time_viewer=False,colormap='hot',alpha=1,clim=clim,
	smoothing_steps=2,transparent=True,initial_time=this_t,time_unit='ms',size=1400,surface=this_surf)
	out_img=out_path_img+event_names[thiscond]+'_'+hemi+'_'+this_view+'_'+this_surf+'_'+str(this_t)+'ms.jpg'
