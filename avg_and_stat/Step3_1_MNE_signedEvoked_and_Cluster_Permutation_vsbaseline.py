...
###############################################################################
# Set parameters
out_path = '/imaging/rf02/Semnet/stc/permutation/evoked/pnt1_30/newregsigned/vsbaseline/' # root
uvttest_path = '/imaging/rf02/Semnet/stc/uvttest/evoked/pnt1_30/newregsigned/vsbaseline/' # root
avg_path= '/imaging/rf02/Semnet/stc/GrandAverage/evoked/pnt1_30/newregsigned/vsbaseline/'
...
#    n_permutations=5000
    for condavg in range(n_levels):
        ...
        out_file4=avg_path + 'Granavg_icomorphed_newreg_19subj_SemDec_m300_600_25ms_pnt1_30ica_'+event_names[condavg]#+'_'+b
        avg_stc.save(out_file4)
        
        
    for thiscond in range(4):
#        refcond=thiscond+1
#        if thiscond<3:
#            X1=np.squeeze(X[:,:,:,thiscond]-X[:,:,:,refcond])
#        else:
#            X1=np.squeeze(X[:,:,:,refcond]-X[:,:,:,thiscond])
        X1=np.squeeze(X[:,:,:,thiscond].copy())
...
#        print cluster_p_values.min()
        ...
        if len(cluster_p_values)>0:
            if cluster_p_values.min()<0.05:
                ...
                out_file1=out_path + 'ClusPer_abs_icomorphed_newreg_clusterp'+str(p_threshold)[2:]+'_p'+str(p_thr)[2:]+'_19subj_SemDec_pnt1_30ica_'+event_names[thiscond]+'_maxstep'+str(max_step)#+'_'+event_names[refcond]
                ...
                out_file2=out_path + 'ClusPer_abs_sw_icomorphed_newreg_clusterp'+str(p_threshold)[2:]+'_p'+str(p_thr)[2:]+'_19subj_SemDec_pnt1_30ica_'+event_names[thiscond]+'_maxstep'+str(max_step)#+'_'+event_names[refcond]+
