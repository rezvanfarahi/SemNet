...
###############################################################################
# Set parameters
out_path = '/imaging/rf02/Semnet/stc/permutation/evoked/pnt1_30/masked2/' # root
uvttest_path = '/imaging/rf02/Semnet/stc/uvttest/evoked/pnt1_30/' # root
...
p_list=[0.1,0.05,0.045,0.04,0.03,0.025,0.02,0.01,0.008,0.005,0.002,0.001, 0.0008,0.0005,0.0002,0.0001,0.00005,0.00001]#0.001,0.0009,0.0008,0.0007,0.0006,0.0005]#
print "subject_inds:"
print subject_inds
print "No rejection"
...
#X_list=range(2)
event_names = ['Visual','Hand','Hear', 'Visual']#'Hear']#, 'Neutral', 'Emotional','Pwordc']
#thiscond=0
#refcond=3
n_levels=4
n_times=13
...
for p_threshold in ll:
    ...
    for ii, meg in enumerate(list_all):
        ...
        for event_no in range(n_levels):     
            fname_in = data_path + meg + 'firstMorphed_ico_SemDec_pnt1_30ica_'+event_names[event_no]+'_Source_Evoked_m300_600'#'firstMorphed_ico_SemDec_pnt1_30ica_'+event_names[event_no]+'_Source_Evoked_mnrsmpl_50_550_100ms_0ov'#'_Source_Evoked_m300_600'#'_Source_Evoked_mnrsmpl_50_550_100ms_0ov'
            stc_cond = read_source_estimate(fname_in)
            stc_cond.resample(40)
            stc_cond.crop(0.100,0.400)
...
#    n_permutations=5000
    for thiscond in range(3):
        ...
        if thiscond==0: #vis hand
            fname_label = '/imaging/rf02/Semnet/masks/mask_vishand_L-lh.label'; labelL = mne.read_label(fname_label)
            fname_label = '/imaging/rf02/Semnet/masks/mask_vishand_R-rh.label'; labelR = mne.read_label(fname_label)
        if thiscond==1: # hand hear
            fname_label = '/imaging/rf02/Semnet/masks/mask_handhear_L-lh.label'; labelL = mne.read_label(fname_label)
            fname_label = '/imaging/rf02/Semnet/masks/mask_handhear_R-rh.label'; labelR = mne.read_label(fname_label)
        if thiscond==2: # vis hear
            fname_label = '/imaging/rf02/Semnet/masks/mask_vishear_L-lh.label'; labelL = mne.read_label(fname_label)
            fname_label = '/imaging/rf02/Semnet/masks/mask_vishear_R-rh.label'; labelR = mne.read_label(fname_label)
        labelss=labelL+labelR
        bb=stc_cond.in_label(labelss)
        fsave_vertices = [np.arange(10242), np.arange(10242)]
        nnl=np.in1d(fsave_vertices[0],bb.lh_vertno)
        ...
        if len(cluster_p_values)>0:
            ...
            if cluster_p_values.min()<=0.05:
                ...
                out_file1=out_path + 'ClusPer_abs_icomorphed_clusterp'+str(p_threshold)[2:]+'_p'+str(p_thr)[2:]+'_19subj_SemDec_pnt1_30ica_'+event_names[thiscond]+'_'+event_names[refcond]+'_maxstep'+str(max_step)
                ...
                out_file2=out_path + 'ClusPer_abs_sw_icomorphed_clusterp'+str(p_threshold)[2:]+'_p'+str(p_thr)[2:]+'_19subj_SemDec_pnt1_30ica_'+event_names[thiscond]+'_'+event_names[refcond]+'_maxstep'+str(max_step)
