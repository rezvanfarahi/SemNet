...
out_path = '/imaging/rf02/Semnet/stc/permutation/connectivity/'    # where event files are
...
labellist2=[labellist21[ii] for ii in np.array([0,1,4,5,6,7])]
event1='Hear'
event2='Hand'
...
#            	X=np.zeros((20484,2,17))
            	Xce=np.zeros((20484,3,4,n_subjects))
            	Xae=np.zeros((20484,3,4,n_subjects))
            	Xco=np.zeros((20484,3,4,n_subjects))
            	Xao=np.zeros((20484,3,4,n_subjects))
            ...
            ### Concrete
            	...
            	Xcfo=np.hstack((np.squeeze(Xco[:,:,0,:]),np.squeeze(Xco[:,:,1,:]),np.squeeze(Xco[:,:,2,:]),np.squeeze(Xco[:,:,3,:]))) #mi,  coh
            	Xafo=np.hstack((np.squeeze(Xao[:,:,0,:]),np.squeeze(Xao[:,:,1,:]),np.squeeze(Xao[:,:,2,:]),np.squeeze(Xao[:,:,3,:])))
                   
            	X = np.subtract(Xcfe,Xafe)#np.subtract(Xcpca,Xapca)
            	X1 = np.subtract(Xcfo,Xafo)             
            	...
            	T_obst, clusterst, cluster_p_valuest, H0t = clut = spatio_temporal_cluster_1samp_test(X, X1,connectivity=connectivity,  n_permutations=n_permutations, n_jobs=4, threshold=t_threshold, tail=0,t_power=1, spatial_exclude=spatial_exclude,step_down_p=0.05, max_step=1)
            	...
            	if cluster_p_valuest.min()<=0.1:
                    ...
                    out_file1=out_path + 'ClusP_Morphed_ico_SD_ica_evenodd_Sub_ctf_ST_allbands_50_450_200ms_ms' + str(max_step) +'_'+ str(p_threshold)[2:5] + '_sx_' + str(p_thresh)[2:5]+ '_' + label_name_sv[0:-3]
                    ...
                    out_file2=out_path + 'ClusP_sw_Morphed_ico_SD_ica_evenodd_Sub_ctf_ST_allbands_50_450_200ms_ms' + str(max_step) +'_sx_'+ str(p_threshold)[2:5] + '_' + str(p_thresh)[2:5]+ '_' + label_name_sv[0:-3]
