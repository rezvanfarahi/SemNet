...
uvttest_path = '/imaging/rf02/Semnet/stc/uvttest/TF/' # root
...
tstep1=1000/60.
n_times=np.int(np.round(900/(tstep1)))
tmin1=-300
...
for p_threshold in ll:
    ...
    for ii, meg in enumerate(list_all):
        print ii
        for  event_no, this_event in enumerate(event_names):     
...
#            stc_cond.crop(0.050,0.450)
            X[ii,:,:,event_no]=np.abs(np.log(np.transpose(stc_cond.data,[1,0]))) #[:,350:650]
...
#            return f_mway_rm(xx2, factor_levels=factor_levels,effects=effects, return_pvals=False)[0]    
    ...
    for thiscond in np.array([0,1,2]):#in range(5):
        ...
        out_file3=uvttest_path + 'mineMorphed_ico_SemDec_ica_'+this_event+'_Power_normori_ratio_equalized_m300_600_200hz_' + b+'_'+event_names[thiscond]+'_'+event_names[refcond]
