...
###############################################################################
# Set parameters
out_path = '/imaging/rf02/Semnet/stc/permutation/evoked/bands/newregsigned/mugamma_ms2/' # root
uvttest_path = '/imaging/rf02/Semnet/stc/uvttest/evoked/bands/newregsigned/mugamma_ms2/' # root
...
#        T_obs, clusters, cluster_p_values, H0 = clu = spatio_temporal_cluster_1samp_test(X1, connectivity=connectivity,  threshold=t_threshold, tail=tail, t_power=1, step_down_p=0.05, spatial_exclude=spatial_exclude, n_permutations=n_permutations, n_jobs=4)#, max_step=0)#, max_step=0)#spatial_exclude=exclude_ver, 
    ...
    T_obs, clusters, cluster_p_values, H0 = clu = \
        spatio_temporal_cluster_test(X_list, connectivity=connectivity, n_jobs=8,max_step=2,#step_down_p=0.05,#max_step=max_step,
...
#        is multiple-comparisons corrected).
    ...
    good_cluster_inds1 = np.where(cluster_p_values <0.1)[0]
    print cluster_p_values[good_cluster_inds]; print good_cluster_inds; 
    print cluster_p_values[good_cluster_inds1]; print good_cluster_inds1; 
#        print cluster_p_values.min()
    ...
    if len(cluster_p_values)>0:
        if cluster_p_values.min()<0.1:
            ...
            T=2*np.divide(T_obs,np.absolute(T_obs))
            T1=np.divide(T_obs,np.absolute(T_obs))
            for cc in range(good_cluster_inds1.shape[0]):
              	Matx[clusters[good_cluster_inds1[cc]][1],clusters[good_cluster_inds1[cc]][0]]=T1[clusters[good_cluster_inds1[cc]][0],clusters[good_cluster_inds1[cc]][1]].copy()

            for cc in range(good_cluster_inds.shape[0]):
              	Matx[clusters[good_cluster_inds[cc]][1],clusters[good_cluster_inds[cc]][0]]=T[clusters[good_cluster_inds[cc]][0],clusters[good_cluster_inds[cc]][1]].copy()
