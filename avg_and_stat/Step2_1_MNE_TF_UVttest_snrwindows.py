...
###############################################################################
# Set parameters
out_path = '/imaging/rf02/Semnet/stc/uvttest/TF/' # root
uvttest_path = '/imaging/rf02/Semnet/stc/uvttest/TF/snrwins/' # root
...
#X_list=range(2)
event_names = ['Visual','Hand', 'Hear','Pword']#, ,'Pwordc']'Neutral', 'Emotional',
#thiscond=0
refcond=3
...
for p_threshold in ll:
    ...
    for ii, meg in enumerate(list_all):
        ...
        for event_no, this_event in enumerate(event_names):#for event_no in range(n_levels):     
            fname_in = data_path + meg + 'mineMorphed_ico_SemDec_ica_'+this_event+'_Power_normori_ratio_equalized_m500_700_200hz_' + b#'firstMorphed_ico_SemDec_ica_'+event_names[event_no]+'_Source_Evoked_mnrsmpl_50_550_100ms_0ov'#'_Source_Evoked_m300_600'#'_Source_Evoked_mnrsmpl_50_550_100ms_0ov'
            stc_cond = mne.read_source_estimate(fname_in)
#            stc_cond.resample(100)
            stc_condc=stc_cond.copy()
            stc_condc.crop(0.100,0.160)
            X[ii,0,:,event_no]=stc_condc.data.mean(axis=1) #[:,350:650]
            stc_condc=stc_cond.copy()
            stc_condc.crop(0.170,0.230)
            X[ii,1,:,event_no]=stc_condc.data.mean(axis=1) #np.transpose(stc_condc.data.mean(axis=1),[1,0])
            stc_condc=stc_cond.copy()
            stc_condc.crop(0.230,0.330)
            X[ii,2,:,event_no]=stc_condc.data.mean(axis=1) #np.transpose(stc_condc.data.mean(axis=1),[1,0])
            stc_condc=stc_cond.copy()
            stc_condc.crop(0.330,0.410)
            X[ii,3,:,event_no]=stc_condc.data.mean(axis=1) #np.transpose(stc_condc.data.mean(axis=1),[1,0])
            stc_condc=stc_cond.copy()
            stc_condc.crop(0.410,0.470)
...
#            return f_mway_rm(xx2, factor_levels=factor_levels,effects=effects, return_pvals=False)[0]    
    ...
    for thiscond in np.array([0,1,2]):#in range(5):
        ...
        out_file3=uvttest_path + 'UVTtest_snrwins_mineMorphed_ico_SemDec_ica_'+this_event+'_Power_normori_ratio_equalized_m300_600_200hz_' + b+'_'+event_names[thiscond]+'_'+event_names[refcond]#icomorphed_19subj_SemDec_m300_600_100ms_pnt1_30ica_'+event_names[thiscond]+'_'+event_names[refcond]
