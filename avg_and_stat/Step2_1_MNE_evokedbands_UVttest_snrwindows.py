...
###############################################################################
# Set parameters
out_path = '/imaging/rf02/Semnet/stc/uvttest/evoked/' # root
uvttest_path = '/imaging/rf02/Semnet/stc/uvttest/evoked/bands/100wins/' # root
...
#X_list=range(2)
event_names = ['Visual','Hand', 'Hear','Pwordc']#, ,'Pwordc']'Neutral', 'Emotional',
#thiscond=0
refcond=3
...
tmin1=50
tstep1=400
#100-160,170-230,230-330,330-410, 410-470
...
for b in bands:
    ...
    for ii, meg in enumerate(list_all):
        print ii
        for event_no, this_event in enumerate(event_names):#for event_no in range(n_levels):     
            fname_in = data_path + meg + 'firstMorphed_ico_SemDec_pnt1_48ica_'+event_names[event_no]+'_Source_Evoked_m300_600_'+b#'mineMorphed_ico_SemDec_ica_'+this_event+'_Power_normori_ratio_equalized_m500_700_200hz_' + b#'firstMorphed_ico_SemDec_ica_'+event_names[event_no]+'_Source_Evoked_mnrsmpl_50_550_100ms_0ov'#'_Source_Evoked_m300_600'#'_Source_Evoked_mnrsmpl_50_550_100ms_0ov'
            stc_cond = mne.read_source_estimate(fname_in)
#            stc_cond.resample(100)
            stc_condc=stc_cond.copy()
            stc_condc.crop(0.05,0.45)#(0.100,0.160)
            X[ii,0,:,event_no]=stc_condc.data.mean(axis=1) #[:,350:650]
#            stc_condc=stc_cond.copy()
#            stc_condc.crop(0.15,0.25)#(0.170,0.230)
#            X[ii,1,:,event_no]=stc_condc.data.mean(axis=1) #np.transpose(stc_condc.data.mean(axis=1),[1,0])
#            stc_condc=stc_cond.copy()
#            stc_condc.crop(0.25,0.35)#(0.230,0.330)
#            X[ii,2,:,event_no]=stc_condc.data.mean(axis=1) #np.transpose(stc_condc.data.mean(axis=1),[1,0])
#            stc_condc=stc_cond.copy()
#            stc_condc.crop(0.35,0.45)#(0.330,0.410)
#            X[ii,3,:,event_no]=stc_condc.data.mean(axis=1) #np.transpose(stc_condc.data.mean(axis=1),[1,0])
#            stc_condc=stc_cond.copy()
#            stc_condc.crop(0.45,0.55)#(0.410,0.470)
#            X[ii,4,:,event_no]=stc_condc.data.mean(axis=1) #np.transpose(stc_condc.data.mean(axis=1),[1,0])
...
#            return f_mway_rm(xx2, factor_levels=factor_levels,effects=effects, return_pvals=False)[0]    
    ...
    for thiscond in np.array([0,1,2]):#in range(5):
        Matx1=np.squeeze(np.subtract(X[:,:,:,thiscond],X[:,:,:,refcond]))
        Matx1=Matx1[:,np.newaxis,:]
        ...
        out_file3=uvttest_path + 'UVTtest_snrwins_mineMorphed_ico_SemDec_ica_evoked_m300_600_' + b+'_'+event_names[thiscond]+'_'+event_names[refcond]#icomorphed_19subj_SemDec_m300_600_100ms_pnt1_30ica_'+event_names[thiscond]+'_'+event_names[refcond]
