...
###############################################################################
# Set parameters
#out_path = '/imaging/rf02/Semnet/stc/permutation/evoked/bands/newregsigned/mugamma_newev/' # root
uvttest_path = '/imaging/rf02/Semnet/stc/uvttest/evoked/bands/newregsigned/mugamma_newev/' # root
avg_path = '/imaging/rf02/Semnet/stc/GrandAverage/evoked/bands/newregsigned/mugamma_newev/'
...
p_list=[0.0005,0.045,0.04,0.03,0.025,0.02,0.01,0.008,0.005,0.002,0.001, 0.0008,0.0005,0.0002,0.0001,0.00005,0.00001]#0.001,0.0009,0.0008,0.0007,0.0006,0.0005]#
print "subject_inds:"
print subject_inds
print "No rejection"
...
#X_list=range(2)
event_names = ['Visual','Hand']# 'Visual']#'Hear']#, 'Neutral', 'Emotional','Pwordc']
    ...
    for ii, meg in enumerate(list_all):
        ...
        for bcnt,b in enumerate(bands):
            ...
            for evcnt, event_name in enumerate(event_names):                
                fname_in = data_path + meg + 'firstMorphed_ico_SemDec_pnt1_48ica_'+event_name+'_Source_signedEvoked_m300_600_'+b#'firstMorphed_ico_SemDec_pnt1_30ica_'+event_names[event_no]+'_Source_Evoked_mnrsmpl_50_550_100ms_0ov'#'_Source_Evoked_m300_600'#'_Source_Evoked_mnrsmpl_50_550_100ms_0ov'
...
#            X[ii,:,:,event_no]=np.transpose(stc_cond.data,[1,0])
    ...
    f=np.zeros((thisX.shape[2],thisX.shape[3]))
    for xii in range(thisX.shape[3]):#over time
        f[:,xii], p= f_mway_rm(thisX[:,:,:,xii], factor_levels=factor_levels,effects=effects, return_pvals=return_pvals)
    matx_stc = mne.SourceEstimate(f, vertices=vertices_avg,tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='fsaverage')
    out_file2=uvttest_path + 'UVttest_sEvoked_icomorphed_newreg_19subj_SemDec_pnt1_30ica_'+event_names[0]+'_'+event_names[1]+'_gammamu'
#    matx_stc.save(out_file2)
    Xalpha=X[:,:,:,0].copy()-X[:,:,:,1].copy()
    Xgamma=X[:,:,:,2].copy()-X[:,:,:,3].copy()
    #alpha
    Xalpha_uvttest=np.transpose(Xalpha.copy(),[0,2,1])#np.zeros((n_subjects,20484,n_times))
    MTalpha=np.zeros((20484,n_times))

    for cnt in range(Xalpha_uvttest.shape[2]):
        print cnt
        MTalpha[:,cnt]= mne.stats.ttest_1samp_no_p(Xalpha_uvttest[:,:,cnt], sigma=1e-3, method='relative')
    tval_stc = mne.SourceEstimate(MTalpha, vertices=vertices_avg,tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='fsaverage')
    out_file3=uvttest_path + 'UVttest_Alpha_sEvoked_icomorphed_newreg_19subj_SemDec_pnt1_30ica_'+event_names[0]+'_'+event_names[1]
    tval_stc.save(out_file3)
    
    #alpha
    Xgamma_uvttest=np.transpose(Xgamma.copy(),[0,2,1])#np.zeros((n_subjects,20484,n_times))
    MTgamma=np.zeros((20484,n_times))

    for cnt in range(Xgamma_uvttest.shape[2]):
        print cnt
        MTgamma[:,cnt]= mne.stats.ttest_1samp_no_p(Xgamma_uvttest[:,:,cnt], sigma=1e-3, method='relative')
    tval_stc = mne.SourceEstimate(MTgamma, vertices=vertices_avg,tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='fsaverage')
    out_file3=uvttest_path + 'UVttest_Gamma_sEvoked_icomorphed_newreg_19subj_SemDec_pnt1_30ica_'+event_names[0]+'_'+event_names[1]
    tval_stc.save(out_file3)
    
    Xalpha_mean=np.transpose(np.mean(Xalpha,0),[1,0])#np.zeros((n_subjects,20484,n_times))
    avg_stc = mne.SourceEstimate(Xalpha_mean, vertices=vertices_avg,tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='fsaverage')
    out_file3=avg_path + 'Grandavg_Alpha_sEvoked_icomorphed_newreg_19subj_SemDec_pnt1_30ica_'+event_names[0]+'_'+event_names[1]
    avg_stc.save(out_file3)
    
    #alpha
    Xgamma_mean=np.transpose(np.mean(Xgamma,0),[1,0])#np.zeros((n_subjects,20484,n_times))
    avg_stc = mne.SourceEstimate(Xgamma_mean, vertices=vertices_avg,tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='fsaverage')
    out_file3=avg_path + 'Grandavg_Gamma_sEvoked_icomorphed_newreg_19subj_SemDec_pnt1_30ica_'+event_names[0]+'_'+event_names[1]
    avg_stc.save(out_file3)
    
    
#stc.plot(hemi='both', views='lateral',subjects_dir=data_path,subject='fsaverage',time_viewer=True,colormap='Set1')
