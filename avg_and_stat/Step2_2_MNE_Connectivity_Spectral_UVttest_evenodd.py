...
out_path = '/imaging/rf02/Semnet/stc/uvttest/connectivity/'    # where event files are
if not os.path.exists(out_path):
    os.makedirs(out_path)

avg_path = '/imaging/rf02/Semnet/stc/GrandAverage/connectivity/'    # where event files are
...
labellist_sv1=['lhATL-lh','lhAG-lh','lhvis-lh','rhvis-rh','lhaud-lh','rhaud-rh','lhhnd-lh','rhhnd-rh']
...
labellist=[labellist1[ii] for ii in np.array([0,1,4,5,6,7])]
labellist_sv=[labellist_sv1[ii] for ii in np.array([0,1,4,5,6,7])]
event1='Hear'
event2='Hand'
...
for label_name, label_name_sv in zip(labellist, labellist_sv):
    ...
    for con_method in conn_methods:
        for band in bands:
            MT=np.zeros((20484,3))
            ii=-1; print band
            X_even=np.zeros((19,20484,3))
            Xa_even=np.zeros((20484,3,19))
            X_odd=np.zeros((19,20484,3))
            Xa_odd=np.zeros((20484,3,19))
            ...
            for meg in list_all:
                ...
                ### Concrete
                fname_event1e = data_path + meg + 'Morphed_ico_SemDec_even_ica_equalized_'+event1+'_Mlttap_new_ctf_Coh_'+band+'_150_450_200ms_' + label_name 
                fname_event1o = data_path + meg + 'Morphed_ico_SemDec_odd_ica_equalized_'+event1+'_Mlttap_new_ctf_Coh_'+band+'_150_450_200ms_' + label_name      
                stc_event1e = mne.read_source_estimate(fname_event1e)#; stc_cncrt.data[stc_cncrt.data<0]=0 
                stc_event1o = mne.read_source_estimate(fname_event1o)#; stc_cncrt.data[stc_cncrt.data<0]=0 
                fname_event2e = data_path + meg + 'Morphed_ico_SemDec_even_ica_equalized_'+event2+'_Mlttap_new_ctf_Coh_'+band+'_150_450_200ms_' + label_name   
                fname_event2o = data_path + meg + 'Morphed_ico_SemDec_odd_ica_equalized_'+event2+'_Mlttap_new_ctf_Coh_'+band+'_150_450_200ms_' + label_name   
                stc_event2e = mne.read_source_estimate(fname_event2e)#; stc_abs.data[stc_abs.data<0]=0 	
                stc_event2o = mne.read_source_estimate(fname_event2o)#; stc_abs.data[stc_abs.data<0]=0 	
                ...
                Xce[:,:,bcnt,ii]=data_event1e#np.dstack((data_cncrt_mi,data_cncrt_wpli2d,data_cncrt_coh))
                Xae[:,:,bcnt,ii]=data_event2e#np.dstack((data_abs_mi,data_abs_wpli2d,data_abs_coh))
                data_event1o = np.abs(stc_event1o.data)#; data_cncrt_wpli2d = np.abs(stc_cncrt_wpli2d.data); data_cncrt_coh = np.abs(stc_cncrt_coh.data)
                data_event2o = np.abs(stc_event2o.data)#; data_abs_wpli2d = np.abs(stc_abs_wpli2d.data); data_abs_coh = np.abs(stc_abs_coh.data)
                Xco[:,:,bcnt,ii]=data_event1o#np.dstack((data_cncrt_mi,data_cncrt_wpli2d,data_cncrt_coh))
                Xao[:,:,bcnt,ii]=data_event2o#np.dstack((data_abs_mi,data_abs_wpli2d,data_abs_coh))

                fname_cncrt = data_path + meg + 'Morphed_ico_SemDec_even_ica_equalized_'+event1+'_Mlttap_new_ctf_Coh_'+band+'_150_450_200ms_' + label_name                   
                stc_cncrt = mne.read_source_estimate(fname_cncrt); #stc_cncrt.data[stc_cncrt.data<0]=0 
                fname_abs = data_path + meg + 'Morphed_ico_SemDec_even_ica_equalized_'+event2+'_Mlttap_new_ctf_Coh_'+band+'_150_450_200ms_' + label_name
            ...
            data_out = out_path +  'UVttest_Morphed_ico_SD_ica_even_Sub_Mlttap_'+event1+'_'+event2+'_ctf_'+con_method+'_'+band+'_150_450_200ms_lh' + label_name_sv[0:-3] 
            MT_stc.save(data_out)
            Xavg_stc = mne.SourceEstimate(Xavg, vertices=stc_abs.vertices, tmin= 0.05, tstep= 0.1, subject='fsaverage')
            data_out = avg_path +  'GrandAverage_Morphed_ico_SD_ica_even_Sub_Mlttap_'+event1+'_'+event2+'_ctf_'+con_method+'_'+band+'_150_450_200ms_lh' + label_name_sv[0:-3] 
            Xavg_stc.save(data_out)
            Xav_stc = mne.SourceEstimate(Xav, vertices=stc_abs.vertices, tmin= 0.05, tstep= 0.1, subject='fsaverage')
            data_out = avg_path +  'GrandAverage_Morphed_ico_SD_ica_even_Avg_Mlttap_'+event1+'_'+event2+'_ctf_'+con_method+'_'+band+'_150_450_200ms_lh' + label_name_sv[0:-3] 
