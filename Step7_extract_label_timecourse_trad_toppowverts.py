...
labellist_path = [label_path+'vis_final-lh.label',label_path+'auditory_final_manual2-lh.label',label_path+'hand_c2-lh.label',label_path+'ATL_manual-lh.label',label_path+'AG_manual3-lh.label',label_path+'lh.parsorbitalis.label']#[ 'lh.posterior_fusiform','rh.posterior_fusiform', 'lh.lateraloccipital','rh.lateraloccipital','lh.precentral','rh.precentral', 'lh.ATL_latmed','rh.ATL_latmed']#labellist = [ 'leftATL-lh','rightATL-rh']#, 'medialtempright-rh','medialtempleft-lh']'lh.postcentral','rh.postcentral',
...
for index, meg in enumerate(ll):
    ...
    print "Epoching"
    if subject_no in np.array([3,5]):#np.array([4,8,9]):
...
#        ediv=np.sqrt(edv)
        ...
        src_avg = mne.read_source_spaces(srcin)#inverse_operator['src']  # the source space used
        tminl=0
        tstepl=1
        ...
        thisstc_scale = mne.SourceEstimate(thisscale[:,np.newaxis], vertices=vertices_avg,tmin=1e-3 * tminl, tstep=1e-3 * tstepl, subject='fsaverage')
...
#        this_stc_none=this_condition_none.morph_precomputed(subject_to,vertices_to,this_morphmat, subject_from)
        ...
        tminl=0
        tstepl=1
        for lblii,thislbl in enumerate(labellist):
            thislbl_data=this_stc_normal.in_label(thislbl).data#np.ones((len(vertices_avg[0])+len(vertices_avg[1]),1))
            lbas=np.argsort(np.sum(thislbl_data.copy()**2,1))[-1::-1]
#            lbas=np.argsort(np.sum(thislbl_data**2,1))[-1::-1]
            thisscale_data=np.squeeze(thisstc_scale.in_label(thislbl).data)#-mne.label.label_sign_flip(thislbl,src_avg)
            signeddata=np.tile(thisscale_data[:,np.newaxis],(1,thislbl_data.shape[1]))*thislbl_data
#            bestlen=int(0.8*thislbl_data.shape[0])
            finaldata=np.mean(signeddata,0)#np.mean(signeddata[lbas[:bestlen],:],0)
            tcmat_normf[index,lblii,:,event_no]=finaldata.copy()

#        thisstc_tolabel = mne.SourceEstimate(thislbl_data, vertices=vertices_avg,tmin=1e-3 * tminl, tstep=1e-3 * tstepl, subject='fsaverage')
#        thisstc_label=mne.stc_to_label(thisstc_tolabel, src_avg)
#        thisscale=-np.hstack((mne.label.label_sign_flip(thisstc_label[0],src_avg),mne.label.label_sign_flip(thisstc_label[1],src_avg)))
#
#        predata1=this_stc_normal.data.copy()
#        ntimes=len(this_stc_normal.times)
...
#
#        
#        label_ts_normal = mne.extract_label_time_course(this_stc_normal, labellist, src_avg, mode='mean_flip',return_generator=False)
...
this_pathto=out_path+'ROItcs_normal_meanflip_top80_ROI_ATAGIFGMTGVISHRHN_evVisHrHn.mat'#'DKA68_orig_modified_coh.mat'#lh.fsaverage_finalpostparc_smooth_split_merge.annot            
...
for roino in roinos:
    ...
    for conno1 in connos1:
        ...
        for conno2 in np.setdiff1d(np.asarray(connos2),np.asarray([conno1])):
            print conno2
            X=np.zeros((19,len(tlowlim)))
            #            tst=350
            Xall=np.zeros((19,len(tlowlim),21))
            ii=-1
...
T_obs, clusters, cluster_p_values, H0 = mne.stats.permutation_cluster_test(X_list, connectivity=None,  threshold=t_threshold, stat_fun=stat_fun, tail=tail, t_power=1,  n_permutations=n_permutations, n_jobs=4)#step_down_p=0.05,, max_step=5)#, max_step=0)#spatial_exclude=exclude_ver, 
plt.plot(T_obs)
print "interaction"
print clusters
print cluster_p_values

thisX=np.swapaxes(Xall[:,:,np.array([4,6,18,20])],1,2)
