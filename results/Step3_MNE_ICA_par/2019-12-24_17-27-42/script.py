...
for cnt, meg in enumerate(ll):
	...
	print (meg)
	fname = data_path + meg + 'block_milk_tsss_filtSL_1_48_raw.fif'
...
#	raw.filter(l_freq=0.1, h_freq=30,  l_trans_bandwidth='auto',h_trans_bandwidth='auto', picks=picks, method='fft',filter_length='auto')#,phase='zero-double')
	...
	for picks, ch_type in get_data_picks(raw, meg_combined=ica_meg_combined):
         if 1>0:#ch_type is 'eeg': #
             ...
             out_path=data_path+meg+'filtSL48_milk_{}-ica.fif'.format(ch_type)
...
#	run_id=subject
	...
	for k, v in artifact_stats.items():
         print(k, v)  
	report.save(op.join(results_dir,'preprocessing_filtSL48_milk-report-{}.html'.format(subject)),open_browser=True, overwrite=True)
     
	ica_path=data_path+meg+'filtSL48_milk_meg-ica.fif'
	ica=read_ica(ica_path)
	raw_orig=ica.apply(raw_orig)
	ica_path=data_path+meg+'filtSL48_milk_eeg-ica.fif'
	ica=read_ica(ica_path)
	raw_orig=ica.apply(raw_orig)
	data_out_path=data_path+meg+'block_milk_tsss_filtSL_1_48_ica_raw.fif'
