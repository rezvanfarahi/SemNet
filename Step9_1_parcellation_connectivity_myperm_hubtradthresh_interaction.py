# -*- coding: utf-8 -*-
"""
Created on Sat Oct 14 01:43:40 2017
...
#        print ii
    ...
    thisXm=np.mean(thisX,0)
    alphadiff=thisXm[2,:,:]-thisXm[0,:,:]
    gammadiff=thisXm[3,:,:]-thisXm[1,:,:]
...
#        MT[:,uvcnt]= mne.stats.ttest_1samp_no_p(thisX[:,:,uvcnt], sigma=1e-3, method='relative')
        MT[:,uvcnt], p[:,uvcnt]= mne.stats.f_mway_rm(thisX[:,:,:,uvcnt], factor_levels=factor_levels,effects=effects, return_pvals=return_pvals,alpha=p_threshold)
...
#    ppos[MTpos>=t_threshold]=1
    ...
    ss=np.sum(MTpos,1)
#    print ss
    sshub=np.where(ss>np.mean(ss)+2*np.std(ss))[0]#np.where(ss>0)[0]#
...
#    print Tpos_sum
    out_fname=out_path+'ppos_interaction_'+parc_names[wparc]+'_'+event_names[thisev]+'_'+event_names[refev]+'_'+freq_names[thisfreq]+'_'+timesmin[thist]+'_'+timesmax[thist]+'.mat'
    scipy.io.savemat(out_fname,{'ppos_corr':ppos_corr})
...
#    
    ...
    title_alpha='alpha_'+event_names[thisev]+'_'+event_names[refev]+'_'+timesmin[thist]+'_'+timesmax[thist]
    plot_connectivity_circle(alphadiff, label_names,colormap='cool',n_lines=None,
                         node_angles=node_angles, node_colors=label_colors,colorbar=True,linewidth=0.5,#vmin=-0.1,vmax=0.1,#, fontsize_title=14.,fontsize_names=6.,fontsize_colorbar=12.,colorbar_size=0.4,
                         fig=fig, padding=3.0,title=title_alpha)#, subplot=(1, 2, thist+1))
    fig = plt.figure(figsize=(8, 6), facecolor='black') 
    title_gamma='gamma_'+event_names[thisev]+'_'+event_names[refev]+'_'+timesmin[thist]+'_'+timesmax[thist]
    plot_connectivity_circle(gammadiff, label_names,colormap='cool',n_lines=None,
                         node_angles=node_angles, node_colors=label_colors,colorbar=True,linewidth=0.5,#vmin=-0.1,vmax=0.1,#, fontsize_title=14.,fontsize_names=6.,fontsize_colorbar=12.,colorbar_size=0.4,
                         fig=fig, padding=3.0, title=title_gamma)#, subplot=(1, 2, thist+1))                                
