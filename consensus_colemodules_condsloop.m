...
%     D_conds_bands = agreement(squeeze(conds_bands_M(:,:,bbcnt,condcnt)))/nrands;
    ciu_conds_bands(:,ii,thist,bbcnt,condcnt) = fcn_consensus_communities(squeeze(conds_bands_M(:,:,bbcnt,condcnt)),reps);%consensus_und(D_conds_bands,tau,reps);
...
ciu_grand(:,gcnt) = fcn_consensus_communities(squeeze(grand_mat_sub_time_band_cond(:,:,gcnt)),reps);%consensus_und(squeeze(D_grand(:,:,gcnt)),tau,reps);
% thistm=2
...
b=(sum(b)-1)/(size(a,1)-1);%b=b-diag(diag(b));
[bm,bmw]=max(b);
% bmw=10;
[ciugs,ciugsw]=sort(ciu_grand(:,bmw));
out_file_grandmat=[out_path,'grand_mat_cole_maxctf_3conds.mat'];
grand_mat=grand_mat_sub_time_band_cond;
save(out_file_grandmat,'grand_mat')
% out_file_Dgrand=[out_path,'D_grand_maxctf_3conds.mat'];
% save(out_file_Dgrand,'D_grand')
out_file_ciugrand=[out_path,'ciu_grand_cole_maxctf_3conds.mat'];
save(out_file_ciugrand,'ciu_grand')
wmodules=ciugsw;
finalmodules=ciugs;
out_file_modules=[out_path,'wmodules_cole_maxctf_3conds.mat'];
save(out_file_modules,'wmodules')
out_file_modules=[out_path,'finalmodules_cole_maxctf_3conds.mat'];
