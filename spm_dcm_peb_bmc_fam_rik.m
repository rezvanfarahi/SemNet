function [BMA,fam] = spm_dcm_peb_bmc_fam_rik(BMA,BMR,families,bma_option)
...
include = find(families~=0);
fams    = unique(families(include));
nK      = length(fams);             % Number of (non-zero) families   
Nm      = length(include);          % Number of (non-zero) models    

% Hacky, but need to get it to work both when =1 AND >1 between-subject factor?
if Nmx == 1
    includeX = 1;
else
    includeX = include;
...
for i = 1:Nm
    for j = 1:Nmx        
        F(i,j) = BMR{include(i),includeX(j)}.F;
    end
end
