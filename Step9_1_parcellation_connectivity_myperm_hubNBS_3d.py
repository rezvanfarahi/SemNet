...
#        print ii
    ...
    thisX=grand_connmat[:,:,:,thist,thisev,:]-grand_connmat[:,:,:,thist,refev,:]
    thisX=np.transpose(thisX,(3,0,1,2))
    MT=np.zeros((nnodes,nnodes,nfreqs))    
...
#        print uvcnt
    ...
    Tpos_sum=np.sum(np.sum(MTpos_1side,2),1)
...
#        print pcnt
    ...
    #            print uvcnt
        ...
        Tpos_sum_perm=np.sum(np.sum(MTpos_1side,2),1)
...
#        this_MTpos[np.where(comps_pos!=cu_pos[pvalsig])[0],:]=0
        out_fname=out_path+'NBSnetwork_3D_pos'+parc_names[wparc]+'_'+event_names[thisev]+'_'+event_names[refev]+'_'+freq_names[thisfreq]+timesmin[thist]+'_'+timesmax[thist]+'_'+str(p_threshold)[2:]+'.mat'#'_'+str(thisp)[2:6]+
...
#        print uvcnt
    ...
    Tneg_sum=np.sum(np.sum(MTneg_1side,2),1)
...
#        print pcnt        
    ...
    #            print uvcnt
        ...
        Tneg_sum_perm=np.sum(np.sum(MTneg_1side,2),1)
...
#        this_MTneg[np.where(comps_neg!=cu_neg[pvalsig])[0],:]=0
        out_fname=out_path+'NBSnetwork_3D_neg'+parc_names[wparc]+'_'+event_names[thisev]+'_'+event_names[refev]+'_'+freq_names[thisfreq]+timesmin[thist]+'_'+timesmax[thist]+'_'+str(p_threshold)[2:]+'.mat'#'_'+str(thisp)[2:6]+
