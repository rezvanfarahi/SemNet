...
labellist_path = [label_path+'vis_final-lh.label',label_path+'auditory_primary2-lh.label',label_path+'hand_c2-lh.label',label_path+'ATL_manual-lh.label',label_path+'AG_manual3-lh.label',label_path+'lh.parsorbitalis.label']#[ 'lh.posterior_fusiform','rh.posterior_fusiform', 'lh.lateraloccipital','rh.lateraloccipital','lh.precentral','rh.precentral', 'lh.ATL_latmed','rh.ATL_latmed']#labellist = [ 'leftATL-lh','rightATL-rh']#, 'medialtempright-rh','medialtempleft-lh']'lh.postcentral','rh.postcentral',
...
this_pathto=out_path+'wbw_ROItcs_normal_meanflip_to30_ROI_ATAGIFGMTGVISHRHN_evVisHrHn3.mat'#'DKA68_orig_modified_coh.mat'#lh.fsaverage_finalpostparc_smooth_split_merge.annot            
...
#aud: 5, 12, 19
#hnd: 6, 13, 20
X_list=[np.squeeze(x) for x in np.split(Xall[:,:,np.array([4,5,6,11,12,13,18,19,20])], 9, axis=-1)]#np.array([1,3])#ROI3 in cond 1, 2, 3
#0-6,7-13,14-20
...
#f_thresh = -stats.distributions.t.ppf(pthresh/2., n_subjects - 1)#
...
thisX=np.swapaxes(Xall[:,:,np.array([12,13,19,20])],1,2)#subxconxtime
