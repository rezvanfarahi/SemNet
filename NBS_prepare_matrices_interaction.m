...
dmat=cat(1,eye(nsubs),eye(nsubs),eye(nsubs),eye(nsubs));
conddmat=[ones(2*nsubs,1);-1*ones(2*nsubs,1)];
banddmat=[ones(nsubs,1);-1*ones(nsubs,1);ones(nsubs,1);-1*ones(nsubs,1)];
dmat=[dmat,conddmat,banddmat,conddmat.*banddmat];
% dmat=ones(nsubs,1);
fname_out=[data_path,'/NBS1.2/Rezvan/','designMatrix_vishnd_alphagamma_',timesmin{thist},'_',timesmax{thist},'.mat'];
save(fname_out,'dmat')
...
pval=ones(nnodes,nnodes);
for p1=1:nnodes
    for p2=setdiff(1:nnodes,p1)
