...
#        print uvcnt
    ...
    MT[np.abs(MT)<t_threshold]=0#np.abs(MT)
    MT[np.abs(MT)>=t_threshold]=1
    this_conn=MT.copy()#np.mean(thisX,2)

    fig = plt.figure(num=thist+1, figsize=(8, 6), facecolor='black')
    ...
    plot_connectivity_circle(this_conn, range(len(label_names)),colormap='hot',n_lines=None,
