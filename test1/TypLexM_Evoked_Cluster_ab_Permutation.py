...
p_list=[0.05,0.045,0.04,0.03,0.025,0.02,0.01,0.005,0.008,0.005,0.002,0.001]
print "subject_inds:"
print subject_inds
print "No rejection"
...
for p_threshold in ll:
    for ii, meg in enumerate(list_all):
    	...
    	for cc in range(5):
    		b1=b2+0.0001; b2=b1+0.1-0.0001; print b1; print b2
    	 	X[:,cc,ii]=np.subtract(np.abs(stc_abs.copy().crop(b1,b2).mean().data.squeeze()),np.abs(stc_abs.copy().crop(-0.3,-0.2).mean().data.squeeze()))#stc_subtract.copy().crop(b1,b2).mean().data.squeeze()
    ...
    T_obs, clusters, cluster_p_values, H0 = clu = spatio_temporal_cluster_1samp_test(X, connectivity=connectivity,  threshold=t_threshold, tail=0)#, max_step=0)#, max_step=0)#spatial_exclude=exclude_ver, 
    ...
    out_file1=out_path + 'ClusterPermutation_abs_firstmorphed_clusterp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_17subj_SemLoc_icaclean_AbstractBaseline_50_550_100ms_0overlap'
    ...
    out_file2=out_path + 'ClusterPermutation_abs_stepwise_firstmorphed_clusterp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_17subj_SemLoc_icaclean_AbstractBaseline_50_550_100ms_0overlap'
