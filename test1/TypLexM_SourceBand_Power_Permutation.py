...
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/stc/permutation/power/' #
...
p_list=[0.05,0.045,0.04,0.03,0.025,0.02,0.01,0.008,0.005,0.002,0.001]
print "subject_inds:"
print subject_inds
print "No rejection"
...
for p_threshold in ll:
    ...
    ### Concrete
    	...
    	fname_cncrt = data_path + meg + 'preMorphed_SemLoc_icaclean_Concrete_Power_ratio_equalized_equalized_m500_700_' + b
    	stc_cncrt = mne.read_source_estimate(fname_cncrt)
    	#abslt_cncrt= np.absolute(stc_cncrt.copy().crop(.3,.4).data)
    	fname_abs = data_path + meg + 'preMorphed_SemLoc_icaclean_Abstract_Power_ratio_equalized_m500_700_' + b
    	stc_abs = mne.read_source_estimate(fname_abs)
    	"""
    	fname_sub = data_path + meg + 'Morphed_SemLoc_icaclean_Subtract_Power_ratio_equalized_m500_700_theta'
    	...
    	for cc in range(5):
    		...
    		print b1; print b2
    		Xc=np.abs(stc_cncrt.copy().crop(b1,b2).mean().data.squeeze())
    		Xa=np.abs(stc_abs.copy().crop(b1,b2).mean().data.squeeze())
    		X[:,cc,ii]=Xc-Xa
    	#X[:,1,:]=X[:,0,:].copy()
    ...
    #p_threshold = 0.05
    n_subjects=17
    t_threshold = -scipy.stats.distributions.t.ppf(p_threshold, n_subjects - 1)
    print('Clustering.')
    max_step=5
    T_obs, clusters, cluster_p_values, H0 = clu = spatio_temporal_cluster_1samp_test(X, connectivity=connectivity, n_jobs=2, threshold=t_threshold,n_permutations=1024,tail=0,t_power=1, step_down_p=0.05)# max_step=max_step)
    ...
    p_thresh=0.05
    fsave_vertices = [np.arange(10242), np.arange(10242)]
    stc_all_cluster_vis = summarize_clusters_stc(clu, tmin=tmin2, tstep=tstep2, p_thresh=p_thresh, subject='fsaverage')
    #t_stc = mne.SourceEstimate(T_obs, vertices=fsave_vertices, tmin=tmin2, tstep=tstep2, subject='average', verbose=None)
    
    out_file1=out_path + 'Permutation_clusterp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_17subj_SemLoc_icaclean_ConcreteAbstract_SourcePower_ratio_equalized_50_550_100ms_maxstep' + str(max_step) +b
    stc_all_cluster_vis.save(out_file1)
    #out_file2=data_path + 'Permutation_SemLoc_ConcreteAbstract_Coherence_Beta_150_350_' + label_name[0:-3]
    ...
    out_file2=out_path + 'Permutation_stepwise_clusterp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_17subj_SemLoc_icaclean_ConcreteAbstract_SourcePower_ratio_equalized_50_550_100ms_maxstep' + str(max_step) +b
