...
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/powertest/'	# where subdirs for MEG data are
...
## Get raw data
	...
	raw2=raw.copy()
	raw2.filter(l_freq=1, h_freq=100, l_trans_bandwidth=0.1, picks=picks, method='fft',filter_length='40s')
	raw3=raw.copy()
	raw3.filter(l_freq=1, h_freq=48, l_trans_bandwidth=0.1, picks=picks, method='fft',filter_length='40s')
	print "filter finished"
	fig=raw.plot_psds(area_mode='range', tmax=500, fmin=0, fmax=500, n_jobs=2)
	fig.savefig(out_path+'psd_subj' + str(ii) + '.png',dpi=fig.dpi)
	pl.close(fig)

	fig=raw2.plot_psds(area_mode='range', tmax=500, fmin=0, fmax=102, n_jobs=2)
	fig.savefig(out_path+'psd_subj' + str(ii) + '_100.png',dpi=fig.dpi)
	pl.close(fig)

	fig=raw3.plot_psds(area_mode='range', tmax=500, fmin=0, fmax=50, n_jobs=2)
	fig.savefig(out_path+'psd_subj' + str(ii) + '_50.png',dpi=fig.dpi)
	pl.close(fig)
	print "psd finished"
	...
	data3, times3 = raw3[picks, 1000:-1000]
	data3=data3**2
	dm3=data3.mean(axis=1)

	pr=np.divide(dm2,dm)
	pr2=np.divide(dm3,dm)
	...
	pl.ylabel('power after filter 1-100 to power before filter ratio')
	pl.show(block=False)
	fig.savefig(out_path+'pr_subj' + str(ii) + '_100.png',dpi=fig.dpi)
	pl.close(fig)

	fig=pl.figure()
	pl.plot(pr2,'.')
	pl.xlabel('number of channel (1:306 MEG, 306:376 EEG)')
	pl.ylabel('power after filter 1-48 to power before filter ratio')
	pl.show(block=False)
	fig.savefig(out_path+'pr_subj' + str(ii) + '_50.png',dpi=fig.dpi)
	pl.close(fig)
	print "pr finished"
	"""
...
# Writing events
	...
	out_name=data_path + meg + 'semloc_ssstf_fft48_raw.fif'
#cd out_path
