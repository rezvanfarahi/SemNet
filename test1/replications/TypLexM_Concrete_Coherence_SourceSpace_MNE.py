...
# lower variance but is slower). By using faverage=False, we directly
...
#		coh, freqs1, times1, n_epochs1, n_tapers1 = spectral_connectivity(stcs, method='cohy', mode='fourier', indices=indices,sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=True, tmin=tmin, tmax=tmax, n_jobs=4)
...
#		coh_stc = mne.SourceEstimate(coh, vertices=stc.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject=subjects[subject_inds[0]])
#		subject_from = subjects[subject_inds[0]]
#    		subject_to = 'avgsubj'
#    		vertices_to = [np.arange(10242), np.arange(10242)]
#		stc_to = mne.morph_data(subject_from, subject_to, coh_stc, subjects_dir=data_path, n_jobs=1, grade=vertices_to)
#		data_out = data_path + meg +  'Morphed_SemLoc_icaclean_Concrete_Coherency_ThetaAlphaBetaGamma_150_350_' + label_name[0:-3]
#		print data_out
...
#spectral_connectivity(data, method='coh', indices=None, sfreq=6.283185307179586, mode='multitaper', fmin=None, fmax=inf, fskip=0, faverage=False, tmin=None, tmax=None, mt_bandwidth=None, mt_adaptive=False, mt_low_bias=True, cwt_frequencies=None, cwt_n_cycles=7, block_size=1000, n_jobs=1, verbose=None)
		...
		subject_from = subjects[subject_inds[0]]
    		subject_to = 'avgsubj'
    		vertices_to = [np.arange(10242), np.arange(10242)]
		stc_to = mne.morph_data(subject_from, subject_to, plv_stc, subjects_dir=data_path, n_jobs=1, grade=vertices_to)
		data_out = data_path + meg +  'Morphed_SemLoc_icaclean_Concrete_PPC_ThetaAlphaBetaGamma_150_350_' + label_name[0:-3]
		print data_out
		stc_to.save(data_out)
