...
for cntt, meg in enumerate(ll):
	...
	stcs_d=stcs.copy().data
	fmin=4; fmax=8
	stcs_d=mne.filter.band_pass_filter(stcs_d,Fs=Fs,Fp1=fmin, Fp2=fmax, method='fft')
	stcs = mne.SourceEstimate(stcs_d, vertices=stcs.vertno, tmin=stcs.tmin, tstep=stcs.tstep, subject='avgsubj')
...
# Generate index parameter for seed-based connectivity analysis
		...
		tstep1 = 100#np.mean(freqs[1]) - tmin
		corr_stc = mne.SourceEstimate(Rc, vertices=stcs.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='avgsubj')
		data_out = data_path + meg +  'Morphed_SemLoc_icaclean_Concrete_Correlation_theta_hilbert_150_450_200ms_' + label_name[0:-3]
		
		corr_stc.save(data_out)
		p_stc = mne.SourceEstimate(Rc, vertices=stcs.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='avgsubj')
		data_out = data_path + meg +  'Morphed_SemLoc_icaclean_Concrete_Correlation_theta_hilbert_pvalues_150_450_200ms_' + label_name[0:-3]
		p_stc.save(data_out)
############
...
# Generate index parameter for seed-based connectivity analysis
		...
		tstep1 = 100#np.mean(freqs[1]) - tmin
		corr_stc = mne.SourceEstimate(Rc, vertices=stcs.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='avgsubj')
		data_out = data_path + meg +  'Morphed_SemLoc_icaclean_Concrete_Correlation_alpha_hilbert_150_450_200ms_' + label_name[0:-3]
		
		corr_stc.save(data_out)
		p_stc = mne.SourceEstimate(Rc, vertices=stcs.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='avgsubj')
		data_out = data_path + meg +  'Morphed_SemLoc_icaclean_Concrete_Correlation_alpha_hilbert_pvalues_150_450_200ms_' + label_name[0:-3]
		p_stc.save(data_out)
############
...
# Generate index parameter for seed-based connectivity analysis
		...
		tstep1 = 100#np.mean(freqs[1]) - tmin
		corr_stc = mne.SourceEstimate(Rc, vertices=stcs.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='avgsubj')
		data_out = data_path + meg +  'Morphed_SemLoc_icaclean_Concrete_Correlation_beta_hilbert_150_450_200ms_' + label_name[0:-3]
		
		corr_stc.save(data_out)
		p_stc = mne.SourceEstimate(Rc, vertices=stcs.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='avgsubj')
		data_out = data_path + meg +  'Morphed_SemLoc_icaclean_Concrete_Correlation_beta_hilbert_pvalues_150_450_200ms_' + label_name[0:-3]
		p_stc.save(data_out)
############
print "beta finished"
...
# Generate index parameter for seed-based connectivity analysis
		...
		tstep1 = 100#np.mean(freqs[1]) - tmin
		corr_stc = mne.SourceEstimate(Rc, vertices=stcs.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='avgsubj')
		data_out = data_path + meg +  'Morphed_SemLoc_icaclean_Concrete_Correlation_gamma_hilbert_150_450_200ms_' + label_name[0:-3]
		
		corr_stc.save(data_out)
		p_stc = mne.SourceEstimate(Rc, vertices=stcs.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='avgsubj')
		data_out = data_path + meg +  'Morphed_SemLoc_icaclean_Concrete_Correlation_gamma_hilbert_pvalues_150_450_200ms_' + label_name[0:-3]
		p_stc.save(data_out)
############
