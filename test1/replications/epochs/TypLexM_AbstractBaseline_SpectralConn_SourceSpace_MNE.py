...
#mne.epochs.equalize_epoch_counts([epochs_concrete,epochs_abstract])
    for ccc in range(len(stcs)):
        ...
        ss=mne.read_source_estimate(data_in)
        stcs[ccc]=ss.data[:,200:400].copy()
...
#  we will later use as seed for the connectivity computation
    ...
    sct_incoh = data_path + meg + 'firstMorphed_SemLoc_icaclean_Abstract_Source_Evoked_m500_700'
...
# get 2 frequency bins
        ...
        coh, freqs1, times1, n_epochs1, n_tapers1 = spectral_connectivity(stcs, method='coh', mode='multitaper', mt_adaptive=True, indices=indices,sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=True, n_jobs=4)
...
# Note: We use a hack to save the frequency axis as time
        ...
        data_out = data_path + meg +  'firstMorphed_SemLoc_icaclean_AbstractBaseline_Coherence_ThetaAlphaBetaGamma_m300_m100_' + label_name[0:-3]
        print data_out
...
#        plv, freqs, times, n_epochs, n_tapers = spectral_connectivity(stcs, method='plv', mode='multitaper', mt_adaptive=True, indices=indices,sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=True, n_jobs=4)
#        #spectral_connectivity(data, method='coh', indices=None, sfreq=6.283185307179586, mode='multitaper', fmin=None, fmax=inf, fskip=0, faverage=False, tmin=None, tmax=None, mt_bandwidth=None, mt_adaptive=False, mt_low_bias=True, cwt_frequencies=None, cwt_n_cycles=7, block_size=1000, n_jobs=1, verbose=None)
#        
#        print('plv fin ')
#        
...
#        data_out = data_path + meg +  'firstMorphed_SemLoc_icaclean_AbstractBaseline_PLV_ThetaAlphaBetaGamma_m300_m100_' + label_name[0:-3]
#        print data_out
#        plv_stc.save(data_out)

###############
	print "spectral connectivity ppc"
        plv, freqs, times, n_epochs, n_tapers = spectral_connectivity(stcs, method='ppc', mode='multitaper', mt_adaptive=True, indices=indices,sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=True, n_jobs=4)
        #spectral_connectivity(data, method='coh', indices=None, sfreq=6.283185307179586, mode='multitaper', fmin=None, fmax=inf, fskip=0, faverage=False, tmin=None, tmax=None, mt_bandwidth=None, mt_adaptive=False, mt_low_bias=True, cwt_frequencies=None, cwt_n_cycles=7, block_size=1000, n_jobs=1, verbose=None)
        ...
        tstep1 = 1#np.mean(freqs[1]) - tmin
        plv_stc = mne.SourceEstimate(plv, vertices=stc_coh.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='fsaverage')
        data_out = data_path + meg +  'firstMorphed_SemLoc_icaclean_AbstractBaseline_PPC_ThetaAlphaBetaGamma_m300_m100_' + label_name[0:-3]
        print data_out
        plv_stc.save(data_out)
