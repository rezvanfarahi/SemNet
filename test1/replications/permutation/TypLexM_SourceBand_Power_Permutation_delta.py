...
for p_threshold in ll:
    ...
    ### Concrete
    	...
    	fname_cncrt = data_path + meg + 'mineMorphed_ico_SemLoc_ica_Concrete_rPLV_normori_ratio_equalized_m500_700_200hz_' +b#'mineMorphed_ico_SemLoc_icaclean_Concrete_rPLV_ratio_equalized_m500_700_' + b
    	stc_cncrt = mne.read_source_estimate(fname_cncrt)
    	#abslt_cncrt= np.absolute(stc_cncrt.copy().crop(.3,.4).data)
    	fname_abs = data_path + meg + 'mineMorphed_ico_SemLoc_ica_Abstract_rPLV_normori_ratio_equalized_m500_700_200hz_' +b#'mineMorphed_ico_SemLoc_icaclean_Abstract_rPLV_ratio_equalized_m500_700_' + b
    	stc_abs = mne.read_source_estimate(fname_abs)
    	"""
    	fname_sub = data_path + meg + 'Morphed_SemLoc_icaclean_Subtract_rPLV_ratio_equalized_m500_700_theta'
    	...
    	for cc in range(5):
    		...
    		X[:,cc,ii]=Xc-Xa#np.log(np.divide(Xc,Xa))#
    	#X[:,1,:]=X[:,0,:].copy()
    ...
    print('Clustering.')
    max_step=5
    T_obs, clusters, cluster_p_values, H0 = clu = spatio_temporal_cluster_1samp_test(X, connectivity=connectivity, n_jobs=4, threshold=t_threshold,n_permutations=2048,tail=0,t_power=1, step_down_p=0.05, spatial_exclude=spatial_exclude)#, max_step=5)
...
#    print cluster_p_values.min()
    ...
    good_cluster_inds = np.intersect1d(np.where(clus_p_values <= 0.01)[0], np.where(cluster_p_values<=0.1)[0])
    ...
    if clus_p_values.min()<=0.01 and cluster_p_values.min()<=0.1:
        print p_thresh
        ...
        out_file1=out_path + 'Per_clusp'+str(p_threshold)[2:]+'_p'+str(cluster_p_values.min())[2:]+'_SL_ica_Subtract_rPLV_ratio_ico_normori_eq_50_550_100ms_sx_ms' + str(max_step) +b
        ...
        out_file2=out_path + 'Per_sw_clusp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_SL_ica_Subtract_rPLV_ratio_normori_eq_50_550_100ms_sx_ms' + str(max_step) +b
