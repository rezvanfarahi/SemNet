...
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/stc/permutation/power/final2/signed/' #
...
for p_threshold in ll:
    ...
    ### word
    	...
    	for cc in range(5):
    		...
    		X[:,cc,ii]=np.subtract(Xc1,Xa1)#np.subtract(np.abs(Xc1-1),np.abs(Xa1-1))#np.log(np.divide(Xc,Xa))#np.subtract(Xc1,Xa1)#
