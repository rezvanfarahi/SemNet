...
p_list=[0.05,0.045,0.04,0.03,0.025,0.02,0.01,0.008,0.005,0.002,0.001,0.0005,0.0001]
print "subject_inds:"
print subject_inds
print "No rejection"
...
for p_threshold in ll:
    ...
    ### Concrete
    		fname_cncrt = data_path + meg + 'firstMorphed_SemLoc_icaclean_equalized_Concrete_' + conn_method + '_WholeBand' + trange + label_name[0:-3] 
    		stc_cncrt = mne.read_source_estimate(fname_cncrt)
    		fname_abs = data_path + meg + 'firstMorphed_SemLoc_icaclean_equalized_Abstract_' + conn_method + '_WholeBand' + trange + label_name[0:-3] 
    		stc_abs = mne.read_source_estimate(fname_abs)	
    		stc_data_subtract=np.subtract(stc_cncrt.data,stc_abs.data)
	...
	spatial_exclude=np.hstack((fsave_vertices[0][nnl], fsave_vertices[0][nnr]+10242))
    	...
    	if cluster_p_valuest.min()<=0.05:
    		...
    		out_file1=out_path + 'Permutation_firstMorphed_SemLoc_icaclean_Subtract_' + conn_method + '_wband' + trange + '_' + str(p_threshold)[2:5] + '_' + str(p_thresh)[2:5]+ '_' + label_name[0:-3]
