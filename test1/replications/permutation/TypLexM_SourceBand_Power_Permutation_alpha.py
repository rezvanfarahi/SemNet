...
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/stc/permutation/power/final/' #
...
for p_threshold in ll:
    ...
    ### word
    	...
    	for cc in range(5):
    		...
    		X[:,cc,ii]=np.subtract(np.abs(Xc1-1),np.abs(Xa1-1))#np.log(np.divide(Xc,Xa))#np.subtract(Xc1,Xa1)#
...
#    print cluster_p_values.min()
    ...
    if cluster_p_values.min()<=0.1:#clus_p_values.min()<=0.01 and cluster_p_values.min()<=0.1:
        ...
        out_file1=out_path + 'Per_clusp'+str(p_threshold)[2:]+'_p'+str(cluster_p_values.min())[2:]+'_SL_ica_Subtract_Power_ratio_ico_normori_eq_50_550_100ms_sx_ms' + str(max_step) +b
        ...
        out_file2=out_path + 'Per_sw_clusp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_SL_ica_Subtract_Power_ratio_normori_eq_50_550_100ms_sx_ms' + str(max_step) +b
