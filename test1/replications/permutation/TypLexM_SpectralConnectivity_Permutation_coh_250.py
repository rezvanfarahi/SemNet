...
p_list=[0.05,0.025,0.01]#,0.01,0.005,0.001,0.0005,0.0001]
print "subject_inds:"
print subject_inds
print "No rejection"
...
labellist = ['leftATL-lh', 'rightATL-rh']#'atlleft-lh']#, 'atlright-rh']#,'medialtempright-rh','medialtempleft-lh']
trange='_250_450_'
conn_method='Coherence'
for p_threshold in ll:
    ...
    ### Concrete
    		fname_cncrt = data_path + meg + 'firstMorphed_SemLoc_icaclean_Concrete_' + conn_method + '_ThetaAlphaBetaGamma' + trange + label_name[0:-3] 
    		stc_cncrt = mne.read_source_estimate(fname_cncrt)
    		fname_abs = data_path + meg + 'firstMorphed_SemLoc_icaclean_ConcreteBaseline_' + conn_method + '_ThetaAlphaBetaGamma_m300_m100_' + label_name[0:-3] 
	...
	spatial_exclude=np.hstack((fsave_vertices[0][nnl], fsave_vertices[0][nnr]+10242))
    	...
    	T_obst, clusterst, cluster_p_valuest, H0t = clut = spatio_temporal_cluster_1samp_test(X_theta, connectivity=connectivity,  n_permutations=n_permutations, n_jobs=4, threshold=t_threshold, tail=0,spatial_exclude=spatial_exclude)#t_power=1, step_down_p=0.05)
    	...
    	T_obsa, clustersa, cluster_p_valuesa, H0a = clua = spatio_temporal_cluster_1samp_test(X_alpha, connectivity=connectivity,  n_permutations=n_permutations, n_jobs=4, threshold=t_threshold, tail=0,spatial_exclude=spatial_exclude)#t_power=1, step_down_p=0.05)
    	#    Now select the clusters that are sig. at p < 0.05 (note that this value
    	#    is multiple-comparisons corrected).
    	...
    	T_obsb, clustersb, cluster_p_valuesb, H0b = club = spatio_temporal_cluster_1samp_test(X_beta, connectivity=connectivity,  n_permutations=n_permutations, n_jobs=4, threshold=t_threshold, tail=0,spatial_exclude=spatial_exclude)#t_power=1, step_down_p=0.05)
    	#    is multiple-comparisons corrected).
    	good_cluster_indsb = np.where(cluster_p_valuesb < 0.05)[0]
    	print cluster_p_valuesb.min()
    	
    	print('Clustering gamma')
    	T_obsg, clustersg, cluster_p_valuesg, H0g = clug = spatio_temporal_cluster_1samp_test(X_gamma, connectivity=connectivity,  n_permutations=n_permutations, n_jobs=4, threshold=t_threshold, tail=0,spatial_exclude=spatial_exclude)#t_power=1, step_down_p=0.05)
    	#    Now select the clusters that are sig. at p < 0.05 (note that this value
    	#    is multiple-comparisons corrected).
    	...
    	if cluster_p_valuest.min()<=0.05:
    		...
    		out_file1=out_path + 'Permutation_firstMorphed_SemLoc_icaclean_ConcreteBaseline_' + conn_method + '_Theta' + trange + '_' + str(p_threshold)[2:5] + '_' + str(p_thresh)[2:5]+ '_' + label_name[0:-3]
    	...
    	if cluster_p_valuesa.min()<=0.05:
    		...
    		out_file1=out_path + 'Permutation_firstMorphed_SemLoc_icaclean_ConcreteBaseline_' + conn_method + '_Alpha' + trange + '_' + str(p_threshold)[2:5]+ '_' + str(p_thresh)[2:5] + '_' + label_name[0:-3]
    	...
    	if cluster_p_valuesb.min()<=0.05:
    		...
    		out_file1=out_path + 'Permutation_firstMorphed_SemLoc_icaclean_ConcreteBaseline_' + conn_method + '_Beta' + trange + '_' + str(p_threshold)[2:5]+ '_' + str(p_thresh)[2:5] + '_' + label_name[0:-3]
    	...
    	if cluster_p_valuesg.min()<=0.05:
    		...
    		out_file1=out_path + 'Permutation_firstMorphed_SemLoc_icaclean_ConcreteBaseline_' + conn_method + '_Gamma' + trange + '_' + str(p_threshold)[2:5]+ '_' + str(p_thresh)[2:5] + '_' + label_name[0:-3]
