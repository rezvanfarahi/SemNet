...
# Labels/ROIs
labellist = ['ATL2-lh', 'ATL-rh']#, 'medialtempright-rh','medialtempleft-lh']
...
for meg in ll:
    ...
    for label_name in labellist:
        ...
        lambda2 = 1.0 / snr ** 2
        stcsp = apply_inverse_epochs(epochs, inverse_operator, lambda2, method, pick_ori="normal", return_generator=False)
        stcs = mne.morph_data(subject_from, subject_to, stcsp, subjects_dir=data_path, n_jobs=1, grade=vertices_to)
...
# get 2 frequency bins
        ...
        coh, freqs1, times1, n_epochs1, n_tapers1 = spectral_connectivity(stcs, method='coh', mode='fourier', indices=indices,sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=True, tmin=tmin1, tmax=tmax1, n_jobs=4)
...
#
        ...
        tmin1 = 2#np.mean(freqs[0])
        tstep1 = 2#np.mean(freqs[1]) - tmin
        coh_stc = mne.SourceEstimate(coh, vertices=stc.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='avgsubj')
        data_out = data_path + meg +  'firstMorphed_SemLoc_icaclean_Abstract_Coherence_ThetaAlphaBetaGamma_250_450_' + label_name[0:-3]
        print data_out
        coh_stc.save(data_out)
        #############

        print "spectral connectivity plv"
        plv, freqs, times, n_epochs, n_tapers = spectral_connectivity(stcs, method='ppc', mode='fourier', indices=indices,sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=True, tmin=tmin, tmax=tmax, n_jobs=4)
        #spectral_connectivity(data, method='coh', indices=None, sfreq=6.283185307179586, mode='multitaper', fmin=None, fmax=inf, fskip=0, faverage=False, tmin=None, tmax=None, mt_bandwidth=None, mt_adaptive=False, mt_low_bias=True, cwt_frequencies=None, cwt_n_cycles=7, block_size=1000, n_jobs=1, verbose=None)
        ...
        tmin1 = 2#np.mean(freqs[0])
        tstep1 = 2#np.mean(freqs[1]) - tmin
        plv_stc = mne.SourceEstimate(plv, vertices=stc.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='avgsubj')
        data_out = data_path + meg +  'firstMorphed_SemLoc_icaclean_Abstract_PPC_ThetaAlphaBetaGamma_250_450_' + label_name[0:-3]
        print data_out
