...
## epoching
    ...
    bands = dict(alpha=[8, 12], beta=[13, 30])#, gamma=[31, 45])
    snr = 3.0
    lambda2 = 1.0 / snr ** 2
    stcs_concrete = source_band_induced_power(epochs_concrete, inv_op, bands=bands, label=None, lambda2=lambda2, method='MNE', n_cycles=2, baseline=(-150,0), use_fft=False, n_jobs=1)

    for b, stc in stcs_concrete.iteritems():
	data_out = data_path + meg + 'Test_SemLoc_Concrete_Power_' + b
        stc.save(data_out)

    #stcs_abstract = source_band_induced_power(epochs_abstract, inv_op, bands, label=None, lambda2=lambda2, method='MNE', n_cycles=2, use_fft=False, n_jobs=1)

    for b, stc in stcs_abstract.iteritems():
        data_out = data_path + meg + 'SemLoc_Abstract_Power_' + b
        stc.save(data_out)

    for b, stc in stcs_abstract.iteritems():
	data_out = data_path + meg + 'SemLoc_Subtract_Power_' + b
        stc1=np.subtract(stcs_concrete.copy()[b], stcs_abstract.copy()[b])
        stc1.save(data_out)
