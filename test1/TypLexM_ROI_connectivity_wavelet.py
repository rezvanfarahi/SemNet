...
#mne.epochs.equalize_epoch_counts([epochs_concrete,epochs_abstract])
    for ccc in range(len(stcs_concrete)):
        ...
        stcs_concrete[ccc]=mne.read_source_estimate(data_in)
...
#mne.epochs.equalize_epoch_counts([epochs_concrete,epochs_abstract])
    for ccc in range(len(stcs_abstract)):
        ...
        stcs_abstract[ccc]=mne.read_source_estimate(data_in)
...
#	lambda2 = 1.0 / snr ** 2
    ...
    path_c=data_path+ meg+'label_ts_concrete.mat'
    	#ltc=scipy.io.loadmat(path_c,mat_dtype=True); label_ts_concrete=ltc['label_ts_concrete']
    ...
    scipy.io.savemat(path_a,{'label_ts_abstract':label_ts_abstract})
    	#lta=scipy.io.loadmat(path_a,mat_dtype=True); label_ts_abstract=lta['label_ts_abstract']
    print "lbl ts"
    ...
    con_cncrt, freqs_cncrt, times_cncrt, nnn, ppp= spectral_connectivity(label_ts_concrete, method=con_methods, mode='cwt_morlet', cwt_n_cycles=cwt_n_cycles, cwt_frequencies=cwt_frequencies, sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=True, n_jobs=4)
    #con_cncrt=np.divide(con_cncrt,con_blcn)
    path_c=data_path+ meg+'pca_ppc_concrete_cwtmorlet'
    np.save(path_c,con_cncrt)
    scipy.io.savemat(path_c,{'con_cncrt':con_cncrt})
    print "con finished"
    con_abs, freqs_abs, times_abs, nnn, ppp = spectral_connectivity(label_ts_abstract,method=con_methods, mode='cwt_morlet', cwt_n_cycles=cwt_n_cycles, cwt_frequencies=cwt_frequencies, sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=True,  n_jobs=4)
    path_a=data_path+ meg+'pca_ppc_abstract_cwtmorlet'
    np.save(path_a,con_abs)
    scipy.io.savemat(path_a,{'con_abs':con_abs})
    print "abs finished"
    con_sub=con_cncrt-con_abs
    path_s=data_path+ meg+'pca_ppc_subtract_cwtmorlet'
