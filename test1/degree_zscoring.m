...
% end
% end
deg_brtzr=zeros(size(deg_brt,1),size(deg_brt,2),1,size(deg_brt,4));
deg_brtr=zeros(size(deg_brt,1),size(deg_brt,2),1,size(deg_brt,4));

deg_brrzp=zeros(size(deg_brtz,1),size(deg_brtz,2),2);
for ii=1
    deg_brtr(:,:,ii,:)=mean(deg_brt(:,:,10:90,:),3);
end
...
out_path='/imaging/rf02/TypLexMEG/icaanalysis_results/jpg/time_resolved/ROI_connectivity/graph/ppc_bothcon_hubs_prob_1win.mat';
