...
### Concrete
	for kk,b in enumerate(bands):
		print kk
		fname_cncrt = data_path + meg + 'Morphed_SemLoc_Concrete_Power_m500_700' + b
		
		stc_cncrt = mne.read_source_estimate(fname_cncrt)

		stc_cncrt1=stc_cncrt.copy().crop(0.,0.1)
		X[:,0+5*kk,ii]=stc_cncrt1.data.mean(axis=1)
	
		stc_cncrt1=stc_cncrt.copy().crop(0.1,0.2)
		X[:,1+5*kk,ii]=stc_cncrt1.data.mean(axis=1)

		stc_cncrt2=stc_cncrt.copy().crop(0.2,0.3)
		X[:,2+5*kk,ii]=stc_cncrt2.data.mean(axis=1)
	
		stc_cncrt3=stc_cncrt.copy().crop(0.3,0.4)
		X[:,3+5*kk,ii]=stc_cncrt3.data.mean(axis=1)

		stc_cncrt4=stc_cncrt.copy().crop(0.4,0.5)
		X[:,4+5*kk,ii]=stc_cncrt4.data.mean(axis=1)

		stc_cncrt5=stc_cncrt.copy().crop(0.5,0.6)
		X[:,5+5*kk,ii]=stc_cncrt5.data.mean(axis=1)
...
print "hi"
for b in bands:
	out_file1=data_path + 'Permutation_ttest_pmap_SemLoc_Concrete_SourcePower_Logratio_m500_700_' + b 
	p_stc.save(out_file1)

	out_file2=data_path + 'Permutation_ttest_tmap_SemLoc_Concrete_SourcePower_Logratio_m500_700_' + b
