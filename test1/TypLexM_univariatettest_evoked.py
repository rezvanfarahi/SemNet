...
out_path='/imaging/rf02/TypLexMEG/icaanalysis_results/stc/UVttest/evoked/'
...
### Concrete
	...
	stc_subtract1=stc_subtract.crop(0.051,0.15).copy()
	stc_subtract1=stc_subtract1.mean()
	bb=stc_subtract1.data
	X0[:,ii]=bb.squeeze()

	stc_subtract=np.subtract(stc_cncrt,stc_abs)
	stc_subtract1=stc_subtract.crop(0.151,0.25).copy()
	stc_subtract1=stc_subtract1.mean()
	bb=stc_subtract1.data
	X1[:,ii]=bb.squeeze()
	
	stc_subtract=np.subtract(stc_cncrt,stc_abs)
	stc_subtract1=stc_subtract.crop(0.251,0.35).copy()
	stc_subtract1=stc_subtract1.mean()
	bb=stc_subtract1.data
	X2[:,ii]=bb.squeeze()

	stc_subtract=np.subtract(stc_cncrt,stc_abs)
	stc_subtract1=stc_subtract.crop(0.351,0.45).copy()
	stc_subtract1=stc_subtract1.mean()
	bb=stc_subtract1.data
	X3[:,ii]=bb.squeeze()

	stc_subtract=np.subtract(stc_cncrt,stc_abs)
	stc_subtract1=stc_subtract.crop(0.451,0.55).copy()
	stc_subtract1=stc_subtract1.mean()
	bb=stc_subtract1.data
	X4[:,ii]=bb.squeeze()

	stc_subtract=np.subtract(stc_cncrt,stc_abs)
	stc_subtract1=stc_subtract.crop(0.551,0.65).copy()
	stc_subtract1=stc_subtract1.mean()
	bb=stc_subtract1.data
	X5[:,ii]=bb.squeeze()
...
t_stc = mne.SourceEstimate(t_map, vertices=fsave_vertices, tmin=1e-3, tstep=1e-3, subject='avgsubj', verbose=None)
print "se1 done"
...
stc_all_cluster_vis = summarize_clusters_stc(clu, tstep=tstep,
                                     ...
                                     subject='average')
"""
out_file1=data_path + 'UVttest_tmap_SemLoc_icaclean_ConcreteAbstract_Source_Evoked_m500_700' 
