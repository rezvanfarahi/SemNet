...
# get 2 frequency bins
        ...
        coh, freqs1, times1, n_epochs1, n_tapers1 = spectral_connectivity(stcs, method='coh', mode='fourier', indices=indices,sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=True, tmin=tminw, tmax=tmaxw, n_jobs=4)
...
# Note: We use a hack to save the frequency axis as time
        ...
        coh_stc = mne.SourceEstimate(coh, vertices=stc.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='fsaverage')
...
#############
        ...
        plv, freqs, times, n_epochs, n_tapers = spectral_connectivity(stcs, method='ppc', mode='fourier', indices=indices,sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=True, tmin=tminw, tmax=tmaxw, n_jobs=4)
        #spectral_connectivity(data, method='coh', indices=None, sfreq=6.283185307179586, mode='multitaper', fmin=None, fmax=inf, fskip=0, faverage=False, tmin=None, tmax=None, mt_bandwidth=None, mt_adaptive=False, mt_low_bias=True, cwt_frequencies=None, cwt_n_cycles=7, block_size=1000, n_jobs=1, verbose=None)
        ...
        tmin1 = 1#np.mean(freqs[0])
        tstep1 = 1#np.mean(freqs[1]) - tmin
        plv_stc = mne.SourceEstimate(plv, vertices=stc.vertno, tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='fsaverage')
