...
for cnt1, meg in enumerate(ll):
	...
	cc_TypWords=cc_TypWords.reshape(cc_TypWords.size/3,3).mean(axis=0)
	cc_AtypWords=con_AtypWords[con_AtypWords!=0]
	cc_AtypWords=cc_AtypWords.reshape(cc_AtypWords.size/3,3).mean(axis=0)

	cc_TypPWords=con_TypPWords[con_TypPWords!=0]
	cc_TypPWords=cc_TypPWords.reshape(cc_TypPWords.size/3,3).mean(axis=0)
	cc_AtypPWords=con_AtypPWords[con_AtypPWords!=0]
	cc_AtypPWords=cc_AtypPWords.reshape(cc_AtypPWords.size/3,3).mean(axis=0)
