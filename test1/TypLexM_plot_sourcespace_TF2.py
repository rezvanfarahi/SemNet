...
# subtract the evoked response in order to exclude evoked activity
	...
	# Compute a source estimate per frequency band
	    bands = dict(alpha=[8, 12], beta=[13, 30], gamma=[30, 45])
	    snr = 3.0
	    lambda2 = 1.0 / snr ** 2
    	    stcs = source_band_induced_power(this_epochs, inv_op, bands, label=None, lambda2=lambda2, method='MNE', n_cycles=1, use_fft=False, n_jobs=1)

	    for b, stc in stcs.iteritems():
	       stc.save('induced_power_%s_%d' % (b, ii))
