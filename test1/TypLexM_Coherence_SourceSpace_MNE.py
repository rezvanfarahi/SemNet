...
from mne import (fiff, read_evokeds, equalize_channels)
...
for meg in list_all:
	...
	fname_label_lh = label_path + subjects[0] + '/' + label_name_lh + '.label'
	# Load data
	...
	coh_stc = mne.SourceEstimate(coh, vertices=stc.vertno, tmin=1e-3 * tmin,
		                     tstep=1e-3 * tstep, subject=subjects[0])
#data_out = data_path + meg + 'coherence_map_typlex'
#coh_stc.save(data_path + meg + data_out)
