...
for ii, meg in enumerate(ll):
	...
	b2=0.1
	for cc in range(7):
		b1=b2-0.05+0.001; b2=b1+0.1-0.001
...
#connectivity = spatial_tris_connectivity(to_con, remap_vertices=True)   
connectivity = spatial_tris_connectivity(grade_to_tris(5))#, n_times=7)   
n_permutations=1024
...
p_threshold = 0.05
t_threshold = dict(start=cstart, step=cstep)#-stats.distributions.t.ppf(p_threshold /2., n_subjects - 1)#dict(start=0, step=1)#
#t_threshold=2
print('Clustering.')
T_obs, clusters, cluster_p_values, H0 = clu = spatio_temporal_cluster_1samp_test(X, connectivity=connectivity, n_jobs=2, spatial_exclude=spatial_exclude, threshold=t_threshold, tail=0)#, max_step=0)#spatial_exclude=spatial_exclude, 
#    Now select the clusters that are sig. at p < 0.05 (note that this value
#    is multiple-comparisons corrected).
good_cluster_inds = np.where(cluster_p_values <0.2)[0]
...
stc_all_cluster_vis = summarize_clusters_stc(clu, tstep=0.05, vertno=fsave_vertices, subject='fsaverage', p_thresh=0.2)

out_file1=out_path + 'TFCEPermutation_firstmorphed_clusterp05_p05_17subj_SemLoc_icaclean_ConcreteAbstract_50_550'
stc_all_cluster_vis.save(out_file1)

Matx=np.zeros((20484,7))
...
out_file2=out_path + 'TFCEPermutation_stepwise_firstmorphed_clusterp05_p05_17subj_SemLoc_icaclean_ConcreteAbstract_50_550'
