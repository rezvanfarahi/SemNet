# -*- coding: utf-8 -*-
"""
Created on Mon Dec 14 13:42:19 2015
...
# Note: We use a hack to save the frequency axis as time
        ...
        data_out = data_path + meg +  'Morphed_ico_SemLoc_ica_equalized_Abstract_Mlttap_bestV_ctf_Coherence_Theta_150_450_200ms_' + label1.name[:-3]
        print 'theta'
        ...
        data_out = data_path + meg +  'Morphed_ico_SemLoc_ica_equalized_Abstract_Mlttap_bestV_ctf_Coherence_Alpha_150_450_200ms_' + label1.name[:-3]
        print 'alpha'
        coh_stc.save(data_out)
        
        coh_stc = mne.SourceEstimate(coh[:,2,:], vertices=stc_coh.vertices, tmin= tmin1, tstep= tstep1, subject='fsaverage')
        data_out = data_path + meg +  'Morphed_ico_SemLoc_ica_equalized_Abstract_Mlttap_bestV_ctf_Coherence_Beta_150_450_200ms_' + label1.name[:-3]
        print 'beta'
        coh_stc.save(data_out)
        
        coh_stc = mne.SourceEstimate(coh[:,3,:], vertices=stc_coh.vertices, tmin= tmin1, tstep= tstep1, subject='fsaverage')
        data_out = data_path + meg +  'Morphed_ico_SemLoc_ica_equalized_Abstract_Mlttap_bestV_ctf_Coherence_Gamma_150_450_200ms_' + label1.name[:-3]
        print 'gamma'
        coh_stc.save(data_out)
...
# Note: We use a hack to save the frequency axis as time
        ...
        data_out = data_path + meg +  'Morphed_ico_SemLoc_ica_equalized_Abstract_Mlttap_bestV_ctf_WPLI2d_Theta_150_450_200ms_' + label1.name[:-3]
        print 'theta'
        coh_stc.save(data_out)
        
        coh_stc = mne.SourceEstimate(dw_pli[:,1,:], vertices=stc_coh.vertices, tmin= tmin1, tstep= tstep1, subject='fsaverage')
        data_out = data_path + meg +  'Morphed_ico_SemLoc_ica_equalized_Abstract_Mlttap_bestV_ctf_WPLI2d_Alpha_150_450_200ms_' + label1.name[:-3]
        print 'alpha'
        coh_stc.save(data_out)
        
        coh_stc = mne.SourceEstimate(dw_pli[:,2,:], vertices=stc_coh.vertices, tmin= tmin1, tstep= tstep1, subject='fsaverage')
        data_out = data_path + meg +  'Morphed_ico_SemLoc_ica_equalized_Abstract_Mlttap_bestV_ctf_WPLI2d_Beta_150_450_200ms_' + label1.name[:-3]
        print 'beta'
        coh_stc.save(data_out)
        
        coh_stc = mne.SourceEstimate(dw_pli[:,3,:], vertices=stc_coh.vertices, tmin= tmin1, tstep= tstep1, subject='fsaverage')
        data_out = data_path + meg +  'Morphed_ico_SemLoc_ica_equalized_Abstract_Mlttap_bestV_ctf_WPLI2d_Gamma_150_450_200ms_' + label1.name[:-3]
        print 'gamma'
        coh_stc.save(data_out)
