...
## epoching
    ...
    epochs = mne.Epochs(raw, events, event_ids, tmin, tmax, picks=picks,
                    baseline=(None, 0), reject=reject, proj=True, preload=True)
    ...
    bands = dict(beta=[13,30])#,beta=[13, 30], gamma=[31, 45])#, beta=[13, 30], gamma=[31, 45])
    snr = 3.0
    lambda2 = 1.0 / snr ** 2
    print "source_band_induced_power concrete"
    stcs_concrete = source_band_induced_power(epochs_concrete, inv_op, bands=bands, label=None, lambda2=lambda2, method='MNE', n_cycles=5, baseline=(-400,0), baseline_mode='ratio', use_fft=False, n_jobs=1)
	...
	data_out = data_path + meg + 'Morphed_SemLoc_icaclean_Concrete_Power_ratio_m500_700_' + b
        stc_to.save(data_out)

    print "source_band_induced_power abstract"
    stcs_abstract = source_band_induced_power(epochs_abstract, inv_op, bands=bands, label=None, lambda2=lambda2, method='MNE', n_cycles=5, baseline=(-400,0), baseline_mode='ratio',use_fft=False, n_jobs=1)
    
    for b, stc in stcs_abstract.iteritems():
        stc_to = mne.morph_data(subject_from, subject_to, stc, subjects_dir=data_path, n_jobs=1, grade=vertices_to)
        data_out = data_path + meg + 'Morphed_SemLoc_icaclean_Abstract_Power_ratio_m500_700_' + b
        stc_to.save(data_out)
