...
for ii, meg in enumerate(ll):
    ...
    #evoked1.resample(50)
    condition1 = apply_inverse_epochs(epochs1, inverse_operator, lambda2, method)
    print "cond1 fin"
    evoked2 = epochs2.average()
    #evoked2.resample(50)
    condition2 = apply_inverse_epochs(epochs2, inverse_operator, lambda2, method)
    subject_from = subjects[subject_no]
    subject_to = 'fsaverage'
    print "cond2 fin"
    vertices_to = [np.arange(10242), np.arange(10242)]
    morphmat1=mne.compute_morph_matrix(subject_from, subject_to, condition1[0].vertno, vertices_to, subjects_dir=data_path)
    morphmat2=mne.compute_morph_matrix(subject_from, subject_to, condition2[0].vertno, vertices_to, subjects_dir=data_path)
    for ccc, con1 in enumerate(condition1):
        print ccc		
        stcf1=con1.morph_precomputed(subject_to,vertices_to,morphmat1, subject_from)
        data_out1 = data_path + meg + 'Morphed_SemLoc_epochs' + str(ccc) + '_icaclean_Concrete_Source_Evoked_m500_700'
    ...
    for ccc, con2 in enumerate(condition2):
        ...
        data_out2 = data_path + meg + 'Morphed_SemLoc_epochs' + str(ccc) + '_icaclean_Abstract_Source_Evoked_m500_700'
