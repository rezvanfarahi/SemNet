...
for meg in ll:
	...
	in_fif_fname = data_path + meg + 'semloc_ssstf_fft49_raw.fif'
	...
	print raw.info['projs']
	proj_raw= mne.compute_proj_raw(raw, start=0, stop=None, duration=1, n_grad=2, n_mag=2, n_eeg=2, reject=dict(eeg=120e-6, eog=150e-6, grad=200e-12, mag=4e-12), flat=None, n_jobs=1,verbose=None)
	picks = mne.pick_types(raw.info, meg=False, eeg=True, stim=False, eog=False, exclude='bads')
	print raw.info['projs']
	tmin, tmax = -0.5, 0.7
	reject = dict(eeg=420e-6, eog=550e-6, grad=200e-12, mag=4e-12)
...
## epoching
    ...
    # get epochs from raw
	epochs = mne.Epochs(raw.copy().apply_proj(), events, event_ids, tmin, tmax, picks=picks, baseline=(None, 0), reject=reject, preload=True, detrend=1, add_eeg_ref=True)
	epochs1 = mne.Epochs(raw, events, event_ids, tmin, tmax, picks=picks, baseline=(None, 0), reject=reject, preload=True, detrend=1, add_eeg_ref=True)
