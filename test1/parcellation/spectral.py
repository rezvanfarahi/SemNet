...
#    def compute_Sxy(self, con_idx, n_epochs, psd_xx, psd_yy):
#        """Compute final con. score for some connections"""
#        if self.Sxy_scores is None:
#            self.Sxy_scores = np.zeros(self.csd_shape)
#        csd_mean = self._acc[con_idx] / n_epochs
#        self.Sxy_scores[con_idx] = np.abs(csd_mean)
...
#            self.Sxy_scores = np.zeros(self.csd_shape)
#        csd_mean = self._acc[con_idx] / n_epochs
#        self.Sxy_scores[con_idx] = np.imag(csd_mean)
...
#                method.compute_Sxy(con_idx, n_epochs, psd_xx, psd_yy)

        # get the connectivity scores
        this_con = method.con_scores
#        this_Sxy = method.Sxy_scores
...
#        Sxy=all_Sxy[0]
    ...
    return con, psd, freqs, times, n_epochs, n_tapers
