...
for simcnt in ll:
    for nseeds in np.array([3,5,10,15,20]): 
        print nseeds
        thissnr=1
        out_path='/imaging/rf02/parcellation/extended_simulations/sims'+str(n_sims)+'_offset'+str(sim_offset)+'_seeds'+str(nseeds)+'_conns'+str(connperc)+'_snr'+str(thissnr)+'_ext'+str(ext)
    ...
    #                        labov_mat[lscnt,lpcnt]=0                  
        ...
        comseed=[1,2]
        comc=-1
        while len(comseed)>0 and comc<10000:
            comc=comc+1
            print comc        
            comseed=list()        
    ...
    #            nullstc_sim=range(n_epochs)
            ...
            #        cphase_mat=np.tile(const_phase[:,np.newaxis],(1,datamat.shape[1]))            
            ...
            #        datamat=np.concatenate((LO[:,np.newaxis],RMF[:,np.newaxis],MTG[:,np.newaxis]),axis=1).transpose()
                    ...
                    nullthislabel_data=np.zeros((len(lvert)+len(rvert),nulldatamat.shape[1]))
                ...
                #    MTGdat=thislabel_stc.in_label(morphed_labels[2])
                    ...
                    nullthislabel_data[tobe_noise,125:]=nullnoise_mat
                    ...
                    nullthislabel_datan=nullthislabel_data+nullrmat
    ...
    #            for ccc, con2 in enumerate(stc_sim):
    #                print ccc		
    #                simstc_morphed[ccc]=con2.morph_precomputed(subject_to='fsaverage',vertices_to=vertices_avg,morph_mat=morphmat1, subject_from=subjects[subject_no])
    #            simstc_morphed_realideal=range(len(stc_sim))
    #            for ccc, con3 in enumerate(stc_sim_realideal):
    #                print ccc		
    #                simstc_morphed_realideal[ccc]=con3.morph_precomputed(subject_to='fsaverage',vertices_to=vertices_avg,morph_mat=morphmat1, subject_from=subjects[subject_no])
    #            nullsimstc_morphed=range(len(nullstc_sim))
    #            for ccc, con4 in enumerate(nullstc_sim):
    #                print ccc		
    #                nullsimstc_morphed[ccc]=con4.morph_precomputed(subject_to='fsaverage',vertices_to=vertices_avg,morph_mat=morphmat1, subject_from=subjects[subject_no])
    #        
    ...
    #            nulllabel_ts = mne.extract_label_time_course(nullsimstc_morphed, labels, src_avg, mode='mean_flip',return_generator=False)
    #            label_ts_ideal = mne.extract_label_time_course(simstc_morphed_ideal, labels, src_avg, mode='mean_flip',return_generator=False)
    #            label_ts_realideal = mne.extract_label_time_course(simstc_morphed_realideal, labels, src_avg, mode='mean_flip',return_generator=False)
    #            erc=-1
    #            print "hi"
    #            for lblcnt,labelcheck in enumerate(morphed_labelsall):
    #                try:
    #                    lblcs=stc_sim[0].in_label(labelcheck)
    #                except:
    #                    print labelcheck
    #                    labellist_mc=copy.deepcopy(labels)
    #                    morphed_labelsall[lblcnt]=labellist_mc[lblcnt].morph(subject_from='fsaverage', smooth=5, subject_to=subjects[subject_no], subjects_dir=data_path)
                ...
                nulllabel_ts, method=con_methods, mode='multitaper', sfreq=sfreq, fmin=fmin,fmax=fmax, 
                faverage=True, mt_adaptive=True, n_jobs=1)                
                ...
                nulllabel_ts, method=con_methods2, mode='multitaper', sfreq=sfreq, fmin=fmin,fmax=fmax, 
                faverage=True, mt_adaptive=True, n_jobs=1)                
                ...
                allnoise_label_ts, method=con_methods2, mode='multitaper', sfreq=sfreq, fmin=fmin,fmax=fmax, 
                faverage=True, mt_adaptive=True, n_jobs=1)                
                ...
                if which_parc==0:
                    ...
                    parc0_connmat[:,:,4,cnt]=allnoise_con_res.copy()
                    parc0_connmat[:,:,5,cnt]=con_res2.copy()
                    parc0_connmat[:,:,6,cnt]=con_res_ideal2.copy()
                    parc0_connmat[:,:,7,cnt]=con_res_realideal2.copy()
                    parc0_connmat[:,:,8,cnt]=nullcon_res2.copy()
                    parc0_connmat[:,:,9,cnt]=allnoise_con_res2.copy()
                if which_parc==1:
                    ...
                    parc1_connmat[:,:,4,cnt]=allnoise_con_res.copy()
                    parc1_connmat[:,:,5,cnt]=con_res2.copy()
                    parc1_connmat[:,:,6,cnt]=con_res_ideal2.copy()
                    parc1_connmat[:,:,7,cnt]=con_res_realideal2.copy()
                    parc1_connmat[:,:,8,cnt]=nullcon_res2.copy()
                    parc1_connmat[:,:,9,cnt]=allnoise_con_res2.copy()
                if which_parc==2:
                    ...
                    parc2_connmat[:,:,4,cnt]=allnoise_con_res.copy()
                    parc2_connmat[:,:,5,cnt]=con_res2.copy()
                    parc2_connmat[:,:,6,cnt]=con_res_ideal2.copy()
                    parc2_connmat[:,:,7,cnt]=con_res_realideal2.copy()
                    parc2_connmat[:,:,8,cnt]=nullcon_res2.copy()
                    parc2_connmat[:,:,9,cnt]=allnoise_con_res2.copy()
                if which_parc==3:
                    ...
                    parc3_connmat[:,:,4,cnt]=allnoise_con_res.copy()
                    parc3_connmat[:,:,5,cnt]=con_res2.copy()
                    parc3_connmat[:,:,6,cnt]=con_res_ideal2.copy()
                    parc3_connmat[:,:,7,cnt]=con_res_realideal2.copy()
                    parc3_connmat[:,:,8,cnt]=nullcon_res2.copy()
                    parc3_connmat[:,:,9,cnt]=allnoise_con_res2.copy()
                if which_parc==4:
                    ...
                    parc4_connmat[:,:,4,cnt]=allnoise_con_res.copy()
                    parc4_connmat[:,:,5,cnt]=con_res2.copy()
                    parc4_connmat[:,:,6,cnt]=con_res_ideal2.copy()
                    parc4_connmat[:,:,7,cnt]=con_res_realideal2.copy()
                    parc4_connmat[:,:,8,cnt]=nullcon_res2.copy()
                    parc4_connmat[:,:,9,cnt]=allnoise_con_res2.copy()
