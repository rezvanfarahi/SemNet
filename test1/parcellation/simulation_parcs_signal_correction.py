...
def estimate_actual_signal(labels, Rmat, labels_ts ):
    ...
    for ecnt in range(len(labels_ts)):
        ...
        for cnt2 in range(len(labels)): #destination labels
            ...
            thislabw=np.where(this_patch==cnt2)[0]
            coef_mat=Rmat[this_patch,:][:,this_patch].transpose()
#            sig_vect=labels_ts[ecnt][this_patch,:]
#            est_sig=np.linalg.lstsq(coef_mat,sig_vect)
#            labels_ts_modified[ecnt][cnt2,:]=est_sig[0][thislabw,:]
            for tcnt in range(labels_ts[ecnt].shape[1]):
                sig_vect=labels_ts[ecnt][this_patch,tcnt]
                est_sig=np.linalg.solve(coef_mat,sig_vect)
                labels_ts_modified[ecnt][cnt2,tcnt]=est_sig[thislabw]
    return labels_ts_modified  

def estimate_actual_signal_altogether(labels, Rmat, labels_ts ):
   
    labels_ts_modified=copy.deepcopy(labels_ts) 
    
    for ecnt in range(len(labels_ts)):
        print ecnt
        coef_mat=Rmat.transpose()
        for tcnt in range(labels_ts[ecnt].shape[1]):
            sig_vect=labels_ts[ecnt][:,tcnt]
            est_sig=np.linalg.lstsq(coef_mat,sig_vect)
            labels_ts_modified[ecnt][:,tcnt]=est_sig[0]
    return labels_ts_modified                
...
for lbl in labellist:
    lbl.values.fill(1.0)
#
for which_parc in range(1): #names below
    which_parc=which_parc+1
    parc_name=['aparc',
    ...
    #    evoked_fwd = mne.EvokedArray(evoked_data, info=info, tmin=-0.769)
        ...
        label_ts3 = mne.extract_label_time_course(simstc_morphed, labels, src_avg, mode='pca_flip',return_generator=False)
        ...
        for cntlts in range(n_epochs):
            for cntcl in range(len(labels)):        
                ...
                label_ts1[cntlts][cntcl,:]=thislabelsign*np.expand_dims(cl[cntcl][clw[cntcl],:].transpose(),0)
                ##ideal
    ...
    #                con_res_ideal[ii1,ii2]=0.
        ...
        label_ts=estimate_actual_signal_altogether(labels=labels, Rmat=ctf_to_ypos, labels_ts=label_ts1 )
        ...
        con_notmod, psds, freqs, times, n_epochs, n_tapers = spectral_connectivity(label_ts1, method=con_methods, mode='multitaper', sfreq=sfreq, fmin=fmin,fmax=fmax, faverage=False, mt_adaptive=True, n_jobs=1)
    ...
    #     for each method
        con_res = np.mean(con_notmod,axis=2)#dict()
        con_modified=np.mean(con,axis=2)
        print "computing second coh"
    #    label_ts_ideal=estimate_actual_signal(labels=labels, Rmat=ctf_to_ypos, labels_ts=label_ts_ideal1)
    ...
    #    con2_modified=estimate_actual_R(labels=labels, Rmat=ctf_to_ypos, con_mat=con2_modified1, psd_mat=psds )
        con2_notmod, psds2, freqs, times, n_epochs, n_tapers = spectral_connectivity(
            label_ts1, method=con_methods2, mode='multitaper', sfreq=sfreq, fmin=fmin,
            fmax=fmax, faverage=False, mt_adaptive=True, n_jobs=1)
        
    #     con is a 3D array, get the connectivity for the first (and only) freq. band
    #     for each method
        con_res2 = np.mean(con2_notmod,axis=2)#con2[:,:,0]#dict()
        con2_modified = np.mean(con2,axis=2)
    ...
    path_to=label_path+outname[which_parc]+'_coh_sigcorrection.mat'#'DKA68_orig_modified_coh.mat'#lh.fsaverage_finalpostparc_smooth_split_merge.annot
    scipy.io.savemat(path_to,{'con_res_final':con_res_final})
    path_to=label_path+outname[which_parc]+'_coh_mod_sigcorrection.mat'#'DKA68_orig_modified_coh.mat'#lh.fsaverage_finalpostparc_smooth_split_merge.annot
    scipy.io.savemat(path_to,{'con_mod_final':con_mod_final})
    
    path_to=label_path+outname[which_parc]+'_ideal_coh_sigcorrection.mat'#'DKA68_ideal_modified_coh.mat'#lh.fsaverage_finalpostparc_smooth_split_merge.annot
    scipy.io.savemat(path_to,{'con_res_final_ideal':con_res_final_ideal})
    
    path_to=label_path+outname[which_parc]+'_imcoh_sigcorrection.mat'#'DKA68_orig_modified_imcoh.mat'#lh.fsaverage_finalpostparc_smooth_split_merge.annot
    scipy.io.savemat(path_to,{'con_res_final2':con_res_final2})
    path_to=label_path+outname[which_parc]+'_imcoh_mod_sigcorrection.mat'#'DKA68_orig_modified_imcoh.mat'#lh.fsaverage_finalpostparc_smooth_split_merge.annot
    scipy.io.savemat(path_to,{'con2_mod_final':con2_mod_final})
    
    path_to=label_path+outname[which_parc]+'_ideal_imcoh_sigcorrection.mat'#'DKA68_ideal_modified_imcoh.mat'#lh.fsaverage_finalpostparc_smooth_split_merge.annot
    scipy.io.savemat(path_to,{'con_res_final_ideal2':con_res_final_ideal2})
#    plt.show()
aa=np.corrcoef(label_ts1[0])
plt.imshow(np.abs(aa),cmap='hot')
plt.colorbar()
plt.figure(2)
aa=np.corrcoef(label_ts[0])
plt.imshow(np.abs(aa),cmap='hot')
plt.colorbar()
plt.close("all")
...
thislabelsign=mne.label.label_sign_flip(labels[20],src_avg)
thislabel_sig=cl[20].copy()
gg=np.tile(thislabelsign,[729,1]).transpose()
thislabel_fsig=gg*thislabel_sig
data=thislabel_fsig.copy().transpose()
pca=PCA(3)
pca.fit(data)
scores=pca.transform(data)
plt.figure(3)
plt.plot(scores)
cmat=pca.components_
plt.figure(3)
plt.plot(cmat[2,:].transpose())

plt.figure(1)
plt.plot(-label_ts3[0][20,:])
plt.figure(2)
plt.plot(-label_ts1[0][20,:])
