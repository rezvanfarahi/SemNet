...
#labellist_path = [label_path+'RMF_smallseed-lh.label',label_path+'STS_smallseed-lh.label']#[ 'lh.posterior_fusiform','rh.posterior_fusiform', 'lh.lateraloccipital','rh.lateraloccipital','lh.precentral','rh.precentral', 'lh.ATL_latmed','rh.ATL_latmed']#labellist = [ 'leftATL-lh','rightATL-rh']#, 'medialtempright-rh','medialtempleft-lh']'lh.postcentral','rh.postcentral',
labellist_path = [label_path+'rostfront_smallseed-lh.label',label_path+'midtemp_smallseed-lh.label']#[ 'lh.posterior_fusiform','rh.posterior_fusiform', 'lh.lateraloccipital','rh.lateraloccipital','lh.precentral','rh.precentral', 'lh.ATL_latmed','rh.ATL_latmed']#labellist = [ 'leftATL-lh','rightATL-rh']#, 'medialtempright-rh','medialtempleft-lh']'lh.postcentral','rh.postcentral',

labellist=[mne.read_label(label) for label in labellist_path]
for lbl in labellist:
    lbl.values.fill(1.0)
import copy
labels=copy.deepcopy(labellist)
...
for cnt, meg in enumerate([ll[0],ll[0]]):
    ii=ii+1
    print cnt
    subjects_dir = data_path 
    ...
    for ecnt in range(n_epochs):
        ...
        LO=np.concatenate((np.zeros(125,),1e-9*np.sin(t)),axis=0)#+LO_phase[ecnt]
        RMF=np.concatenate((np.zeros(125,),1e-9*np.sin(t+np.pi/6.)),axis=0)#+RMF_phase[ecnt]
        this_std=np.sqrt(np.mean(RMF[125:]**2)/9.)
        datamat=np.concatenate((LO[:,np.newaxis],RMF[:,np.newaxis]),axis=1).transpose()
        tmin=-0.124
        tstep=0.001
    ...
    #    MTGdat=thislabel_stc.in_label(morphed_labels[2])
        ...
        print ecnt
        rmat=0.01*this_std*np.random.randn(thislabel_data.shape[0],thislabel_data.shape[1])
...
#    evoked_fwd = mne.EvokedArray(evoked_data, info=info, tmin=-0.769)
    ...
    stc_seed_label=simstc_morphed_ideal[0].in_label(labels[0])
    idx_seed_label=stc_seed_label.vertices[0][0]
    n_sources = simstc_morphed_ideal[0].data.shape[0]
    indices = seed_target_indices([idx_seed_label], np.arange(n_sources))
...
#        
    ...
    con, psds, freqs, times, n_epochs, n_tapers = spectral_connectivity(simstc_morphed, method=con_methods, mode='fourier', sfreq=sfreq, fmin=fmin,fmax=fmax, indices=indices,faverage=True, n_jobs=4)
    tmin = 0
    tstep = 1
    coh_stc = mne.SourceEstimate(con, vertices=simstc_morphed[0].vertices, tmin=1e-3 * tmin,tstep=1e-3 * tstep, subject='fsaverage')
    coh_path=label_path+'coherence_leakage_simulation_RMF_STS'
    coh_stc.save(coh_path)
    #     con is a 3D array, get the connectivity for the first (and only) freq. band
#     for each method
    print "computing second coh"
    con_ideal, psds_ideal, freqs, times, n_epochs, n_tapers = spectral_connectivity(
        simstc_morphed_ideal, method=con_methods, mode='fourier', sfreq=sfreq, fmin=fmin,
        fmax=fmax, faverage=True, indices=indices, n_jobs=4)
    coh_stc_ideal = mne.SourceEstimate(con_ideal, vertices=simstc_morphed[0].vertices, tmin=1e-3 * tmin,tstep=1e-3 * tstep, subject='fsaverage')
    coh_ideal_path=label_path+'coherence_noleakage_simulation_RMF_STS'
    coh_stc_ideal.save(coh_ideal_path)
    print "computing 1st imcoh"
    con_methods2 = ['imcoh']#['pli2_unbiased']
    con2, psds2, freqs, times, n_epochs, n_tapers = spectral_connectivity(
        simstc_morphed, method=con_methods2, mode='fourier', sfreq=sfreq, fmin=fmin,
        fmax=fmax, faverage=True,indices=indices, n_jobs=4)
    imcoh_stc = mne.SourceEstimate(np.abs(con2), vertices=simstc_morphed[0].vertices, tmin=1e-3 * tmin,tstep=1e-3 * tstep, subject='fsaverage')
    imcoh_path=label_path+'imcoherence_leakage_simulation_RMF_STS'
    imcoh_stc.save(imcoh_path)
    
    #     con is a 3D array, get the connectivity for the first (and only) freq. band
    #     for each method
    print "computing second imcoh"
    con_ideal2, psds_ideal2, freqs, times, n_epochs, n_tapers = spectral_connectivity(
        simstc_morphed_ideal, method=con_methods2, mode='fourier', sfreq=sfreq, fmin=fmin,
        fmax=fmax, indices=indices,faverage=True,  n_jobs=4)
        
    imcoh_stc_ideal = mne.SourceEstimate(con_ideal2, vertices=simstc_morphed[0].vertices, tmin=1e-3 * tmin,tstep=1e-3 * tstep, subject='fsaverage')
    imcoh_ideal_path=label_path+'imcoherence_noleakage_simulation_RMF_STS'
    imcoh_stc_ideal.save(imcoh_ideal_path)
