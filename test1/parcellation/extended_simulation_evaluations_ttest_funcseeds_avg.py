...
#                    aa=dir ('*parc_connmat*.mat');
                    ...
                    nnodes=74;
                    inpath='/imaging/rf02/parcellation/extended_simulations/funcseeds_avgsims70_offset0_seeds'+str(nseeds)+'_conns'+str(nconns)+'_bothsnrs_extglasser/aparc_mod/'
                    filelist0=list()
                    for(dirpath,dirnames,filenames) in walk(inpath):
                        filelist0.extend(filenames)
                    filelist=[fn for fn in filelist0 if fn[:12]=='parc_connmat'] 
...
#                ncm_vect[subcnt]=np.percentile(ncm_sim,99.999)
            ...
            connmat=np.zeros((nnodes,nnodes,9,nsubs,nsims))#;%leakageSNR3,noiseSNR3,ideal,leakageSNR1,noiseSNR1 (COH 1st, imCOH2nd)
            print filelist
#            cc=-1;
...
#            base_connmat=repmat(nullconnmat(:,:,[2,1,1,4,3,6,5,5,8,7],:),[1,1,1,1,nsims]);
            ...
            connmat_threshs=np.zeros((connmat.shape[0],connmat.shape[1],connmat.shape[2],connmat.shape[3],connmat.shape[4],99))
            for cc0,cc0val in enumerate(range(0,99,5)):  
                print cc0
                for cc1 in range(9):
                    for cc2 in range(nsubs):
                        for cc3 in range(nsims):
                            ...
                            ss=np.sort(this_connmat[this_connmat<1000])
                            cff=(99-cc0)/100.
                            tcm=ss[int(cff*len(ss))]#ncm_vect[cc2]#np.mean(this_connmat[this_connmat<1000])
#                            print tcm
...
#                            basecorr_connmat[:,:,cc1,cc2,cc3]=this_connmat.copy()
                            connmat_threshs[:,:,cc1,cc2,cc3,cc0]=this_connmat.copy()
            pval=np.zeros((nnodes,nnodes,9,nsims,20));
            print 'ttest'
#            for cc1 in range(9):
#                print cc1
#                
                ...
                for cc1 in range(nnodes):                
                    for cc2 in range(nnodes):
                        if cc1>cc2:
                            for cc3 in range(9):
#                                ntests=nnodes*(nnodes-1)/2                   
                                thisvect=np.squeeze(connmat_threshs[cc1,cc2,cc3,:,:,cc0]);
    ...
    #                            this_tval[this_tval<0]=1
                                for ttcnt,thist in enumerate(this_tval):
                                    pval[cc1,cc2,cc3,ttcnt,cc0]=scistats.t.sf(thist,nsubs-1) 
            out_name=inpath+'ttest_pval_parc_threshs_'+str(which_parc)+'.mat'
