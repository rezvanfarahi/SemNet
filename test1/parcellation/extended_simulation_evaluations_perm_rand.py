...
#            base_connmat=repmat(nullconnmat(:,:,[2,1,1,4,3,6,5,5,8,7],:),[1,1,1,1,nsims]);
            ...
            basecorr_connmat=np.abs(nullconnmat[:,:,[1,3,5,7],:])#-abs(base_connmat);%ideal 3COH,8imCOH            
            for cc1 in range(4):
            ...
            pval=np.zeros((nnodes,nnodes,4,nsims));
            print 'ttest'
            for cc1 in range(4):
                print cc1
...
#                        pval[cc1,cc2,cc3,ttcnt]=scistats.t.sf(np.abs(thist),nsubs-1) 
            out_name=inpath+'nullperm_pval_parc'+str(which_parc)+'.mat'
