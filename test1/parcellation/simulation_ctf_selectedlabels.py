# -*- coding: utf-8 -*-
"""
Created on Thu Jul  7 01:35:09 2016
...
label_RMF=thislabel_stc.in_label(labellist[0])
stc_all_data[label_RMF.vertices[0],1,:]=-10
stc_summary=np.concatenate((stc_mean[:,np.newaxis],stc_all_data.mean(axis=2)[:,0][:,np.newaxis],-stc_all_data.mean(axis=2)[:,1][:,np.newaxis],stc_all_data.mean(axis=2)[:,2][:,np.newaxis]),axis=1)
...
out_file=label_path + 'postcentral_parts_CTF'
...
#######
#stc_ad=np.mean(stc_all_data,axis=2)
#stc_ad_out = mne.SourceEstimate(stc_ad, vertices=vertices_avg, tmin=1e-3*2, tstep=1e-3*2, subject=subject_avg, verbose=None)
#out_file=label_path + 'RMF_STS_LOC_all3_CTF'
#stc_ad_out.save(out_file)
