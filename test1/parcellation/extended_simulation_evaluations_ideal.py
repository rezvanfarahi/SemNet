...
#            base_connmat=repmat(nullconnmat(:,:,[2,1,1,4,3,6,5,5,8,7],:),[1,1,1,1,nsims]);
            ...
            basecorr_connmat=np.mean(np.abs(connmat),axis=3)#-abs(base_connmat);%ideal 3COH,8imCOH  
            pvalid=np.zeros((nnodes,nnodes,nsims));
            for cc1 in range(nsims):
                this_connmat=np.squeeze(basecorr_connmat[:,:,2,cc1]).copy()
                this_connmat[this_connmat<0.08]=-1000
                this_connmat[this_connmat>=0.08]=0.01
                this_connmat=this_connmat+np.triu(1000*np.ones((nnodes,nnodes)))
                this_connmat[this_connmat<0]=1.0
                
                pvalid[:,:,cc1]=this_connmat.copy()
...
#                        pval[cc1,cc2,cc3,ttcnt]=scistats.t.sf(np.abs(thist),nsubs-1) 
            out_name=inpath+'ideal_pval_parc'+str(which_parc)+'.mat'
            scipy.io.savemat(out_name,{'pvalid':pvalid})
