...
#ypos_RG_name='ypos_aparcmod_badremoved_74.mat'
#path_ypos='/imaging/rf02/parcellation/leakage_pattern/'+ypos_RG_name
#ltc=scipy.io.loadmat(path_ypos,mat_dtype=True); ypos_RG=np.squeeze(ltc['ypos_sort'])
#ctf_to_ypos=ctf_to_new[:,ypos_sort][ypos_sort]


anat_name='fsaverage_finalpostparc2009_smooth_stddiff_split_merge_halfmax_98_badremoved_74'#'fsaverage_sorted_RG_finalpostparc_smooth_stddiff_split_merge_halfmax_82_badremoved_70'#'fsaverage_finalpostparc2009_smooth_stddiff_split_merge_halfmax_98_badremoved_74'#
ypos_anat_name='ypos_RG_badremoved_70.mat'#'ypos_aparc2009mod_badremoved_74.mat'
path_ypos='/imaging/rf02/parcellation/leakage_pattern/'+ypos_anat_name
ltc=scipy.io.loadmat(path_ypos,mat_dtype=True); ypos_anat=np.squeeze(ltc['ypos_sort'])[::2]
...
labels_anatl=labels_anat1[::2]
labels_anatr=labels_anat1[1::2]
lh_cov=np.sum(np.asarray([len(stc_obj.in_label(labels_anatl[ii]).vertices[0]) for ii in range(len(labels_anatl))]))
rh_cov=np.sum(np.asarray([len(stc_obj.in_label(labels_anatr[ii]).vertices[1]) for ii in range(len(labels_anatr))]))
print lh_cov+rh_cov



#orig_aparc = mne.read_labels_from_annot(subject='fsaverage', parc='aparc', subjects_dir=data_path)#parc='rez_aparc_24sep15'
#orig_aparc.remove(orig_aparc[-1])
#orig_aparcl=orig_aparc[::2]
#orig_aparcr=orig_aparc[1::2]
#lh_cov=np.sum(np.asarray([len(stc_obj.in_label(orig_aparcl[ii]).vertices[0]) for ii in range(len(orig_aparcl))]))
#rh_cov=np.sum(np.asarray([len(stc_obj.in_label(orig_aparcr[ii]).vertices[1]) for ii in range(len(orig_aparcr))]))
#print lh_cov+rh_cov
...
labels_RGl=labels_RG1[::2]
labels_RGr=labels_RG1[1::2]
lh_cov=np.sum(np.asarray([len(stc_obj.in_label(labels_RGl[ii]).vertices[0]) for ii in range(len(labels_RGl))]))
rh_cov=np.sum(np.asarray([len(stc_obj.in_label(labels_RGr[ii]).vertices[1]) for ii in range(len(labels_RGr))]))
print lh_cov+rh_cov
...
anat_vert_sort=np.argsort(anat_nvert)[-1::-1]#ypos_anat/2#
not_avail=list()
...
thist=[anat_vert_sort[not_avail.index(ii)] for ii in range(len(not_avail)) if ii in not_avail]
final_overlap=np.zeros((len(labels_anat),len(labels_RG)))
for cntrg in range(len(labels_RG)):
    for cntanat, thisanat in enumerate(thist):
        thisRG_verts=labels_RG[cntrg].vertices
        thisanat_verts=labels_anat[thisanat].vertices
        final_overlap[cntanat,cntrg]=len(np.intersect1d(thisanat_verts,thisRG_verts))/np.float(len(thisanat_verts))
aa=np.concatenate((np.sum(final_overlap,1)[:,np.newaxis],final_overlap),axis=1)  
bb=np.concatenate((np.zeros((aa.shape[0],1)),aa),axis=1)       
plt.imshow(bb, cmap='hot', aspect='equal', interpolation="nearest")#, vmin=0, vmax=1)
plt.xlabel('Modified Desikan-Killiany Parcellation', fontsize=14)
plt.ylabel('Modified Destrieux Parcellation')
#plt.ylabel('Modified Region Growing Parcellation')
plt.title('Percentage of Label Overlaps')        
cbar=plt.colorbar()
#np.corrcoef(final_overlap,np.identity())