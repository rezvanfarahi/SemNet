...
for simcnt in ll:
    for nseeds in np.array([3]): #,10,15,20
        print nseeds
#        thissnr=3
        out_path='/imaging/rf02/parcellation/extended_simulations/sims2'+str(n_sims)+'_offset'+str(sim_offset)+'_seeds'+str(nseeds)+'_conns'+str(connperc)+'_bothsnrs_ext'+str(ext)
...
#                allnoise_label_ts = mne.extract_label_time_course(allnoise_stc_sim, morphed_labelsall, src_sub, mode='mean_flip',return_generator=False)
        ...
        missedcons_mat=np.asarray(missedcons_grandmat)
        import pickle
        ...
        this_pathto=out_path+'/missedcons_mat_sim'+str(perm_inds[0]+sim_offset)+'.mat'
        scipy.io.savemat(this_pathto,{'missedcons_mat':missedcons_mat})
