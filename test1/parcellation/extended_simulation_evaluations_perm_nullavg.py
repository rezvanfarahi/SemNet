...
for nseeds in [3]:
    ...
    for nconns in [1]:
        print nconns
        concnt=concnt+1;
        for which_parc in range(1):
            nsims=36;#length(FileName);
            nsubs=17;
            print which_parc
            #%both SNRs
            #;%leakageSNR3,noiseSNR3,ideal,leakageSNR1,noiseSNR1 (COH 1st, imCOH2nd)
#            cc=-1;
...
#            base_connmat=repmat(nullconnmat(:,:,[2,1,1,4,3,6,5,5,8,7],:),[1,1,1,1,nsims]);
            ...
            basecorr_connmat=np.abs(nullconnmat[:,:,[0,2],:,:])#-abs(base_connmat);%ideal 3COH,8imCOH            
            for cc1 in range(2):
                for cc2 in range(nsubs):
                    for cc3 in range(nsims):
                        ...
                        tcm=np.mean(this_connmat[this_connmat<1000])
                        this_connmat=this_connmat-np.triu(1000*np.ones((nnodes,nnodes)))
                        this_connmat=this_connmat-tcm+np.triu(tcm*np.ones((nnodes,nnodes)))
                        basecorr_connmat[:,:,cc1,cc2,cc3]=this_connmat.copy()
            pval=np.zeros((nnodes,nnodes,2,nsims));
            print 'ttest'
            for cc1 in range(2):
                print cc1
                ...
                for cc2 in range(nsims):
                    ...
                    tval,thispval,H0=mne.stats.permutation_t_test(X,tail=1,n_jobs=1)
...
#                        pval[cc1,cc2,cc3,ttcnt]=scistats.t.sf(np.abs(thist),nsubs-1) 
            out_name=inpath2+'nullperm_pval_parc'+str(which_parc)+'.mat'
