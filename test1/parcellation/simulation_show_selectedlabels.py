...
labelss = mne.read_labels_from_annot(subject='fsaverage', parc='rez_aparc_25oct16', subjects_dir=data_path) #rez_aparc_24sep15
...
#label_index=np.array([label_names.index('supramarginal-lh'),label_names.index('lateraloccipital-lh')])
#label_index=np.array([label_names.index('parsorbitalis-lh'),label_names.index('parstriangularis-lh')])
label_index=np.array([label_names.index('inferior2_postcentral-lh'),label_names.index('superior2_postcentral-lh')])
#label_index=np.array([label_names.index('supramarginal-lh')])
#labellist=[mne.read_label(label) for label in labellist_path]
labellist=[labelss[lcnt] for lcnt in label_index]
thislabel_data=np.zeros((20484,1))
...
label0=thislabel_stc.in_label(labellist[0])
label1=thislabel_stc.in_label(labellist[1])
#label2=thislabel_stc.in_label(labellist[2])

thislabel_data[label0.vertices[0]]=10
#thislabel_data[label1.vertices[0]]=5
thislabel_data[label1.vertices[0]]=-10

final_stc = mne.SourceEstimate(thislabel_data, vertices=vertices_avg, tmin=1e-3*2, tstep=1e-3*2, subject=subject_avg, verbose=None)
out_path=data_path+'parcellation/labels_and_CTFs/'
#out_file=out_path + 'sim_RMF_STS_LOC'
out_file=out_path + 'postcentral_parts'

final_stc.save(out_file)
...
#    if label_append[0].vertices.shape[0]>=min_act_lbl and labels[newlabels[labcnt,1]].hemi==labels[newlabels[labcnt,0]].hemi:
#        nver_merge.append(thislabel_verts)
