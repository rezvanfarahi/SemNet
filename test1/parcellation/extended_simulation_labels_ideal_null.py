...
for simcnt in ll:
    for nseeds in np.array([5]): 
        print nseeds
#        thissnr=3
        out_path='/imaging/rf02/parcellation/extended_simulations/null_sims'+str(n_sims)+'_offset'+str(sim_offset)+'_bothsnrs'
...
#            labellist=mne.grow_labels(subject=subject_avg,seeds=seedss,extents=ext,hemis=hemchoose,subjects_dir=data_path,overlap=False)
#            for which_parc in range(len(parc_name)):                
...
#            ovcheck=np.sum(seedov)
        ...
        parc0_connmat=np.zeros((parc_len[0],parc_len[0],8,len(subject_inds)))#nconxnconxnmeths,nsubs
        parc1_connmat=np.zeros((parc_len[1],parc_len[1],8,len(subject_inds)))
        parc2_connmat=np.zeros((parc_len[2],parc_len[2],8,len(subject_inds)))
        parc3_connmat=np.zeros((parc_len[3],parc_len[3],8,len(subject_inds)))
        parc4_connmat=np.zeros((parc_len[4],parc_len[4],8,len(subject_inds)))
        n_epochs=40
...
#        nulldatamat_grand=np.zeros((nseeds,blen+len(t),n_epochs))
        datamat1=np.zeros((nseeds,blen+len(t)))
#        nulldatamat1=np.zeros((nseeds,blen+len(t)))
#        nullAmat=np.eye(Amat.shape[0],Amat.shape[1])
...
#                nulldatamat1[rcnt,:]=np.matmul(nullAmat[rcnt,:],seed_sig_forR.transpose())
            datamat_grand[:,:,ecnt]=datamat1.copy()
#            nulldatamat_grand[:,:,ecnt]=nulldatamat1.copy()
        ii=-1   
...
#                fname_split=parc_name[which_parc]+'_'+str(perm_inds[0])
...
##                    idealconn_grandmat.append(idealconn)
...
#                    misscatch_grandmat.append(misscatch)
    ...
    #[labellist_m[jj].morph(subject_from='fsaverage', smooth=5, subject_to=subjects[subject_no], subjects_dir=data_path) for jj in range(len(labellist_m))]
                parc_datamat_grand=datamat_grand.copy()
#                nullparc_datamat_grand=nulldatamat_grand[np.where(im>0)[0],:,:].copy()
                label_colors = [label.color for label in labels]
                allnoise_stc_sim=range(n_epochs)
                lowsnr_allnoise_stc_sim=range(n_epochs)
#                nullstc_sim=range(n_epochs)
                for ecnt in range(n_epochs):  
                    datamat=parc_datamat_grand[:,:,ecnt].copy()
#                    nulldatamat=nullparc_datamat_grand[:,:,ecnt].copy()
                    this_std=np.sqrt(np.mean(datamat[0,125:]**2)/(thissnr**2))#SNR=3 
                    lowsnr_this_std=np.sqrt(np.mean(datamat[0,125:]**2)/(lowsnr**2))
#                    nullthis_std=np.sqrt(np.mean(nulldatamat[0,125:]**2)/(thissnr**2))
                    tmin=-0.124
                    tstep=0.001
                    ...
                    nullthislabel_data=np.zeros((len(lvert)+len(rvert),datamat.shape[1]))
                    lowsnr_allnoise_thislabel_data=np.zeros((len(lvert)+len(rvert),datamat.shape[1]))
                    lowsnr_nullthislabel_data=np.zeros((len(lvert)+len(rvert),datamat.shape[1]))
...
#                        nullthislabel_data[fwd_idx+offset,:]=nulldatamat[llcnt,:].copy()
...
#                    tobe_noise=np.asarray([thiszero for thiszero in range(thislabel_data.shape[0]) if thiszero not in not_noise])
                    ...
                    lowsnr_noise_std=lowsnr_this_std.copy()
#                    nullnoise_std=nullthis_std.copy()#np.sqrt(np.mean(nulldatamat[0,125:]**2))
#                    noise_mat=noise_std*np.random.randn(tobe_noise.shape[0],thislabel_data[:,125:].shape[1])+np.mean(datamat[0,125:])
#                    nullnoise_mat=nullnoise_std*np.random.randn(tobe_noise.shape[0],nullthislabel_data[:,125:].shape[1])+np.mean(nulldatamat[0,125:])
                    allnoise_mat=noise_std*np.random.randn(tobe_allnoise.shape[0],thislabel_data[:,125:].shape[1])+np.mean(datamat[0,125:])
                    lowsnr_allnoise_mat=lowsnr_noise_std*np.random.randn(tobe_allnoise.shape[0],thislabel_data[:,125:].shape[1])+np.mean(datamat[0,125:])
...
#                    thislabel_data[tobe_noise,125:]=noise_mat
                    nullthislabel_data[:,125:]=allnoise_mat
                    allnoise_thislabel_data[:,125:]=allnoise_mat
                    lowsnr_nullthislabel_data[:,125:]=lowsnr_allnoise_mat
                    lowsnr_allnoise_thislabel_data[:,125:]=lowsnr_allnoise_mat
                    print ecnt
                    rmat=this_std*np.random.randn(thislabel_data.shape[0],thislabel_data.shape[1])
                    lowsnr_rmat=lowsnr_this_std*np.random.randn(thislabel_data.shape[0],thislabel_data.shape[1])
#                    nullrmat=nullthis_std*np.random.randn(nullthislabel_data.shape[0],nullthislabel_data.shape[1])
    
#                    thislabel_datan=thislabel_data+rmat
                    allnoise_datan=allnoise_thislabel_data+rmat
                    nullthislabel_datan=allnoise_thislabel_data+rmat
                    lowsnr_allnoise_datan=lowsnr_allnoise_thislabel_data+lowsnr_rmat
                    lowsnr_nullthislabel_datan=lowsnr_allnoise_thislabel_data+lowsnr_rmat
                    allnoise_stc_sim[ecnt]=mne.SourceEstimate(allnoise_datan, vertices=vertices_sub, tmin=tmin, tstep=tstep, subject=subjects[subject_no], verbose=None)#thislabel_datan
                    lowsnr_allnoise_stc_sim[ecnt]=mne.SourceEstimate(lowsnr_allnoise_datan, vertices=vertices_sub, tmin=tmin, tstep=tstep, subject=subjects[subject_no], verbose=None)#thislabel_datan
#                    stc_sim_realideal[ecnt]=mne.SourceEstimate(thislabel_data_ideal, vertices=vertices_sub, tmin=tmin, tstep=tstep, subject=subjects[subject_no], verbose=None)#thislabel_datan
    #                nullstc_sim[ecnt]=mne.SourceEstimate(nullthislabel_datan, vertices=vertices_sub, tmin=tmin, tstep=tstep, subject=subjects[subject_no], verbose=None)#thislabel_datan
    
                    nullevoked_data=np.matmul(leadfield,nullthislabel_datan)[np.newaxis,:,:]
                    lowsnr_nullevoked_data=np.matmul(leadfield,lowsnr_nullthislabel_datan)[np.newaxis,:,:]
#                    nullevoked_data=np.matmul(leadfield,nullthislabel_datan)[np.newaxis,:,:]
#                    allnoise_evoked_data=np.matmul(leadfield,allnoise_datan)[np.newaxis,:,:]
                    if ecnt==0:
#                        epochs_data=evoked_data.copy()
                        nullepochs_data=nullevoked_data.copy()
                        lowsnr_nullepochs_data=lowsnr_nullevoked_data.copy()
...
#                        allnoise_epochs_data=np.append(allnoise_epochs_data,allnoise_evoked_data,axis=0)
                        nullepochs_data=np.append(nullepochs_data,nullevoked_data,axis=0)
                        lowsnr_nullepochs_data=np.append(lowsnr_nullepochs_data,lowsnr_nullevoked_data,axis=0)
...
#                allnoise_epochs_fwd = mne.EpochsArray(allnoise_epochs_data, info=epochs1.info, events=events, event_id=1, tmin=tmin)
                nullepochs_fwd = mne.EpochsArray(nullepochs_data, info=epochs1.info, events=events, event_id=1, tmin=tmin)
                lowsnr_nullepochs_fwd = mne.EpochsArray(lowsnr_nullepochs_data, info=epochs1.info, events=events, event_id=1, tmin=tmin)
    
                snr = thissnr#3.0#ediv.mean()
                lambda2 = 1.0 / snr ** 2
                lowsnr_lambda2 = 1.0 / lowsnr ** 2
                method = 'MNE'
...
#                allnoise_stc_sim = apply_inverse_epochs(allnoise_epochs_fwd, inverse_operator_eegmeg, lambda2,method=method, pick_ori="normal")
                nullstc_sim = apply_inverse_epochs(nullepochs_fwd, inverse_operator_eegmeg, lambda2,method=method, pick_ori="normal")
                lowsnr_nullstc_sim = apply_inverse_epochs(lowsnr_nullepochs_fwd, inverse_operator_eegmeg, lowsnr_lambda2,method=method, pick_ori="normal")
...
#                label_ts = mne.extract_label_time_course(stc_sim, morphed_labelsall, src_sub, mode='mean_flip',return_generator=False)
                ...
                nulllabel_ts = mne.extract_label_time_course(nullstc_sim, morphed_labelsall, src_sub, mode='mean_flip',return_generator=False)
                lowsnr_allnoise_label_ts = mne.extract_label_time_course(lowsnr_allnoise_stc_sim, morphed_labelsall, src_sub, mode='mean_flip',return_generator=False)
                lowsnr_nulllabel_ts = mne.extract_label_time_course(lowsnr_nullstc_sim, morphed_labelsall, src_sub, mode='mean_flip',return_generator=False)
...
#                label_ts_realideal = mne.extract_label_time_course(stc_sim_realideal, morphed_labelsall, src_sub, mode='mean_flip',return_generator=False)
                ...
                allnoise_label_ts, method=con_methods, mode='multitaper', sfreq=sfreq, fmin=fmin,fmax=fmax, 
                ...
                con_ideal, freqs, times, n_epochs, n_tapers = spectral_connectivity(
                    nulllabel_ts, method=con_methods, mode='multitaper', sfreq=sfreq, fmin=fmin,
                ...
                con_realideal, freqs, times, n_epochs, n_tapers = spectral_connectivity(
                    lowsnr_allnoise_label_ts, method=con_methods, mode='multitaper', sfreq=sfreq, fmin=fmin,
                    fmax=fmax, faverage=True, mt_adaptive=True, n_jobs=1)
                con_res_realideal = np.squeeze(con_realideal.copy())
                print "computing 4th coh"
                nullcon, freqs, times, n_epochs, n_tapers = spectral_connectivity(
                lowsnr_nulllabel_ts, method=con_methods, mode='multitaper', sfreq=sfreq, fmin=fmin,fmax=fmax, 
                faverage=True, mt_adaptive=True, n_jobs=1)                
                nullcon_res = np.squeeze(nullcon.copy())
#                print "computing 5th coh"
#                allnoise_con, freqs, times, n_epochs, n_tapers = spectral_connectivity(
#                allnoise_label_ts, method=con_methods, mode='multitaper', sfreq=sfreq, fmin=fmin,fmax=fmax, 
#                faverage=True, mt_adaptive=True, n_jobs=1)                
#                allnoise_con_res = np.squeeze(allnoise_con.copy())#np.mean(con,axis=2)#dict()
                
                print "computing 1st imcoh"
                con_methods2 = ['imcoh']#['pli2_unbiased']
                con2,  freqs, times, n_epochs, n_tapers = spectral_connectivity(
                    allnoise_label_ts, method=con_methods2, mode='multitaper', sfreq=sfreq, fmin=fmin,
                    fmax=fmax, faverage=True, mt_adaptive=True, n_jobs=1)
                ...
                con_ideal2, freqs, times, n_epochs, n_tapers = spectral_connectivity(
                    nulllabel_ts, method=con_methods2, mode='multitaper', sfreq=sfreq, fmin=fmin,
                    fmax=fmax, faverage=True, mt_adaptive=True, n_jobs=1)
                ...
                con_realideal2, freqs, times, n_epochs, n_tapers = spectral_connectivity(
                    lowsnr_allnoise_label_ts, method=con_methods2, mode='multitaper', sfreq=sfreq, fmin=fmin,
                    fmax=fmax, faverage=True, mt_adaptive=True, n_jobs=1)
                con_res_realideal2 = np.squeeze(con_realideal2.copy())    
                print "computing 4th imcoh"
                nullcon2, freqs, times, n_epochs, n_tapers = spectral_connectivity(
                lowsnr_nulllabel_ts, method=con_methods2, mode='multitaper', sfreq=sfreq, fmin=fmin,fmax=fmax, 
                faverage=True, mt_adaptive=True, n_jobs=1)                
                nullcon_res2 = np.squeeze(nullcon2.copy())
#                print "computing 5th imcoh"
#                allnoise_con2, freqs, times, n_epochs, n_tapers = spectral_connectivity(
#                lowsnr_nulllabel_ts, method=con_methods2, mode='multitaper', sfreq=sfreq, fmin=fmin,fmax=fmax, 
#                faverage=True, mt_adaptive=True, n_jobs=1)                
#                allnoise_con_res2 = np.squeeze(allnoise_con2.copy())
                #     con is a 3D array, get the connectivity for the first (and only) freq. band
                #     for each method
                if which_parc==0:
                    ...
                    parc0_connmat[:,:,3,cnt]=nullcon_res.copy()
                    parc0_connmat[:,:,4,cnt]=con_res2.copy()
                    parc0_connmat[:,:,5,cnt]=con_res_ideal2.copy()
                    parc0_connmat[:,:,6,cnt]=con_res_realideal2.copy()
                    parc0_connmat[:,:,7,cnt]=nullcon_res2.copy()
                if which_parc==1:
                    ...
                    parc1_connmat[:,:,3,cnt]=nullcon_res.copy()
                    parc1_connmat[:,:,4,cnt]=con_res2.copy()
                    parc1_connmat[:,:,5,cnt]=con_res_ideal2.copy()
                    parc1_connmat[:,:,6,cnt]=con_res_realideal2.copy()
                    parc1_connmat[:,:,7,cnt]=nullcon_res2.copy()
                if which_parc==2:
                    ...
                    parc2_connmat[:,:,3,cnt]=nullcon_res.copy()
                    parc2_connmat[:,:,4,cnt]=con_res2.copy()
                    parc2_connmat[:,:,5,cnt]=con_res_ideal2.copy()
                    parc2_connmat[:,:,6,cnt]=con_res_realideal2.copy()
                    parc2_connmat[:,:,7,cnt]=nullcon_res2.copy()                  
                if which_parc==3:
                    ...
                    parc3_connmat[:,:,3,cnt]=nullcon_res.copy()
                    parc3_connmat[:,:,4,cnt]=con_res2.copy()
                    parc3_connmat[:,:,5,cnt]=con_res_ideal2.copy()
                    parc3_connmat[:,:,6,cnt]=con_res_realideal2.copy()
                    parc3_connmat[:,:,7,cnt]=nullcon_res2.copy()
                if which_parc==4:
                    ...
                    parc4_connmat[:,:,3,cnt]=nullcon_res.copy()
                    parc4_connmat[:,:,4,cnt]=con_res2.copy()
                    parc4_connmat[:,:,5,cnt]=con_res_ideal2.copy()
                    parc4_connmat[:,:,6,cnt]=con_res_realideal2.copy()
                    parc4_connmat[:,:,7,cnt]=nullcon_res2.copy()
...
#        this_pathto=out_path+'/idealconn_grandmat_sim'+str(perm_inds[0]+sim_offset)+'.py'#'DKA68_orig_modified_coh.mat'#lh.fsaverage_finalpostparc_smooth_split_merge.annot            
#        with open(this_pathto,'wb') as fp:
#            pickle.dump(idealconn_grandmat,fp)
