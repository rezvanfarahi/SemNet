...
for simcnt in ll:
    for nseeds in np.array([5]): #,10,15,20
        print nseeds
#        thissnr=3
        out_path='/imaging/rf02/parcellation/extended_simulations/avgsims'+str(n_sims)+'_offset'+str(sim_offset)+'_bothsnrs_extglasser'
...
#                    not_noise=np.unique(np.where(thislabel_data!=0)[0])
                    tobe_noise=np.asarray([thiszero for thiszero in range(thislabel_data.shape[0])])
...
#                    thislabel_data_ideal[tobe_noise,125:]=0.0001*noise_mat
...
#                    stc_sim_realideal[ecnt]=mne.SourceEstimate(thislabel_data_ideal, vertices=vertices_avg, tmin=tmin, tstep=tstep, subject='fsaverage', verbose=None)#thislabel_datan
                    ...
                    thislabel_datan_morphed=stc_sim_pre_morphed.data
...
#                #     for each method
                ...
                if which_parc==0:
                    ...
                    parc0_connmat[:,:,2,cnt]=lowsnr_con_res.copy()
                    parc0_connmat[:,:,3,cnt]=lowsnr_con_res_ideal.copy()
                    parc0_connmat[:,:,4,cnt]=con_res2.copy()
                    parc0_connmat[:,:,5,cnt]=con_res_ideal2.copy()
                    parc0_connmat[:,:,6,cnt]=lowsnr_con_res2.copy()
                    parc0_connmat[:,:,7,cnt]=lowsnr_con_res_ideal2.copy()
                if which_parc==1:
                    parc1_connmat[:,:,0,cnt]=con_res.copy()#con_res_final=con_res[:,:,np.newaxis].copy()
                    parc1_connmat[:,:,1,cnt]=con_res_ideal.copy()
                    parc1_connmat[:,:,2,cnt]=lowsnr_con_res.copy()
                    parc1_connmat[:,:,3,cnt]=lowsnr_con_res_ideal.copy()
                    parc1_connmat[:,:,4,cnt]=con_res2.copy()
                    parc1_connmat[:,:,5,cnt]=con_res_ideal2.copy()
                    parc1_connmat[:,:,6,cnt]=lowsnr_con_res2.copy()
                    parc1_connmat[:,:,7,cnt]=lowsnr_con_res_ideal2.copy()
                if which_parc==2:
                    parc2_connmat[:,:,0,cnt]=con_res.copy()#con_res_final=con_res[:,:,np.newaxis].copy()
                    parc2_connmat[:,:,1,cnt]=con_res_ideal.copy()
                    parc2_connmat[:,:,2,cnt]=lowsnr_con_res.copy()
                    parc2_connmat[:,:,3,cnt]=lowsnr_con_res_ideal.copy()
                    parc2_connmat[:,:,4,cnt]=con_res2.copy()
                    parc2_connmat[:,:,5,cnt]=con_res_ideal2.copy()
                    parc2_connmat[:,:,6,cnt]=lowsnr_con_res2.copy()
                    parc2_connmat[:,:,7,cnt]=lowsnr_con_res_ideal2.copy()
                if which_parc==3:
                    parc3_connmat[:,:,0,cnt]=con_res.copy()#con_res_final=con_res[:,:,np.newaxis].copy()
                    parc3_connmat[:,:,1,cnt]=con_res_ideal.copy()
                    parc3_connmat[:,:,2,cnt]=lowsnr_con_res.copy()
                    parc3_connmat[:,:,3,cnt]=lowsnr_con_res_ideal.copy()
                    parc3_connmat[:,:,4,cnt]=con_res2.copy()
                    parc3_connmat[:,:,5,cnt]=con_res_ideal2.copy()
                    parc3_connmat[:,:,6,cnt]=lowsnr_con_res2.copy()
                    parc3_connmat[:,:,7,cnt]=lowsnr_con_res_ideal2.copy()
                if which_parc==4:
                    parc4_connmat[:,:,0,cnt]=con_res.copy()#con_res_final=con_res[:,:,np.newaxis].copy()
                    parc4_connmat[:,:,1,cnt]=con_res_ideal.copy()
                    parc4_connmat[:,:,2,cnt]=lowsnr_con_res.copy()
                    parc4_connmat[:,:,3,cnt]=lowsnr_con_res_ideal.copy()
                    parc4_connmat[:,:,4,cnt]=con_res2.copy()
                    parc4_connmat[:,:,5,cnt]=con_res_ideal2.copy()
                    parc4_connmat[:,:,6,cnt]=lowsnr_con_res2.copy()
                    parc4_connmat[:,:,7,cnt]=lowsnr_con_res_ideal2.copy()
