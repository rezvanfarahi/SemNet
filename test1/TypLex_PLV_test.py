...
# get 2 frequency bins
		coh, freqs, times, n_epochs, n_tapers = spectral_connectivity(stcs, method='coh', mode='fourier', indices=indices,sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=True, tmin=tmin, tmax=tmax, n_jobs=2)
#spectral_connectivity(data, method='coh', indices=None, sfreq=6.283185307179586, mode='multitaper', fmin=None, fmax=inf, fskip=0, faverage=False, tmin=None, tmax=None, mt_bandwidth=None, mt_adaptive=False, mt_low_bias=True, cwt_frequencies=None, cwt_n_cycles=7, block_size=1000, n_jobs=1, verbose=None)
		...
		coh_stc = mne.SourceEstimate(coh, vertices=stc.vertno, tmin=1e-3 * tmin, tstep=1e-3 * tstep, subject=subjects[ii])
		data_out = data_path + meg +  'TEST_SemLoc_Concrete_Coherence_AlphaBeta_' + label_name[0:-3]
