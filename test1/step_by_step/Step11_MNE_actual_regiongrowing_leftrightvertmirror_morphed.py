...
stc_all=range(16)
assign_vertex=np.zeros((20484,16))
...
leftright_labels=np.hstack((unver,unver)) 
...
for flccnt,flc in enumerate(unver):
    print flc
    thislabel_left=flc#np.where(assign_vertex==unver[flc+1])[0]
    #    num_verts[flc]=thislabel.shape[0]
    #    thislabel_left=np.zeros((assign_vertex.shape[0],1))
    #    thislabel_left=np.where(assign_vertex==thislabel)[0]
    #    thislabel_left=thislabel_left[thislabel_left<10242]
    leftright_labels[flccnt*2]=int(thislabel_left)
    #    thislabel_right=thislabel_left.copy()
    #    for thislabii in range(len(thislabel_left)):
    this_vertex=MNIlh[flc,:]
    this_dist=np.sqrt(np.sum((MNIrh-this_vertex)**2,1))
    min_dist=np.min(this_dist)
    w_min_dist=np.argmin(this_dist)
    while np.sum(thislabel_left==w_min_dist+10242)==1:
            this_dist[w_min_dist]=20000;
            min_dist=np.min(this_dist)
            w_min_dist=np.argmin(this_dist)
    thislabel_right=w_min_dist+10242;
    leftright_labels[flccnt*2+1]=int(thislabel_right)

unver=leftright_labels.copy()
...
for rgcnt in range(len(Mdiag_unver)):
    ...
    if np.min(Mdiag_unver)<1000:
        ...
        if unver[rgcnt]<10242:
            this_seed_realm=this_seed_realm[this_seed_realm<10242]
        else:
            this_seed_realm=this_seed_realm[this_seed_realm>=10242]
...
unvercount= np.diff(idx)
#unver=unver_pre[unvercount>=10]
unver_remove=unver_pre[unvercount<10]
if unver_remove.any():
...
unver_bal=list()
print len(unver)
for klcnt in range(0,len(leftright_labels),2):
    if leftright_labels[klcnt] in unver and leftright_labels[klcnt+1] in unver:
        unver_bal.append(leftright_labels[klcnt])
        unver_bal.append(leftright_labels[klcnt+1])
...
for flc in range(len(unver)):
    print flc
    thislabel=np.where(assign_vertex_new==unver[flc])[0]
#    num_verts[flc]=thislabel.shape[0]
    ...
    if label_append[0].hemi=='lh':#unver[flc+1]<len(assign_vertex)/2.0:
        labelsv[flc][0].name='L'+str(int(unver[flc/2]))+'-lh'
    else:
        labelsv[flc][0].name='L'+str(int(unver[flc/2]))+'-rh'     
