...
## epoching
    ...
    bands = dict(alpha=[8,12])#, alpha=[8,12], beta=[13, 30], gamma=[31, 45])#, beta=[13, 30], gamma=[31, 45])
    snr = ediv.mean()
    lambda2 = 1.0 / snr ** 2
    print "source_band_induced_power concrete"
    stcs_concrete = source_band_induced_power(epochs_concrete, inv_op, bands=bands, label=None, lambda2=lambda2, method='MNE', n_cycles=5, baseline=(-400,0), baseline_mode='ratio', use_fft=False)
    ...
    stcs_abstract = source_band_induced_power(epochs_abstract, inv_op, bands=bands, label=None, lambda2=lambda2, method='MNE', n_cycles=5, baseline=(-400,0), baseline_mode='ratio', use_fft=False)
