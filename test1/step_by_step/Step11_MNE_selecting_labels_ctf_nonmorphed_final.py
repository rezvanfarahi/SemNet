...
for cnt, meg in enumerate(ll):
    ...
    path_c=data_path+'explained90all.mat'
    ltc=scipy.io.loadmat(path_c,mat_dtype=True); explained90=np.squeeze(ltc['EXP90'])
    llnew=[labels[ii] for ii in range(len(labels)) if explained90[ii]==1 or len(labels[ii].vertices)<100]
    lln1=[labels[ii].split(explained90[ii], subjects_dir=data_path)[:] for ii in range(len(labels)) if explained90[ii]>1 and len(labels[ii].vertices)>=100]
    llnn=list()
...
#    
    stc_ctf_mne, ep90=psf_ctf_18Dec15.cross_talk_function(inverse_operator_eegmeg, forward, labels_forcolor,method=method, lambda2=lambda2, signed=False, mode=mode, n_svd_comp=1, verbose=None)
...
#        Matx_normalised[:,cntr]=np.divide(Matx[:,cntr],Matx[:,-1])
    ...
    merge_keepB=np.concatenate((merge_keepL,merge_keepR),axis=0)
