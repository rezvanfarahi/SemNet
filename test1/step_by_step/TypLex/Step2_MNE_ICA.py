...
for cnt, meg in enumerate(ll):
	...
	print meg
	fname = data_path + meg + 'typlex_pw_ssstf_fft_1_48_raw.fif'
	...
	for picks, ch_type in get_data_picks(raw, meg_combined=ica_meg_combined):
	    ...
	    out_path=data_path+meg+'{}-ica_typlex.fif'.format(ch_type)
	    print ch_type + " finished"
	    ica.save(out_path)
	#raw.append(ica)
	#out_path2=data_path+meg+'semloc_ssstf_fft_1_48_clean_ica_raw.fif'
	#raw.save(out_path2)
	    report.save(data_path+'typlex_preprocessing-report-'+subject+'-'+ch_type+'.html', open_browser=False, overwrite=True)
	ica_path=data_path+meg+'meg-ica_typlex.fif'
	ica=read_ica(ica_path)
	raw=ica.apply(raw)
	ica_path=data_path+meg+'eeg-ica_typlex.fif'
	ica=read_ica(ica_path)
	raw=ica.apply(raw)
	data_out_path=data_path+meg+'typlex_pw_ssstf_fft_1_48_clean_ica_raw.fif'
