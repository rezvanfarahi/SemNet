...
for ii, meg in enumerate(list_all):
	...
	epochs_words = epochs['Words']
	epochs_pwords = epochs['PWords']
	print ii; print epochs_words.get_data().shape[0]
	print epochs_pwords.get_data().shape[0]



	#epochs.plot()
	## averaging
	out_evoked_fname = data_path + meg + 'typlex_pw_ssstf_fft_1_48_clean_ica_raw-ave.fif'
	print "averaging"
	evokeds = [epochs[name].average() for name in ['Words', 'PWords']]  # average epochs and get an Evoked dataset.
