...
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/typlex/stc/permutation/connectivity/WB_hubs/st/newag/'    # where event files are
...
#            		fname_cncrt = data_path + meg + 'Morphed_ico_typlex_ica_equalized_word_new_ctf_MI_150_450_200ms_' + label_name[0:-3] 
            		stc_cncrt_mi = mne.read_source_estimate(fname_cncrt)#; stc_cncrt.data[stc_cncrt.data<0]=0 
            		fname_cncrt = data_path + meg + 'Morphed_ico_typlex_ica_equalized_word_Mlttap_new_ctf_PPC_'+band+'_150_450_200ms_' + label_name[0:-3] 
            		stc_cncrt_wpli2d = mne.read_source_estimate(fname_cncrt)
            		fname_cncrt = data_path + meg + 'Morphed_ico_typlex_ica_equalized_word_Mlttap_new_ctf_Coherence_'+band+'_150_450_200ms_' + label_name[0:-3] 
...
#            		fname_abs = data_path + meg + 'Morphed_ico_typlex_ica_equalized_nonword_new_ctf_MI_150_450_200ms_' + label_name[0:-3]   
            		stc_abs_mi = mne.read_source_estimate(fname_abs)#; stc_abs.data[stc_abs.data<0]=0 	
            		fname_abs = data_path + meg + 'Morphed_ico_typlex_ica_equalized_nonword_Mlttap_new_ctf_PPC_'+band+'_150_450_200ms_' + label_name[0:-3] 
            		stc_abs_wpli2d = mne.read_source_estimate(fname_abs)
            		fname_abs = data_path + meg + 'Morphed_ico_typlex_ica_equalized_nonword_Mlttap_new_ctf_Coherence_'+band+'_150_450_200ms_' + label_name[0:-3] 
...
#            		Xa[:,:,:,ii]=np.dstack(((data_abs_mi-data_abs_mi.min(axis=0))/(data_abs_mi.max(axis=0)-data_abs_mi.min(axis=0)),(data_abs_wpli2d-data_abs_wpli2d.min(axis=0))/(data_abs_wpli2d.max(axis=0)-data_abs_wpli2d.min(axis=0)),(data_abs_coh-data_abs_coh.min(axis=0))/(data_abs_coh.max(axis=0)-data_abs_coh.min(axis=0))))#np.dstack((np.abs(stc_abs_mi.data)/np.abs(stc_abs_mi.data).max(axis=0),np.abs(stc_abs_wpli2d.data)/np.abs(stc_abs_wpli2d.data).max(axis=0),np.abs(stc_abs_coh.data)/np.abs(stc_abs_coh.data).max(axis=0)))
            		Xc[:,:,:,ii]=np.dstack((data_cncrt_mi,data_cncrt_wpli2d,data_cncrt_coh))
            		Xa[:,:,:,ii]=np.dstack((data_abs_mi,data_abs_wpli2d,data_abs_coh))
...
# for PCA!!!!!!!!!                       
            		path_mtx=data_path+'3conn_newag_orig_words_'+label_name_sv[:-3]+'_'+band+'.mat'
            		scipy.io.savemat(path_mtx,{'Xc':Xc})
            		path_mtx=data_path+'3conn_newag_orig_nonwords_'+label_name_sv[:-3]+'_'+band+'.mat'
...
#            	T_obst, clusterst, cluster_p_valuest, H0t = clut = spatio_temporal_cluster_1samp_test(X, connectivity=connectivity,  n_permutations=n_permutations, n_jobs=4, threshold=t_threshold, tail=0,t_power=1, spatial_exclude=spatial_exclude,step_down_p=0.05, max_step=3)
...
#                    out_file1=out_path + 'ClusP_Morphed_ico_TL_ica_equal_Sub_new_ctf_ST_'+band+'_150_450_200ms_ms' + str(max_step) +'_'+ str(p_threshold)[2:5] + '_sx_' + str(p_thresh)[2:5]+ '_' + label_name_sv[0:-3]
#                    stc_all_cluster_vist.save(out_file1)
#                      	
#                    Matx=np.zeros((20484,6))
...
#                    out_file2=out_path + 'ClusP_sw_Morphed_ico_TL_ica_equal_Sub_new_ctf_ST_'+band+'_150_450_200ms_ms' + str(max_step) +'_sx_'+ str(p_threshold)[2:5] + '_' + str(p_thresh)[2:5]+ '_' + label_name_sv[0:-3]
