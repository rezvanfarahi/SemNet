...
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/typlex/stc/permutation/power/final_allbands/' #
...
#    print cluster_p_values.min()
    ...
    good_cluster_inds = np.where(cluster_p_values<=0.1)[0]#np.intersect1d(np.where(clus_p_values <= 0.01)[0], np.where(cluster_p_values<=0.1)[0])
    ...
    if cluster_p_values.min()<=0.1:#clus_p_values.min()<=0.01 and cluster_p_values.min()<=0.1:
        ...
        out_file1=out_path + 'Per_clusp'+str(p_threshold)[2:]+'_p'+str(cluster_p_values.min())[2:]+'_TL_ica_Subtract_Power_ratio_ico_normori_eq_50_550_100ms_sx_ms' + str(max_step) 
        ...
        out_file2=out_path + 'Per_sw_clusp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_TL_ica_Subtract_Power_ratio_normori_eq_50_550_100ms_sx_ms' + str(max_step) 
