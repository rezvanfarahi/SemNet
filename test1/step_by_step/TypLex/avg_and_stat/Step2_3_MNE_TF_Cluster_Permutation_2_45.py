...
ii=-1
X=np.zeros((20484,44,17))

#X_beta=np.zeros((20484,16))
b=['delta','theta','alpha','beta','gamma']
for p_threshold in ll:
    for meg in list_all:
        ii=ii+1; Xnew=np.zeros((20484,44))
        print ii
        time_windows=['50_150ms']#,'150_250ms','250_350ms','350_450ms','450_550ms']
        Xc=np.zeros((20484,44))
        Xa=np.zeros((20484,44))
        for cc, twin in zip(range(1),time_windows):            
            fname_cncrt = data_path + meg + 'mineMorphed_ico_TypLex_ica_word_Power_normori_ratio_eq_' + twin
            stc_cncrt = mne.read_source_estimate(fname_cncrt)
            Xc[:,cc*44:(cc+1)*44]=stc_cncrt.data.copy()
            fname_abs = data_path + meg + 'mineMorphed_ico_TypLex_ica_nonword_Power_normori_ratio_eq_' + twin
            stc_abs = mne.read_source_estimate(fname_abs)
            Xa[:,cc*44:(cc+1)*44]=stc_abs.data.copy()  
                  
        X[:,:,ii]=Xc-Xa#np.log(np.divide(Xc,Xa))#
    	#X[:,1,:]=X[:,0,:].copy()
...
#        p_thresh=np.max(cluster_p_values[good_cluster_inds])#cluster_p_values.min()+0.000001
    ...
    if cluster_p_values.min()<=0.05:#clus_p_values.min()<=0.01 and cluster_p_values.min()<=0.1:
        ...
        out_file1=out_path + 'Per_clusp'+str(p_threshold)[2:]+'_p'+str(cluster_p_values.min())[2:]+'_TL_ica_Subtract_rPLV_ratio_ico_normori_eq_50_550_100ms_sx_ms' + str(max_step) +'2_45_1'
