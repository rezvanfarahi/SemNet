...
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/typlex/stc/permutation/connectivity/WB_hubs/st/newag_micoh_step1/'    # where event files are
...
#        		scipy.io.savemat(path_mtx,{'Xa':Xa})            
            	Xcf=np.hstack((np.squeeze(Xc[:,:,0,:]),np.squeeze(Xc[:,:,2,:]))) #mi,  coh
            	Xaf=np.hstack((np.squeeze(Xa[:,:,0,:]),np.squeeze(Xa[:,:,2,:])))
            	...
            	if cluster_p_valuest.min()<=0.1:
                    ...
                    out_file1=out_path + 'ClusP_Morphed_ico_SL_ica_equal_Sub_new_ctf_ST_'+band+'_150_450_200ms_ms' + str(max_step) +'_'+ str(p_threshold)[2:5] + '_sx_' + str(p_thresh)[2:5]+ '_' + label_name_sv[0:-3]
                    ...
                    out_file2=out_path + 'ClusP_sw_Morphed_ico_SL_ica_equal_Sub_new_ctf_ST_'+band+'_150_450_200ms_ms' + str(max_step) +'_sx_'+ str(p_threshold)[2:5] + '_' + str(p_thresh)[2:5]+ '_' + label_name_sv[0:-3]
