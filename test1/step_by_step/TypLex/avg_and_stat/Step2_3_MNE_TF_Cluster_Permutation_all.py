...
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/typlex/stc/permutation/power/' #
...
p_list=[0.05,0.025,0.01,0.005,0.002,0.001]
print "subject_inds:"
print subject_inds
print "No rejection"
...
for p_threshold in ll:
    for meg in list_all:
        ...
        ### word
        	...
        	fname_cncrt = data_path + meg + 'mineMorphed_ico_TypLex_ica_word_rPLV_normori_ratio_equalized_m500_700_200hz_' +thisband#'mineMorphed_ico_TypLex_icaclean_word_rPLV_ratio_equalized_m500_700_' + b
        	stc_cncrt = mne.read_source_estimate(fname_cncrt)
        	#abslt_cncrt= np.absolute(stc_cncrt.copy().crop(.3,.4).data)
        	fname_abs = data_path + meg + 'mineMorphed_ico_TypLex_ica_nonword_rPLV_normori_ratio_equalized_m500_700_200hz_' +thisband#'mineMorphed_ico_TypLex_icaclean_nonword_rPLV_ratio_equalized_m500_700_' + b
        	stc_abs = mne.read_source_estimate(fname_abs)
        	"""
        	fname_sub = data_path + meg + 'Morphed_TypLex_icaclean_Subtract_rPLV_ratio_equalized_m500_700_theta'
        	...
        	for cc in range(5):
        		...
        		Xnew[:,cc+(bandcntr-1)*5]=Xc-Xa#np.log(np.divide(Xc,Xa))#
                  
        	X[:,:,ii]=Xnew#np.log(np.divide(Xc,Xa))#
    	#X[:,1,:]=X[:,0,:].copy()
    ...
    print('Clustering.')
    max_step=25
    T_obs, clusters, cluster_p_values, H0 = clu = spatio_temporal_cluster_1samp_test(X, connectivity=connectivity, n_jobs=4, threshold=t_threshold,n_permutations=2048,tail=0,t_power=1, step_down_p=0.05, spatial_exclude=spatial_exclude,max_step=25)#, max_step=5)
...
#        p_thresh=np.max(cluster_p_values[good_cluster_inds])#cluster_p_values.min()+0.000001
    ...
    if cluster_p_values.min()<=0.05:#clus_p_values.min()<=0.01 and cluster_p_values.min()<=0.1:
        ...
        out_file1=out_path + 'Per_clusp'+str(p_threshold)[2:]+'_p'+str(cluster_p_values.min())[2:]+'_TL_ica_Subtract_rPLV_ratio_ico_normori_eq_50_550_100ms_sx_ms' + str(max_step) +'allbands'
        ...
        out_file2=out_path + 'Per_sw_clusp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_TL_ica_Subtract_rPLV_ratio_normori_eq_50_550_100ms_sx_ms' + str(max_step) +'allbands'
