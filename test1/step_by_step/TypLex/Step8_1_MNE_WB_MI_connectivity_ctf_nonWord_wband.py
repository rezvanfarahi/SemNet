...
#  n_cycles[np.logical_and((frequencies>10), (frequencies<=20))] = 3
    ...
    fname_raw = data_path + meg + 'typlex_pw_ssstf_fft_ica_raw.fif'
...
# n_cycles[np.logical_and((frequencies>10), (frequencies<=20))] = 3
        ...
        for ccc in range(len(stcs)):
            print ccc		
            data_in = data_path + meg + 'Morphed_ico_TypLex_epochs' + str(ccc) + '_icaclean_nonword_Source_m500_700'
            stc_orig=mne.read_source_estimate(data_in)
            
            seed_ts = mne.extract_label_time_course(stc_orig, label_new, src, mode='max')
...
#            stc_orig.resample(100)
        ...
        data_out = data_path + meg +  'Morphed_ico_typlex_ica_equalized_nonword_bestV_ctf_MI_150_450_200ms_' + label1.name[:-3]                                        
        print data_out
