...
inv_fname = 'typlex_InverseOperator_newreg_fft_1_48_clean_ica_EMEG-inv.fif'#'InverseOperator_fft_1_48_clean_ica_EMEG-inv.fif'
event_path = '/group/erp/data/olaf.hauk/MEG/TypLexM/data/MF22_new/'
...
## epoching
    ...
    stcs_concrete = source_band_induced_power(epochs_concrete, inv_op, bands=bands, label=None, lambda2=lambda2, method='MNE', n_cycles=9, baseline=(-400,0), baseline_mode='ratio', use_fft=False,subject=subject_from,subjects_dir=data_path,n_jobs=2)
	...
	data_out = data_path + meg + 'mineMorphed_typlex_icaclean_word_Power_ratio_equalized_m500_700_' + b
	stc_to.save(data_out)     

    print "source_band_induced_power abstract"
    stcs_abstract = source_band_induced_power(epochs_abstract, inv_op, bands=bands, label=None, lambda2=lambda2, method='MNE', n_cycles=9, baseline=(-400,0), baseline_mode='ratio', use_fft=False,subject=subject_from,subjects_dir=data_path,n_jobs=2)
    
    for b, stc in stcs_abstract.iteritems():
        #morphmat1=mne.compute_morph_matrix(subject_from, subject_to, stc.vertno, vertices_to, subjects_dir=data_path)
        stc_to=stc.copy()#stc.morph_precomputed(subject_to,vertices_to,morphmat1, subject_from) 
        data_out = data_path + meg + 'mineMorphed_typlex_icaclean_nonword_Power_ratio_equalized_m500_700_' + b
        stc_to.save(data_out)
