...
for ii, meg in enumerate(ll):
    ...
    if subject_inds[ii] < 3:
	    	word_ids=np.array([2]);# print events[events[:,2]==1,2].shape+events[events[:,2]==2,2].shape
	    	nonword_ids=np.array([6]); #print events[events[:,2]==5,2].shape+events[events[:,2]==6,2].shape
	    	...
	    	word_ids=np.array([8]); #print events[events[:,2]==7,2].shape+events[events[:,2]==8,2].shape
	    	nonword_ids=np.array([6]); #print events[events[:,2]==5,2].shape+events[events[:,2]==6,2].shape
	    	events=mne.merge_events(events,word_ids,601,replace_events=True)
	    	events=mne.merge_events(events,nonword_ids,602,replace_events=True)
	    	event_id = {'Words': 601, 'PWords': 602}
...
# 
    data_out = data_path + meg + 'firstMorphed_ico_typlex_ica_typword_Source_Evoked_m500_700'
    stc1.save(data_out)
    data_out = data_path + meg + 'firstMorphed_ico_typlex_ica_typnonword_Source_Evoked_m500_700'
    stc2.save(data_out)
