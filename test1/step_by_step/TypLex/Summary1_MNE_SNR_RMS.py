...
#    raw_fname = data_path + meg + 'typlex_pw_ssstf_fft_1_48_clean_ica_raw.fif' #clean_ssp_
...
out_path_sl=data_path+'evoked_listall_sl_pnt1.npy'
#esl_ar=np.asarray(evoked_sl)
#np.save(out_path_sl,esl_ar)
esl_ar=np.load(out_path_sl)
out_path_tl=data_path+'evoked_listall_tl_pnt1.npy'
...
evoked_tl_ga=mne.grand_average(evoked_tl)#,interpolate_bads=False
epdatatl=evoked_tl_ga.data 
emtl=epdatatl#np.abs(epdata)
#emax=np.mean(em[:,:,600:900],axis=2)#
signaltl=emtl[:,100:1100]#.max(axis=2)
#bm=np.sqrt(np.mean(epdata[:,:,100:500]**2,axis=2))
noisetl=np.mean(emtl[:,100:500],axis=1)
noise_stdtl=np.std(emtl[:,100:500],axis=1)
...
ediv_stdtl=np.sqrt(edv_stdtl.mean(axis=0))

evoked_sl_ga=mne.grand_average(evoked_sl)#,interpolate_bads=False
epdatasl=evoked_sl_ga.data 
emsl=epdatasl#np.abs(epdata)
#emax=np.mean(em[:,:,600:900],axis=2)#
signalsl=emsl[:,100:1100]#.max(axis=2)
#bm=np.sqrt(np.mean(epdata[:,:,100:500]**2,axis=2))
noisesl=np.mean(emsl[:,100:500],axis=1)
noise_stdsl=np.std(emsl[:,100:500],axis=1)
...
ediv_stdsl=np.sqrt(edv_stdsl.mean(axis=0))
...
xvals=np.arange(-200,600,1)
import matplotlib.pyplot as plt
plt.plot(xvals,ediv_stdtl[200:],color='red',label='Lexical Decision')
plt.hold(True) 
plt.plot(xvals,ediv_stdsl[200:],color='black',label='Semantic Decision')
plt.xlabel('time (ms)')
plt.ylabel('RMS of SNR')
plt.legend()
plt.show()
