...
labels_fc = mne.read_labels_from_annot(subject='fsaverage', parc='fsaverage_onlyvertices_actualRG_74', subjects_dir=subjects_dir)
...
for cnt, meg in enumerate(ll):
    ...
    labels_fc_copy=copy.deepcopy(labels_fc)
    labels=[labels_fc_copy[jj].morph(subject_from='fsaverage', smooth=5, subject_to=subjects[subject_no], subjects_dir=data_path) for jj in range(len(labels_fc))]
...
#merge_remain=merge_labels1%2
#merge_tokeep=np.abs(merge_remain[:,0]-merge_remain[:,1])
#merge_labels2=merge_labels1[merge_tokeep==0,:]
...
#        label_append[0].name=labels_merged[labels_lh_ind[int(flc/2)]].name[:-3]+'-lh'
#        #hemi_src = src_avg[0]
#    else:
#        label_append[0].name=labels_merged[labels_lh_ind[int(flc/2)]].name[:-3]+'-rh' 
#        #hemi_src = src_avg[1]
...
for cnt, meg in enumerate(ll):
    ...
    inverse_operator_eegmeg = read_inverse_operator(fname_inv)
    sub_src=inverse_operator_eegmeg['src']
    labels_fc_copy2=copy.deepcopy(labels_fc)
    labels_split=[labels_fc_copy2[jj].morph(subject_from='fsaverage', smooth=5, subject_to=subjects[subject_no], subjects_dir=data_path) for jj in range(len(labels_fc))]
    ...
    fname_subj='rev2_RG2_balanced_splitmerged_labels_prefinal'
...
path_mtx=data_path+'rev2_Matx_second.mat'
scipy.io.savemat(path_mtx,{'stc_mean_data':stc_mean_data})
Matx_zscore=np.zeros(Matx.shape)
Matx_zscore=zscore(Matx,axis=1)
assign_vertex=np.argmax(Matx_zscore, axis=1)
...
balanced_labels00c=balanced_labels00[1::2]
...
fname_blm='rev2_RG2_finalpreparc_smooth_stddiff_mergeonly_halfmax'+str(len(balanced_labelsmerge_final))
make_parcellation_mine(src_avg,balanced_labelsmerge_final,clrs[len(balanced_labelssplit_final)/2:,:],subject_avg,fname_blm)

fname_bls='rev2_RG2_finalpreparc_smooth_stddiff_splitonly_halfmax'+str(len(balanced_labelssplit_final))
make_parcellation_mine(src_avg,balanced_labelssplit_final,clrs[:len(balanced_labelssplit_final)/2,:],subject_avg,fname_bls)

fname_final='rev2_RG2_finalpostparc_smooth_stddiff_split_merge_halfmax_'+str(len(balanced_labels))
...
col_fname='rev2_colour_RG2_winnerhem_'+str(len(balanced_labels))+'.mat'
scipy.io.savemat(col_fname,{'clrs':clrs})
fname_blb='rev2_RG2_finalpreparc_smooth_stddiff_splitmerge_halfmax'+str(len(balanced_labels00_final))
