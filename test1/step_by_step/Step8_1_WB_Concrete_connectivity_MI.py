...
#        indices = seed_target_indices([seed_idx], np.arange(n_sources))
        ...
        for ccc in range(len(stcs)): #epoch count
            ...
            seed_ts = mne.extract_label_time_course(stc_orig, label1, src, mode='pca_flip')
            for actc,tc in enumerate(range(150,251,100)): #time count
                ...
                for vc in range(n_sources): #vertex count
                    MI[vc,ccc,actc]=mi(curr_sig[vc,:],seed_ts[0,tc+500:tc+500+200],bins=20)#[0,1]
...
#            stc_orig.resample(100)
        ...
        data_out = data_path + meg +  'firstMorphed_SemLoc_icaclean_equalized_Concrete_MutualInfo_PCALabel_150_450_200ms_' + label_name[0:-3]
        print data_out
