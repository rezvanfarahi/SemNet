...
import operator
from mne.minimum_norm import (read_inverse_operator, make_inverse_operator, write_inverse_operator, apply_inverse, apply_inverse_epochs,cross_talk_function, point_spread_function)
...
rat=np.zeros((17,len(labelss),len(labelss)))
rat_blnc=np.zeros((len(labelss),17))
stc_all=range(17)

label_colors = [label.color for label in labelss]
overlaps1=np.zeros((len(labelss),len(labelss),17))
# read label(s)
for lbl in labelss:
    lbl.values.fill(1.0)
...
#    print "data cov finished!"
    ...
    #stc_ctf_mne=cross_talk_function(inverse_operator_eegmeg, forward, labels,method=method, lambda2=lambda2, signed=False, mode=mode, n_svd_comp=1, verbose=None)
...
#    ff=np.intersect1d(ee,bbv)
    ...
    Matx=stc_to.data
    Matx_normalised=np.zeros(Matx.shape)
    for cntr in range(Matx.shape[1]-1):
        Matx_normalised[:,cntr]=Matx[:,cntr]/Matx[:,-1]
    gg=np.argmax(Matx_normalised, axis=1)
#    for cnt in range(jj.shape[1]):
#        jj[0][cnt]=np.where(gg==cnt)[0].shape[0]    
    best_vert=range(len(labelss))
    for iii in range(len(labelss)):
        actual_label=stc_to.in_label(labelss[iii])
        if iii%2==0:
            actual_label_vert=actual_label.vertices[0]
        else:
            actual_label_vert=actual_label.vertices[1]+len(stc_to.lh_vertno)
            
        best_vert[iii]=np.intersect1d(np.where(gg==iii)[0],actual_label_vert)
        for jjj in range(len(labelss)):
            #print iii,jjj
            intersect_shape=np.intersect1d(np.where(gg==jjj)[0],actual_label_vert).shape[0]
            overlaps1[iii,jjj,cnt]=intersect_shape/float(actual_label_vert.shape[0])
            #print (np.intersect1d(np.where(gg==jjj)[0],actual_label_vert).shape[0])/actual_label_vert.shape[0]
overlaps_avg=overlaps1.mean(axis=2)
wsb=np.argmax(overlaps_avg, axis=1)        
wmax=np.max(overlaps_avg, axis=1)  
diagov=np.diag(overlaps_avg)
wsb[wmax<diagov.mean()]=100#-diagov.std()/2.
label_new=copy.deepcopy(labelss)
for cnt1 in range(len(labelss)):
    if (cnt1%2==0 and wsb[cnt1+1]!=wsb[cnt1]+1):
        wsb[cnt1+1]=100
        wsb[cnt1]=100
           
           
for cnt2 in range(len(labelss)):
    if wsb[cnt2]!=cnt2:
        label_new[cnt2]=None
    else:
        tosum_thislabel=np.where(wsb==cnt2)[0]
        labels_tosum=[labelss[ii] for ii in tosum_thislabel]
        label_new[cnt2]=np.sum(labels_tosum)
        
newlabels_final=[label for label in label_new if label!=None]  
label_names=[label.name for label in newlabels_final]   
label_colors=[label.color for label in newlabels_final]
annot_path=data_path
mne.write_labels_to_annot(newlabels_final, subjects_dir=annot_path, subject='fsaverage', parc='03oct15_labels_old_mn_svd', hemi='both',overwrite=True, colormap='hot') 
...
#    labelss_new=range(len(labelss))#np.zeros((len(labelss),100))
#    srcin='/imaging/rf02/MNE-sample-data/subjects/fsaverage/bem/fsaverage-withdist-ico-5-src.fif'
...
#        labelss_new[kk]=ff
##        stc_label.data[:]=0.
##        stc_label.data[ff]=1.0;
##        labelss_new[kk]=mne.stc_to_label(stc_label,src='fsaverage',smooth=False, connected=False, subjects_dir=data_path)
#   
#    for kk, lblk in enumerate(labelss):
#        bb2=labelss_new[kk]#stc_to.in_label(labelss[kk])
...
#            ee2=dd2[:len(bb2)]
#            
#            ff2=np.intersect1d(ee2,bb2)
#            rat[kk,kk2,cnt]=len(ff2)/float(len(bb2))
#    #print rat[:,cnt].mean()
path_c=data_path+'aparc_18sep15_overlaps.mat'
scipy.io.savemat(path_c,{'overlaps1':overlaps1})
#rat_new=rat.copy()
