...
## epoching
    ...
    epochs_concrete1 = epochs['cncrt_wrd']
    epochs_abstract1 = epochs['abs_wrd']
    mne.epochs.equalize_epoch_counts([epochs_concrete1,epochs_abstract1], method='mintime')
    epochs_concrete=epochs_concrete1.copy().subtract_evoked()
    epochs_abstract=epochs_abstract1.copy().subtract_evoked()
    ...
    bands = dict(gamma=[31,45])#, alpha=[8,12], beta=[13, 30], gamma=[31, 45])#, beta=[13, 30], gamma=[31, 45])
    snr = ediv.mean()
    lambda2 = 1.0 / snr ** 2
    print "source_band_induced_power concrete"
    stcs_concrete = source_band_induced_power(epochs_concrete, inv_op, bands=bands, label=None, lambda2=lambda2, method='MNE', n_cycles=9, baseline=(-400,0), baseline_mode='ratio', use_fft=False)
    subject_from = subjects[subject_no]
    subject_to = 'fsaverage'
    vertices_to = [np.arange(10242), np.arange(10242)]

    for b, stc in stcs_concrete.iteritems():
	morphmat1=mne.compute_morph_matrix(subject_from, subject_to, stc.vertno, vertices_to, subjects_dir=data_path)
	stc_to=stc.morph_precomputed(subject_to,vertices_to,morphmat1, subject_from)      
	data_out = data_path + meg + 'preMorphed_SemLoc_icaclean_Concrete_induced_Power_ratio_equalized_m500_700_' + b
	stc_to.save(data_out)     

    print "source_band_induced_power abstract"
    stcs_abstract = source_band_induced_power(epochs_abstract, inv_op, bands=bands, label=None, lambda2=lambda2, method='MNE', n_cycles=9, baseline=(-400,0), baseline_mode='ratio', use_fft=False)
    
    for b, stc in stcs_abstract.iteritems():
        morphmat1=mne.compute_morph_matrix(subject_from, subject_to, stc.vertno, vertices_to, subjects_dir=data_path)
        stc_to=stc.morph_precomputed(subject_to,vertices_to,morphmat1, subject_from)
        data_out = data_path + meg + 'preMorphed_SemLoc_icaclean_Abstract_induced_Power_ratio_equalized_m500_700_' + b
        stc_to.save(data_out)
