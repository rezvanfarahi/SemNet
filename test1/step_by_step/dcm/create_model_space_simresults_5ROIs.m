...
all_DCMs={};
subj=struct();
dcm_path={'/imaging/rf02/TypLexMEG/dcm/5ROIs_hubvsfull/filtered_250ms_28models_5ROIs/semloc/simulated/inverted/model4/';%'/imaging/rf02/TypLexMEG/dcm/5ROIs_hubvsfull/filtered_250ms_28models_5ROIs/semloc/';
'/imaging/rf02/TypLexMEG/dcm/5ROIs_hubvsfull/filtered_250ms_28models_5ROIs/semloc/simulated/inverted/model3/';
'/imaging/rf02/TypLexMEG/dcm/5ROIs_hubvsfull/filtered_250ms_28models_5ROIs/semloc/simulated/inverted/model2/';
'/imaging/rf02/TypLexMEG/dcm/5ROIs_hubvsfull/filtered_250ms_28models_5ROIs/semloc/simulated/inverted/model1/'};
allFs=zeros(1,length(dcm_path));
Ffam=zeros(length(dosubs),length([2,4,39:40,6,8,41:42,10,12,43:44,14,16,45:46,18:2:36,37:38]));
grand_F=zeros(length(dosubs),length([2,4,39:40,6,8,41:42,10,12,43:44,14,16,45:46,18:2:36,37:38]),length(dcm_path));
...
for modcnt=1:length(dcm_path)
        load(sprintf([dcm_path{modcnt},'DCM_simfin_erpf_SemLoc_5ROIs_sub%d_mod%d.mat'],sub,n));%'DCM_erpf_SemLoc_5ROIs_sub%d_mod%d.mat'
       allFs(modcnt)=DCM.F;
       grand_F(s,ncnt,modcnt)=DCM.F;
end
        this_model=struct();
this_model.fname=sprintf([dcm_path{modcnt},'DCM_simfin_erpf_SemLoc_5ROIs_sub%d_mod%d.mat'],sub,n);%DCM.name;
this_model.F=allFs(2);%mean(allFs);%DCM.F;
Ffam(s,ncnt)=mean(allFs);
...
out_dir='/imaging/rf02/TypLexMEG/dcm/5ROIs_hubvsfull/filtered_250ms_28models_5ROIs/semloc/simulated/inverted/model2/';%'/home/rf02/rezvan/test1/step_by_step/dcm/latest/200ms_7models/semloc';
if ~exist(out_dir,'dir')
    mkdir(out_dir)
end
% load('/imaging/rf02/TypLexMEG/dcm/5ROIs_hubvsfull/250ms_38models_5ROIs/families_hierarchical_1st.mat')
% family.infer='FFX';
% [family,model] = spm_compare_families(grand_F(:,:,4),family);

save([out_dir,'model2_space_simfin_ATLhub_f250.mat'],'subj')
