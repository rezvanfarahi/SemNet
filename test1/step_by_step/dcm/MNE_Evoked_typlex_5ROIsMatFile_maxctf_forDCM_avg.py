...
out_path = '/imaging/rf02/TypLexMEG/dcm/maxCTF/'
...
#        fname_inv = data_path + meg + 'typlex_InvOp_ico5diagreg_fft_1_48_clean_ica_EMEG-inv.fif'#'InvOp_ico5diagreg_fft_1_48_clean_ica_EMEG-inv.fif'#
#        # Load data
...
ctfstc_path=data_path+'dcm/stc_ctf_SLm_modlabels'
stc_bothm.save(ctfstc_path)
#stc_bothm = mne.read_source_estimate(ctfstc_path) 
Matx=stc_bothm.data[:,:-1].copy()
Matxs=np.sum(Matx,1)
nsubs=len(subject_inds)
labels_tcb=np.zeros((nsubs,len(labellist),1201,2))
#Matx=Matx/np.vstack((Matxs,Matxs,Matxs,Matxs,Matxs)).T
for ii, meg in enumerate(ll):
    conabs_mat=np.zeros((5,1201,2))
    subject_no=subject_inds[ii]
    #    conind=0
    for event_no,event_name in enumerate(['word','nonword']):
    #        conind=conind+1
    #        print conind
                    # Load data
        srcin='/imaging/rf02/TypLexMEG/fsaverage/bem/fsaverage_dist-ico-5-src.fif'
        src_avg = mne.read_source_spaces(srcin)
        fname = data_path + meg + 'firstMorphed_ico_signed_typlex_ica_'+event_name+'_Source_Evoked_m500_700' 
        this_stc = mne.read_source_estimate(fname)  
        tmin1=-500
        tstep1=1
        thisstc_data=np.ones((this_stc.data.shape[0],1))
        ...
        for this_labelc, this_label in enumerate(labellist): 
            ...
            testlabel_path=data_path+'dcm/testDCMctflabs/'+'SL_subject'+str(ii)+'_'+event_name+'_'+this_label.name
...
#        cons[ii,:,:]=np.squeeze(conc-cona)
    X[ii,:,:,:]=conabs_mat
    out_file=out_path + meg[:10] + '_TypLex_SL_Evoked_5ROIs_meanflip_maxCTF_forDCM_avg.mat'
