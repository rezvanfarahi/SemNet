...
LogEvd=[]; DCMname={};
subj=struct();
dcm_path='/imaging/rf02/TypLexMEG/dcm/latest/150ms_32models_2ROIs/typlex/';%150ms_7models/semloc/';%150ms_32models_2ROIs/semloc/';
...
for s = 1:length(dosubs) %parfor
    ...
    for n=1:32%numel(Model)
        sub
        n

        load(sprintf([dcm_path,'DCM_erp_TypLex_ATL_WFA_new_sub%d_mod%d.mat'],sub,n));
       
        this_model=struct();
this_model.fname=sprintf([dcm_path,'DCM_erp_TypLex_ATL_WFA_new_sub%d_mod%d.mat'],sub,n);%DCM.name;
this_model.F=DCM.F;

this_model.Ep=DCM.Ep;
this_model.Cp=DCM.Cp;
sess.model(1,n)=this_model;
%         LogEvd(s,n) = DCM.F;
%         DCMname{s,n} = DCM.name;
    end
    subj(1,s).sess=sess;
end
save([dcm_path,'model_space.mat'],'subj')
