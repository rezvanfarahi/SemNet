# -*- coding: utf-8 -*-
"""
Created on Sun Oct 22 02:57:03 2017
...
#        iop[gainpick]=orient_prior.copy()
    ...
    eigen_leads[:,gainpick]=gain.copy()#eigen_leads_pre.copy()
    eigen_fields=np.eye(gain.shape[0])
    sing=np.ones((gain.shape[0],))
...
out_path = '/imaging/rf02/TypLexMEG/dcm/dipole/nosvd/'
...
ctfstc_path=data_path+'dcm/stc_ctf_bothm_origlabels'
...
X=np.zeros((nsubs,n_ROIs_final,1201,2))
for ii, meg in enumerate(ll):
    conabs_mat=np.zeros((n_ROIs_final,1201,2))
    subject_no=subject_inds[ii]
...
#        label_verts=np.asarray([np.where(forward['src'][0]['vertno']==lv0)[0][0] for lv0 in label_verts_pre])
        ...
        vert50=np.sort(Matx[label_verts,this_labelc])[-5]#[avgvert_select]
...
#        allwin_verts[ii,2*this_labelc+1,:]=rhlabel_verts_subm.copy()
        testlabel_path=data_path+'dcm/dipoleDCMavglabs/'+'avg_subject'+str(ii)+'_'+this_label.name
...
#    winvert_sub=np.squeeze(allwin_verts[ii,:,:]).copy()
    ...
    print vertsub_vect, len(vertsub_vect)
    info = evoked.info
...
#        print thisscale
    ...
    X[ii,:,:,:]=conabs_mat
    out_file=out_path + meg[:10] + '_SemLoc_Evoked_5ROIs_dipole_max5verts_CTF_avg.mat'#+str(np.abs(roivert))+'verts_avg.mat'
    scio.savemat(out_file,{'conabs_mat':conabs_mat})
import matplotlib.pyplot as plt
plt.close("all")
for ii in range(len(labellist[:2:-2])):
    plt.figure()#plt.subplot(1,5,ii+1) 
    plt.plot(np.linspace(-500,700,1201),np.mean(X[:,ii,:,1],0).T)
    plt.plot(np.linspace(-500,700,1201),np.mean(X[:,ii,:,0],0).T,'red')
    
for jj in range(len(labellist[:2:-2])):
    plt.figure()
    for ii in range(17):
        plt.subplot(4,5,ii+1) 
        plt.plot(np.linspace(-500,700,1201),X[ii,jj,:,1].T)
        plt.plot(np.linspace(-500,700,1201),X[ii,jj,:,0].T,'red')
#    conind=0
#    for event_no,event_name in enumerate(['Concrete','Abstract']):
##        conind=conind+1
##        print conind
