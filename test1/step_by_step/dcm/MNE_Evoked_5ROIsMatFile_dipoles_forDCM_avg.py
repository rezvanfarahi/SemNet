...
#Matx=np.mean(Matx,2)  
allwin_verts=np.zeros((n_subjects,2*n_ROIs,np.abs(roivert)))
for ii, meg in enumerate(ll):
    conabs_mat=np.zeros((5,1201,2))
    subject_no=subject_inds[ii]
...
#        label_temp_sub=thisstc_labeltest.morph(subject_from='fsaverage', subject_to=subjects[subject_no], subjects_dir=data_path) 
        allwin_verts[ii,2*this_labelc,:]=label_verts_subm.copy()
    ...
    vertsub_vect=np.reshape(winvert_sub,(2*n_ROIs*np.abs(roivert),)).astype(int)
    info = evoked.info
    print "a"
...
#    condition2.save(data_out)
    for tlc, tlab in enumerate(sub_labellist[::2]):
...
#    X[ii,:,:,:]=conabs_mat
    out_file=out_path + meg[:10] + '_SemLoc_Evoked_5ROIs_dipole_50verts_avg.mat'
