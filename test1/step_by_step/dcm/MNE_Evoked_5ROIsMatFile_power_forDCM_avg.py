...
###############################################################################
out_path = '/imaging/rf02/TypLexMEG/dcm/'#'/home/rf02/rezvan/test1/step_by_step/dcm/' # root directory for your MEG data
...
labellist=[labelatl,labelss[label_index[1]],labelss[label_index[2]],labelag,labelss[label_index[4]]]
...
sigend=sonset+int(abs(tmaxwin*1000)/sfrq)

Matx_all=np.zeros((20484,n_subjects,2))
...
#        stc_to=stc_ctf_mne.morph_precomputed(subject_to,vertices_to,morphmat1, subject_from)
        Matx_all[:,ii,event_no]=np.mean(np.abs(this_stc.data[:,sonset:sigend]),1)# Nv x Nlabels+1
        
        
Matx=np.mean(Matx_all,2)
Matx=np.mean(Matx,1)  
for ii, meg in enumerate(ll):
    conabs_mat=np.zeros((5,1201,2))
    subject_no=subject_inds[ii]
    ...
    for event_no,event_name in enumerate(['Concrete','Abstract']):
        ...
        for this_labelc, this_label in enumerate(labellist): 
            ...
            vert50=np.sort(Matx[label_verts])[-100]
            winner_verts=label_verts[np.where(Matx[label_verts]>=vert50)[0]]#label_verts[np.where(Matx[label_verts,this_labelc]>=np.max(Matx[label_verts,this_labelc])/2.)[0]]#np.where(Matx[:,this_labelc]>=np.max(Matx[:,this_labelc])/2.)[0]#
            #            print winner_verts
    ...
    X[ii,:,:,:]=conabs_mat
    out_file=out_path + meg[:10] + '_SemLoc_Evoked_5ROIs_maxpower_100verts_forDCM_avg.mat'
