...
for s = 1:length(dosubs) %parfor
   ...
   ncnt=0;
    for n=[2,4,39:40,6,8,41:42,10,12,43:44,14,16,45:46,18:2:36,37:38]%,18:2:36,37:38]%[2:2:36,37:38]%1:38%numel(Model)
        ncnt=ncnt+1;
        sub
        n
...
sess.model(1,ncnt)=this_model;
        ...
        all_DCMs{s,ncnt}=DCM;
        ncnt=ncnt+1;
        load(sprintf([dcm_path2,'/DCM_erpf_SemLoc_5ROIs_avg_maxCTF_sub%d_mod%d.mat'],sub,n));%'DCM_erpf_SemLoc_5ROIs_sub%d_mod%d.mat'
       
        this_model=struct();
this_model.fname=sprintf([dcm_path2,'/DCM_erpf_SemLoc_5ROIs_avg_maxCTF_sub%d_mod%d.mat'],sub,n);%DCM.name;DCM_erpf_SemLoc_5ROIs_mCTFavg_sub14_mod38
this_model.F=DCM.F;
...
save([out_dir,'/modelspace_ERPCMC_maxCTF_all_f250.mat'],'subj')
cd(dcm_path)
