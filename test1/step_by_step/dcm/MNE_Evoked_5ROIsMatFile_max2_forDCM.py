...
#            clw[cntw]=bl1[ml1]; alw[cntw]=bl1[ml1]
        ...
        cncrt_mat[cntw,:]=thislabelsign*np.expand_dims(cl[cntw][clw[cntw],:].transpose(),0)
        abs_mat[cntw,:]=thislabelsign*np.expand_dims(al[cntw][alw[cntw],:].transpose(),0)
        vertices_test = [np.arange(10242), np.arange(10242)]
        this_label_temp=copy.deepcopy(labellist[cntw])
        if this_label_temp.hemi=='lh':
            label_verts=stc_alla.in_label(this_label_temp).lh_vertno
        else:
            label_verts=stc_alla.in_label(this_label_temp).rh_vertno+len(vertices_test[0])

        thisstc_testdata=np.zeros((stc_alla.data.shape[0],1))
        ...
        testlabel_path=data_path+'dcm/testDCMmaxlabs/'+'subject'+str(ii)+'_'+labellist[cntw].name
        thisstc_labeltest.save(testlabel_path)
    labels_tca[ii,:,:]=abs_mat
    labels_tcc[ii,:,:]=cncrt_mat
...
for ii in range(len(labellist)):
    ...
    plt.plot(np.linspace(0,250,251),np.mean(labels_tca[:,ii,500:751],0).T)
    plt.plot(np.linspace(0,250,251),np.mean(labels_tcc[:,ii,500:751],0).T,'red')
    
for jj in range(len(labellist)):
    plt.figure()
    for ii in range(17):
        plt.subplot(4,5,ii+1) 
        plt.plot(np.linspace(-200,250,451),labels_tca[ii,jj,300:751].T)
        plt.plot(np.linspace(-200,250,451),labels_tcc[ii,jj,300:751].T,'red')
