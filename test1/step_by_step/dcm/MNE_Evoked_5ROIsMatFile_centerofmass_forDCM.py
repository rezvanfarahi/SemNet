...
###############################################################################
out_path = '/imaging/rf02/TypLexMEG/dcm/centre_of_mass/' # root directory for your MEG data
...
#            clw[cntw]=bl1[ml1]; alw[cntw]=bl1[ml1]
        ll1c=labellist[cntw].center_of_mass(subject='fsaverage',subjects_dir=data_path)
#        ws=np.where(labellist[cntw].vertices==ll1c)[0]
        ln=mne.grow_labels(subject='fsaverage',seeds=ll1c,extents=5.0,hemis=0)
        lda=stc_alla[ii].in_label(ln[0]).data.copy()
        ldc=stc_allc[ii].in_label(ln[0]).data.copy()
        ldbw=np.argmax(np.mean(((np.abs(lda[:,500:1000])+np.abs(ldc[:,500:1000]))/2.),1))
        thissignala=lda[ldbw,:].copy()
        
#        ldcw=np.argmax(np.mean(np.abs(ldc[:,500:1000]),1))
        thissignalc=ldc[ldbw,:].copy()
        thislabelsign=mne.label.label_sign_flip(ln[0],src)[ldbw]
        cncrt_mat[cntw,:]=thissignalc.copy()#thislabelsign*np.expand_dims(thissignalc.transpose(),0)
        abs_mat[cntw,:]=thissignala.copy()#thislabelsign*np.expand_dims(thissignala.transpose(),0)

#        ld=stc_alla[ii].in_label(ln[0])
#        print ld.data.shape[0]
        print ln
#        thissignala=mne.extract_label_time_course(stc_alla[ii],ln[0],src_avg,'mean_flip')#stc_alla[ii].in_label(ln).data
#        thissignalc=mne.extract_label_time_course(stc_allc[ii],ln[0],src_avg,'mean_flip')#stc_allc[ii].in_label(ln).data
##        clw[cntw]=blw[cntw].copy(); alw[cntw]=blw[cntw].copy()
##        thislabelsign=mne.label.label_sign_flip(labellist[cntw],src)[alw[cntw]]
#        cncrt_mat[cntw,:]=thissignala.copy()#thislabelsign*np.expand_dims(thissignalc[clw[cntw],:].transpose(),0)
#        abs_mat[cntw,:]=thissignalc.copy()#thislabelsign*np.expand_dims(thissignala[alw[cntw],:].transpose(),0)
...
#        thisstc_tolabel_test = mne.SourceEstimate(thisstc_testdata, vertices=vertices_test,tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject='fsaverage')
        thisstc_labeltest=ln[0].copy()##mne.stc_to_label(thisstc_tolabel_test, src_avg,smooth=False)[0]
        testlabel_path=data_path+'dcm/testDCMcmasslabs/'+'subject'+str(ii)+'_'+labellist[cntw].name
...
#    abs_mat=np.concatenate((abs_label1,abs_label2),0)       
    ...
    out_file=out_path + meg[:10] + '_SemLoc_Evoked_5ROIs_cmass_forDCM.mat'
...
for ii in range(len(labellist)):
    plt.figure()#plt.subplot(1,5,ii+1) 
    plt.plot(np.linspace(-500,700,1201),labels_tca[0,ii,:].T)
    plt.plot(np.linspace(-500,700,1201),labels_tcc[0,ii,:].T,'red')
