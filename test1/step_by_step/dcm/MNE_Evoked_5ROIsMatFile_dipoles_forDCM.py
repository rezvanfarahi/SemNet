...
labellist=[labelatl,labelifg,labelss[label_index[2]],labelag,labelss[label_index[4]]]
...
Matx_all=np.zeros((20484,5,n_subjects,2))
stc_con=list()
stc_abs=list()
...
#    vertices_sub=[np.arange(forward['src'][0]['nuse']),np.arange(forward['src'][1]['nuse'])]
    ...
    Matx=stc_bothm_sub.data[:,:-1].copy()
    Matxs=np.sum(Matx,1)
    ...
    src_sub=forward['src']
    print "select 100 most sensitive labels"
    sub_labellist=range(len(labellist))     
    for this_labelc, this_label in enumerate(labellist):
        this_label_temp=copy.deepcopy(this_label)
        label_temp_sub=this_label_temp.morph(subject_from='fsaverage', subject_to=subjects[subject_no], subjects_dir=data_path) 

        label_verts_pre=stc_bothm_sub.in_label(label_temp_sub).lh_vertno
        ...
        winner_verts=label_verts[np.where(Matx[label_verts,this_labelc]>=vert50)[0]][:np.abs(roivert)]#label_verts[np.where(Matx[label_verts,this_labelc]>=np.max(Matx[label_verts,this_labelc])/2.)[0]]#np.where(Matx[:,this_labelc]>=np.max(Matx[:,this_labelc])/2.)[0]#
        allwin_verts[ii,this_labelc,:]=winner_verts.copy()
        thisstc_testdata=np.zeros((len(vertices_sub[0])+len(vertices_sub[1]),1))
        thisstc_testdata[winner_verts,0]=1
        thisstc_tolabel_test = mne.SourceEstimate(thisstc_testdata, vertices=vertices_sub,tmin=1e-3 * tmin1, tstep=1e-3 * tstep1, subject=subjects[subject_no])
        thisstc_labeltest=mne.stc_to_label(thisstc_tolabel_test, forward['src'],smooth=False)[0]

        testlabel_path=data_path+'dcm/dipoleDCMsublabs/'+'subject'+str(ii)+'_'+this_label.name
        thisstc_labeltest.save(testlabel_path)
        sub_labellist[this_labelc]=thisstc_labeltest.copy()
    winvert_sub=np.squeeze(allwin_verts[ii,:,:]).copy()
    vertsub_vect=np.reshape(winvert_sub,(n_ROIs*np.abs(roivert),)).astype(int)
    info = evoked.info
    print "a"
...
#    condition2.save(data_out)
    for tlc, tlab in enumerate(sub_labellist):
...
#    X[ii,:,:,:]=conabs_mat
    out_file=out_path + meg[:10] + '_SemLoc_Evoked_5ROIs_dipole_50verts.mat'
