...
print "ll:"
print ll
Dmat_cncrt=np.zeros((17,46,4,10))
Dmat_abs=np.zeros((17,46,4,10))
Dmat_sub=np.zeros((17,46,4,10))
...
for cnt, meg in enumerate(ll):
    ...
    path_c=data_path+ meg+'degrees_proportional_15_pca_ppc_concrete_resampled_10win'
    ...
    path_a=data_path+ meg+'degrees_proportional_15_pca_ppc_abstract_resampled_10win'
...
#                dchunck=Dmat_cncrt[:,ii:ii+1,jj,b1:b2].mean(axis=2)-Dmat_abs[:,ii:ii+1,jj,b1:b2].mean(axis=2)#;dchunck=dchunck.reshape(dchunck.shape[0]*dchunck.shape[2],dchunck.shape[1])
                dchunck=Dmat_cncrt[:,ii:ii+1,jj,2*kk+1]-Dmat_abs[:,ii:ii+1,jj,2*kk+1]#;dchunck=dchunck.reshape(dchunck.shape[0]*dchunck.shape[2],dchunck.shape[1])
...
#path_MTn=data_path+ graphpath+'perN1024single_ppc_degree_15_resampled_10win_50ms_bothcon2_hubs_prob.mat'
#scipy.io.savemat(path_MTn,{'MTmaxN':MTmaxN}) 
##ltc=scipy.io.loadmat(path_MTn,mat_dtype=True); MTmaxN=ltc['MTmaxN'] 
#path_MTp=data_path+ graphpath+'perP1024single_ppc_degree_15_resampled_10win_50ms_bothcon2_hubs_prob.mat'
...
#    
    out_file_Xs=out_path  + 'Perttest_10logPval_firstMorphed_SemLoc_icaclean_hubs_proportional_15_pca_ppc_subtract_meanresampled_50_550_100ms_0overlap_'+band
...
#out_path='/imaging/rf02/TypLexMEG/icaanalysis_results/jpg/time_resolved/ROI_connectivity/graph/ppc_degforpcaf_hubs_meanz.mat';
