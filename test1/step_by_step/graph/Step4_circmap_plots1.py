# -*- coding: utf-8 -*-
"""
Created on Fri Jun 26 15:31:38 2015
...
print "ll:"
print ll
Dmat_cncrt=np.zeros((46,46,4,2,17))
Dmat_abs=np.zeros((46,46,4,2,17))
Dmat_both=np.zeros((46,46,4,2,17))

Dmat_c=np.zeros((46,46,4,2,17))
Dmat_a=np.zeros((46,46,4,2,17))
Dmat_b=np.zeros((46,46,4,2,17))
MTc=np.zeros((46,46,4,2))
MTa=np.zeros((46,46,4,2))
MTb=np.zeros((46,46,4,2))
...
#    label_ts_abstract = mne.extract_label_time_course(stcs_abstract, labels[:-2], src, mode='pca_flip',return_generator=False)
    path_c=data_path+ meg+'thresholded_15_pca_ppc_concrete_resampled_2win.mat'
    ...
    path_a=data_path+ meg+'thresholded_15_pca_ppc_abstract_resampled_2win.mat'
    ...
    path_b=data_path+ meg+'thresholded_15_pca_ppc_both_resampled_2win.mat'
    #scipy.io.savemat(path_a,{'label_ts_abstract':label_ts_abstract})
...
path_bf=data_path+ graphpath+'ppc_degrees_15_resampled_2win_bothcon2_hubs_prob_ff.mat'
...
path_bf=data_path+ graphpath+'ppc_degrees_prob_15_resampled_2win_bothcon2_hubs_ff.mat'
ltc=scipy.io.loadmat(path_bf,mat_dtype=True); deg_bzmz=ltc['deg_p']
...
deg_finaldr=np.round(deg_finald)
Dmatb=Dmat_b.mean(axis=4)
#Dmatb=Dmatb[:,:,:,1::2]

Dmatbz=np.zeros(Dmatb.shape)
Dz=np.zeros(Dmatb.shape)
for ii in range(Dmatb.shape[1]):
    for jj in range(Dmatb.shape[2]):
        for kk in range(Dmatb.shape[3]):
            if deg_finaldr[ii,jj,kk]>0:
                sigs=Dmatb[:,ii,jj,kk]
                sigs2=Dz[:,ii,jj,kk]
                sigs2[sigs.argsort()[::-1][:deg_finaldr[ii,jj,kk]]]=deg_bzmz[ii,jj,kk]
                Dmatbz[:,ii,jj,kk]=sigs2
            
for ii in range(Dmatbz.shape[0]):
    for jj in range(Dmatbz.shape[1]):
        for kk in range(Dmatbz.shape[2]):
            for mm in range(Dmatbz.shape[3]):
                if Dmatbz[ii,jj,kk,mm]>Dmatbz[jj,ii,kk,mm]:
                    Dmatbz[jj,ii,kk,mm]=Dmatbz[ii,jj,kk,mm]
...
path_bf='/imaging/rf02/TypLexMEG/icaanalysis_results/jpg/time_resolved/ROI_connectivity/graph/ppc_degrees_15_resampled_2win_subtract_t_orig.mat'

ltc=scipy.io.loadmat(path_bf,mat_dtype=True); Dmatbz=ltc['torig'] 
Dmatbz[:41,:,:,:]=0; 
Dmatbz[42:,:,:,:]=0;
Dmatbz[Dmatbz>-2.1]=0               
        
for ii,trange in zip(range(Dmatbz.shape[3]),['150-350ms', '250-450ms']):#['50-150ms','150-250ms','250-350ms','350-450ms','450-550ms']):#time Dmatc.shape[2]):
    for jj,bands in zip(range(Dmatbz.shape[2]),['Theta','Alpha','Beta','Gamma']):#freq bands
        label_names1=copy.deepcopy(l_new)
        for kk in range(len(label_names)):
#            dthr=np.mean(deg_bzmz[deg_bzmz>0])+np.std(deg_bzmz[deg_bzmz>0])
            dthr=np.mean(deg_bzmz)+np.std(deg_bzmz)
            if deg_bzmz[kk,jj,ii]>=dthr:
                posl=''
            else:
                posl=''
...
#                    label_names1[kk]=l_new[kk].replace(l_new[kk].encode(),l_new[kk].encode())
        ...
        plot_connectivity_circle(Dmatbz[:,:,jj,ii],node_names=label_names1, node_angles=node_angles, node_colors=label_colors, vmin=-0.1,vmax=0.1,  colormap=cmap, colorbar=False,colorbar_pos=(-3.3,0.06),colorbar_size=0.15,fontsize_names=15,fontsize_colorbar=5,facecolor='black',textcolor='white', node_linewidth=2.5)#facecolor='white',textcolor='black',#######subplot=(5, 4, ii*4 + jj + 1),,####Dmatc[Dmatc[:,:,ii,2].nonzero()[0],Dmatc[:,:,ii,2].nonzero()[1],ii,2], indices=Dmatc[:,:,ii,2].nonzero(),padding=0, fontsize_colorbar=10, fig=fig,title='GrandAverage ppc Concrete-Abstract '+bands+' '+trange, 
        out_name=out_path+'ppc_15_resampled_2win_subtract_lSTL_'+bands+'_'+trange+'.jpg'
...
db[:41,:,:]=0
db[42:,:,:]=0
plot_connectivity_circle(db[:,:,3,1],node_names=l_new, node_angles=node_angles, node_colors=label_colors, vmin=0,vmax=0.5,  colormap='hot', colorbar=True,colorbar_pos=(-3.3,0.06),colorbar_size=0.15,fontsize_names=15,fontsize_colorbar=5,facecolor='black',textcolor='white', node_linewidth=2.5)#facecolor='white',textcolor='black',#######subplot=(5, 4, ii*4 + jj + 1),,####Dmatc[Dmatc[:,:,ii,2].nonzero()[0],Dmatc[:,:,ii,2].nonzero()[1],ii,2], indices=Dmatc[:,:,ii,2].nonzero(),padding=0, fontsize_colorbar=10, fig=fig,title='GrandAverage ppc Concrete-Abstract '+bands+' '+trange, 
out_name=out_path+'ppc_15_resampled_2win_bothcon_rSTL_gamma_250_450ms_prob.jpg'
plt.savefig(out_name, facecolor='black',format='jpg',dpi=150)#,tight_layout=True)
plt.close()
