...
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/stc/permutation/connectivity/WB_hubs/spectral/newag/'    # where event files are
...
#p_list=p_list=[0.001,0.0025,0.005,0.008,0.01,0.025,0.05]#,0.008,0.01,0.025,0.05]#[0.005,0.01,0.025, 0.05]#,0.045,0.04,0.03,0.025,0.02,0.01,0.008,0.005,0.002,0.001,0.0005,0.0001]
...
labellistm=['ATLl','AGl','pIFGl','MTLl']#labellist = ['lh.ATL_latmed','lh.supramarginal','lh.posterior_inferiorfrontal','lh.middle_temporal']#labellist = [ 'lh.posterior_fusiform','rh.posterior_fusiform', 'lh.lateraloccipital','rh.lateraloccipital','lh.precentral','rh.precentral']#, 'lh.ATLsmall','rh.ATLsmall']#labellist = [ 'leftATL-lh','rightATL-rh']#, 'medialtempright-rh','medialtempleft-lh']'lh.postcentral','rh.postcentral',
...
for p_threshold in ll:
    ...
    for label_name,lblname in zip(labellist,labellistm):
        ...
        for con_method in conn_methods:
            ...
            ### Concrete
            		fname_cncrt = data_path + meg + 'Morphed_ico_SemLoc_ica_equalized_Concrete_Mlttap_new_ctf_'+con_method+'_'+band+'_150_450_200ms_' + label_name[0:-3] 
            		stc_cncrt = mne.read_source_estimate(fname_cncrt)#; stc_cncrt.data[stc_cncrt.data<0]=0 
            		fname_abs = data_path + meg + 'Morphed_ico_SemLoc_ica_equalized_Abstract_Mlttap_new_ctf_'+con_method+'_'+band+'_150_450_200ms_' + label_name[0:-3]   
...
#            	print('Computing connectivity.')
            	...
            	good_cluster_indst = np.where(cluster_p_valuest < 0.05)[0]#good_cluster_indst = np.where(clus_p_values <= 0.003125)[0]
            	...
            	if cluster_p_valuest.min()<0.05:#clus_p_values.min()<=0.003125:
                    ...
                    out_file1=out_path + 'ClusP_Morphed_ico_SL_ica_eq_Sub_Mlttap_new_ctf_'+con_method+'_'+band+'_150_450_200ms_ms' + str(max_step) +'_'+ str(p_threshold)[2:5] + '_' + str(cluster_p_valuest.min())[2:5]+ '_' + lblname
                    ...
                    out_file2=out_path + 'ClusP_sw_Morphed_ico_SL_ica_eq_Sub_Mlttap_new_ctf_'+con_method+'_'+band+'_150_450_200ms_ms' + str(max_step) +'_'+ str(p_threshold)[2:5] + '_' + str(cluster_p_valuest.min())[2:5]+ '_' + lblname
