...
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/stc/permutation/connectivity/WB_hubs/temporal/'    # where event files are
...
bands=['Alpha']#['Theta','Alpha','Beta','Gamma']
...
#            	X=np.zeros((20484,2,17))
            ...
            ### Concrete
            		fname_cncrt = data_path + meg + 'Morphed_ico_SemLoc_ica_equalized_Concrete_bestV_ctf_MI_'+band+'_150_450_200ms_' + label_name[0:-3] 
#            		fname_cncrt = data_path + meg + 'Morphed_ico_SemLoc_ica_equalized_Concrete_bestV_ctf_MI_150_450_200ms_' + label_name[0:-3] 
            		stc_cncrt_mi = mne.read_source_estimate(fname_cncrt)#; stc_cncrt.data[stc_cncrt.data<0]=0 
            		fname_cncrt = data_path + meg + 'Morphed_ico_SemLoc_ica_equalized_Concrete_Mlttap_bestV_ctf_WPLI2d_'+band+'_150_450_200ms_' + label_name[0:-3] 
            		stc_cncrt_wpli2d = mne.read_source_estimate(fname_cncrt)
            		fname_cncrt = data_path + meg + 'Morphed_ico_SemLoc_ica_equalized_Concrete_Mlttap_bestV_ctf_Coherence_'+band+'_150_450_200ms_' + label_name[0:-3] 
            		stc_cncrt_coh = mne.read_source_estimate(fname_cncrt)
              
              
            		fname_abs = data_path + meg + 'Morphed_ico_SemLoc_ica_equalized_Abstract_bestV_ctf_MI_'+band+'_150_450_200ms_' + label_name[0:-3]   
#            		fname_abs = data_path + meg + 'Morphed_ico_SemLoc_ica_equalized_Abstract_bestV_ctf_MI_150_450_200ms_' + label_name[0:-3]   
            		stc_abs_mi = mne.read_source_estimate(fname_abs)#; stc_abs.data[stc_abs.data<0]=0 	
            		fname_abs = data_path + meg + 'Morphed_ico_SemLoc_ica_equalized_Abstract_Mlttap_bestV_ctf_WPLI2d_'+band+'_150_450_200ms_' + label_name[0:-3] 
            		stc_abs_wpli2d = mne.read_source_estimate(fname_abs)
            		fname_abs = data_path + meg + 'Morphed_ico_SemLoc_ica_equalized_Abstract_Mlttap_bestV_ctf_Coherence_'+band+'_150_450_200ms_' + label_name[0:-3] 
            		stc_abs_coh = mne.read_source_estimate(fname_abs)
            		data_cncrt_norm = (np.abs(stc_cncrt_mi.data)/np.abs(stc_cncrt_mi.data).max(axis=0)+np.abs(stc_cncrt_wpli2d.data)/np.abs(stc_cncrt_wpli2d.data).max(axis=0)+np.abs(stc_cncrt_coh.data)/np.abs(stc_cncrt_coh.data).max(axis=0))/3.0
...
#            		X[:,:,ii]=stc_data_subtract
            		Xc[:,:,:,ii]=np.dstack((np.abs(stc_cncrt_mi.data)/np.abs(stc_cncrt_mi.data).max(axis=0),np.abs(stc_cncrt_wpli2d.data)/np.abs(stc_cncrt_wpli2d.data).max(axis=0),np.abs(stc_cncrt_coh.data)/np.abs(stc_cncrt_coh.data).max(axis=0)))
            		Xa[:,:,:,ii]=np.dstack((np.abs(stc_abs_mi.data)/np.abs(stc_abs_mi.data).max(axis=0),np.abs(stc_abs_wpli2d.data)/np.abs(stc_abs_wpli2d.data).max(axis=0),np.abs(stc_abs_coh.data)/np.abs(stc_abs_coh.data).max(axis=0)))
            	...
            	T_obst, clusterst, cluster_p_valuest, H0t = clut = spatio_temporal_cluster_1samp_test(X, connectivity=connectivity,  n_permutations=n_permutations, n_jobs=4, threshold=t_threshold, tail=0,t_power=1, spatial_exclude=spatial_exclude,step_down_p=0.05)
            	...
            	if cluster_p_valuest.min()<0.05:
                    ...
                    stc_all_cluster_vist = summarize_clusters_stc(clut, tstep=tstep, p_thresh=p_thresh, vertices=fsave_vertices, subject='fsaverage')
                    out_file1=out_path + 'ClusP_Morphed_ico_SL_ica_equal_Sub_bestV_ctf_STmean_'+band+'_150_450_200ms_ms' + str(max_step) +'_'+ str(p_threshold)[2:5] + '_sx_' + str(p_thresh)[2:5]+ '_' + label_name_sv[0:-3]
                    ...
                    out_file2=out_path + 'ClusP_sw_Morphed_ico_SL_ica_equal_Sub_bestV_ctf_STmean_'+band+'_150_450_200ms_ms' + str(max_step) +'_sx_'+ str(p_threshold)[2:5] + '_' + str(p_thresh)[2:5]+ '_' + label_name_sv[0:-3]
