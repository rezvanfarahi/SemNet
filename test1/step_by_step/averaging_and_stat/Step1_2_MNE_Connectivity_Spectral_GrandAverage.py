...
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/stc/GrandAverage/connectivity/WB_hubs/spectral/'    # where event files are
...
for label_name in labellist:
    ...
    for con_method in conn_methods:
        ...
        #    	X_gamma=np.zeros((20484,17))
            ...
            ### Concrete
            		fname_cncrt = data_path + meg + 'Morphed_SemLoc_ica_equalized_Concrete_Mlttap_PCA_ctf_'+con_method+'_'+band+'_150_450_200ms_' + label_name[0:-3] 
              
            		stc_cncrt = mne.read_source_estimate(fname_cncrt); stc_cncrt.data[stc_cncrt.data<0]=0                      
            		fname_abs = data_path + meg + 'Morphed_SemLoc_ica_equalized_Abstract_Mlttap_PCA_ctf_'+con_method+'_'+band+'_150_450_200ms_' + label_name[0:-3]  
             ...
             #X = np.transpose(X, [2, 1, 0])	
            	X_stc = mne.SourceEstimate(X, vertices=stc_abs.vertices, tmin= 0.15, tstep= 0.1, subject='fsaverage')
            	data_out = out_path +  'Grandaverage_Morphed_SemLoc_ica_equalized_Subtract_Mlttap_PCA_ctf_'+con_method+'_'+band+'_150_450_200ms_' + label_name[0:-3]
            	X_stc.save(data_out)
