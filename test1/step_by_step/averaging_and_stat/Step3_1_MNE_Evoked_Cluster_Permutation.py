...
###############################################################################
# Set parameters
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/stc/permutation/evoked/final_jan17/' # root
...
p_list=[0.00005,0.045,0.04,0.03,0.025,0.02,0.01]#,0.008,0.005,0.002,0.001, 0.0008,0.0005,0.0002,0.0001,0.00005,0.00001]#0.001,0.0009,0.0008,0.0007,0.0006,0.0005]#
print "subject_inds:"
print subject_inds
print "No rejection"
...
stc_c=range(n_subjects)
stc_a=range(n_subjects)

X=np.zeros((20484,5, 17))
for p_threshold in ll:
    for ii, meg in enumerate(list_all):
    	print ii
    	fname_cncrt = data_path + meg + 'firstMorphed_ico_SemLoc_ica_Concrete_Source_Evoked_m500_700'
    	stc_cncrt = mne.read_source_estimate(fname_cncrt); stc_c[ii]=copy.deepcopy(stc_cncrt)
    	#abslt_cncrt= np.absolute(stc_cncrt.copy().crop(.3,.4).data)
    	fname_abs = data_path + meg + 'firstMorphed_ico_SemLoc_ica_Abstract_Source_Evoked_m500_700'
    	stc_abs = mne.read_source_estimate(fname_abs); stc_a[ii]=copy.deepcopy(stc_abs)
    ...
    tail=0
    max_step=5;
    T_obs, clusters, cluster_p_values, H0 = clu = spatio_temporal_cluster_1samp_test(X, connectivity=connectivity,  threshold=t_threshold, tail=tail, t_power=1, step_down_p=0.05, spatial_exclude=spatial_exclude,n_permutations=n_permutations, n_jobs=4)#, max_step=0)#, max_step=0)#spatial_exclude=exclude_ver, 
    ...
    out_file1=out_path + 'ClusPer_abs_icomorphed_clusterp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_17subj_SemLoc_ica_ConcreteAbstract_tail'+str(tail)+'_maxstep'+str(max_step)
    ...
    out_file2=out_path + 'ClusPer_abs_sw_icomorphed_clusterp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_17subj_SemLoc_ica_ConcreteAbstract_tail'+str(tail)+'_maxstep'+str(max_step)
...
sdata=stcam.data
sma=np.mean(sdata,0)
plt.figure(), plt.plot(t,smc[300:1000],'red',linewidth=4)
plt.plot(t,sma[300:1000],'blue',linewidth=4)
plt.xlabel('Time (ms)',fontsize=16)
plt.ylabel('Abs of Mean Source Estimated Evoked (A)',fontsize=16)
