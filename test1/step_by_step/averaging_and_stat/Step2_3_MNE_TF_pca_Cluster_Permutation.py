...
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/stc/permutation/power/pca5/' #
...
b='alpha'#NOTE!! doesn't matter. stc used for vert positions only
for p_threshold in ll:
    for which_pc in range(4):
        ...
        ### word
        	...
        	fname_cncrt = data_path + meg + 'mineMorphed_ico_SemLoc_ica_Concrete_Power_normori_ratio_equalized_m500_700_200hz_' +b#'mineMorphed_ico_TypLex_icaclean_word_Power_ratio_equalized_m500_700_' + b
        	stc_cncrt = mne.read_source_estimate(fname_cncrt)
        	#abslt_cncrt= np.absolute(stc_cncrt.copy().crop(.3,.4).data)
    #    	fname_abs = data_path + meg + 'mineMorphed_ico_TypLex_ica_nonword_Power_normori_ratio_equalized_m500_700_200hz_' +b#'mineMorphed_ico_TypLex_icaclean_nonword_Power_ratio_equalized_m500_700_' + b
    ...
    #    		X[:,cc,ii]=Xc1-Xa1#np.log(np.divide(Xc,Xa))#
    ...
    #    path_mtx=data_path+'icaanalysis_results/tf_pca/WB_PCA_acrossverts_SL_allbands_Xcscore.mat'
    #    Xcscore=scipy.io.loadmat(path_mtx,mat_dtype=True); Xc_score=np.squeeze(Xcscore['Xc_score'])
    #    path_mtx=data_path+'icaanalysis_results/tf_pca/WB_PCA_acrossverts_SL_allbands_Xascore.mat'
    ...
    #    X=np.squeeze(np.subtract(Xc_score[:,:,:,which_pc],Xa_score[:,:,:,which_pc]));
        path_mtx=data_path+'icaanalysis_results/tf_pca/WB_PCA_acrossverts_SL_allbands_Xbscore.mat'
        Xbscore=scipy.io.loadmat(path_mtx,mat_dtype=True); Xb_score=np.squeeze(Xbscore['Xb_score'])
        X=np.squeeze(np.subtract(np.abs(Xb_score[:20484,:,:,which_pc]),np.abs(Xb_score[20484:,:,:,which_pc])));	       
        ...
        #p_threshold = 0.05
        n_subjects=17
        t_threshold = -scistats.distributions.t.ppf(p_threshold/2., n_subjects - 1)
        print('Clustering.')
        max_step=1
    ...
    #    print cluster_p_values.min()
        ...
        if cluster_p_values.min()<=0.05:#clus_p_values.min()<=0.01 and cluster_p_values.min()<=0.1:
            ...
            out_file1=out_path + 'Per_clusp'+str(p_threshold)[2:]+'_p'+str(cluster_p_values.min())[2:]+'_SL_ica_Subtract_Power_ratio_ico_normori_eq_50_550_100ms_sx_ms' + str(max_step) +'_pc'+str(which_pc+1)
            ...
            out_file2=out_path + 'Per_sw_clusp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_SL_ica_Subtract_Power_ratio_normori_eq_50_550_100ms_sx_ms' + str(max_step) +'_pc'+str(which_pc+1)
