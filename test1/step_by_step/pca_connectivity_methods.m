...
%% over verts- bands
connms={'mi','ppc','coh'};%'mi',
exp_mtx_c=nan(17,2,length(connms),length(seeds));
C_hmuch_c=nan(17,2,length(connms),length(seeds));
C_which_c=nan(17,2,length(connms),length(seeds));
exp_mtx_a=nan(17,2,length(connms),length(seeds));
C_hmuch_a=nan(17,2,length(connms),length(seeds));
C_which_a=nan(17,2,length(connms),length(seeds));
exp_mtx_s=nan(17,2,length(connms),length(seeds));
C_hmuch_s=nan(17,2,length(connms),length(seeds));
C_which_s=nan(17,2,length(connms),length(seeds));
exp_mtx_b=nan(17,2,length(connms),length(seeds));
C_hmuch_b=nan(17,2,length(connms),length(seeds));
C_which_b=nan(17,2,length(connms),length(seeds));
...
%         sub_no
        for time_no=1:ntimes
            ...
            for conn_no=1:nconns
                %concrete
                X1=squeeze(Xc_grand(:,time_no,conn_no,sub_no,:));%-Xa(:,time_no,:,sub_no));
                X1o=squeeze(Xc_grando(:,time_no,conn_no,sub_no,:));
                X=X1;%-repmat(mean(X1),size(X1,1),1);
                [coeff,score,latent,tsquared,explained,mu]=pca(X,'Centered',true);%,'VariableWeights','variance','Centered',true);
...
%                 [C_hmuch_c(sub_no,time_no,conn_no,seed_no),C_which_c(sub_no,time_no,conn_no,seed_no)]=max(corr(X,score(:,which_pc)));
                ...
                %abstract
                X1=squeeze(Xa_grand(:,time_no,conn_no,sub_no,:));
                X1o=squeeze(Xa_grando(:,time_no,conn_no,sub_no,:));
                X=X1;%-repmat(mean(X1),size(X1,1),1);
                [coeff,score,latent,tsquared,explained,mu]=pca(X,'Centered',true);%,'VariableWeights','variance','Centered',true);
...
%                 [C_hmuch_a(sub_no,time_no,conn_no,seed_no),C_which_a(sub_no,time_no,conn_no,seed_no)]=max(corr(X,score(:,which_pc)));
                ...
                %subtract
                X1=squeeze(Xs_grand(:,time_no,conn_no,sub_no,:));
                X1o=squeeze(Xs_grando(:,time_no,conn_no,sub_no,:));
                X=X1;%-repmat(mean(X1),size(X1,1),1);
                [coeff,score,latent,tsquared,explained,mu]=pca(X,'Centered',true);%,'VariableWeights','variance','Centered',true);
...
%                 [C_hmuch_s(sub_no,time_no,conn_no,seed_no),C_which_s(sub_no,time_no,conn_no,seed_no)]=max(corr(X,score(:,which_pc)));
                ...
                %pooled both
                X1=squeeze(Xb_grand(:,time_no,conn_no,sub_no,:));
                X1o=squeeze(Xb_grando(:,time_no,conn_no,sub_no,:));
                X=X1;%-repmat(mean(X1),size(X1,1),1);
                [coeff,score,latent,tsquared,explained,mu]=pca(X,'Centered',true);%,'VariableWeights','variance','Centered',true);
