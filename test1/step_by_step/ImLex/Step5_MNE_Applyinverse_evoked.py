...
inv_fname = 'imlex_InvOp_ico5diagreg_fft_1_48_clean_ica_EMEG-inv.fif'#'InverseOperator_fft_1_48_clean_ica_EMEG-inv.fif'
...
for ii, meg in enumerate(ll):
    ...
    raw_fname = data_path + meg + 'imlex_pw_ssstf_fft_1_48_clean_ica_raw.fif' #clean_ssp_
    event_fname = event_path + meg + 'imlex_pw_raw_ssstnew.txt'
    subjects_dir = data_path 
    events = mne.read_events(event_fname)
    if subject_inds[ii] < 18:
	    	word_ids=np.array([1]); # print events[events[:,2]==1,2].shape+events[events[:,2]==2,2].shape
	    	nonword_ids=np.array([2]); # print events[events[:,2]==5,2].shape+events[events[:,2]==6,2].shape
	    	...
	    	event_id = {'LIm': 601, 'HIm': 602}

    else:
	    	word_ids=np.array([7]); #word_ids=np.array([8,7]); print events[events[:,2]==7,2].shape+events[events[:,2]==8,2].shape
	    	nonword_ids=np.array([8]); #nonword_ids=np.array([6,5]); print events[events[:,2]==5,2].shape+events[events[:,2]==6,2].shape
	    	events=mne.merge_events(events,word_ids,601,replace_events=True)
	    	events=mne.merge_events(events,nonword_ids,602,replace_events=True)
	    	event_id = {'HIm': 601, 'LIm': 602}
    ...
    event_id = 601  # word
    if subject_inds[ii] in np.array([3,4,13]):
...
#    ediv=np.sqrt(edv)
    ...
    #evoked1.resample(50)
    condition1 = apply_inverse(evoked1, inverse_operator, lambda2, method)#,pick_ori="normal")
    
    evoked2 = epochs2.average()
    #evoked2 = whiten_evoked(evoked21, noise_cov)
    #evoked2.resample(50)
    condition2 = apply_inverse(evoked2, inverse_operator, lambda2, method)#,pick_ori="normal")
...
# 
#    data_out = data_path + meg + 'firstMorphed_ico_imlex_ica_word_Source_Evoked_m500_700'
#    stc1.save(data_out)
#    data_out = data_path + meg + 'firstMorphed_ico_imlex_ica_nonword_Source_Evoked_m500_700'
#    stc2.save(data_out)
