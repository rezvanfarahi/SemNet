...
for index, meg in enumerate(subjects):
   ...
   raw_fname = directory + 'imlex_pw_ssstf_fft_1_48_clean_ica_raw.fif'
   raw = mne.io.Raw(raw_fname, preload=True)

   print "Reading events"   
   events_fname = orig_path + meg + 'imlex_pw_raw_ssstnew.txt'
   ...
   if index < 2:
	    	...
	    	nonword_ids=np.array([8]); # print events[events[:,2]==5,2].shape+events[events[:,2]==6,2].shape
	    	events=mne.merge_events(events,word_ids,601,replace_events=True)
	    	events=mne.merge_events(events,nonword_ids,602,replace_events=True)
	    	event_id = {'HIm': 601, 'LIm': 602}
...
#   mne.add_source_space_distances(sub_src)
   ...
   write_inverse_operator(data_path + subjects[index] + 'imlex_InvOp_ico5diagreg_fft_1_48_clean_ica_EMEG-inv.fif', inverse_operator_meeg)
