...
###############################################################################
# Set parameters
out_path = '/imaging/rf02/TypLexMEG/icaanalysis_results/imlex/stc/permutation/evoked/' # root
...
for p_threshold in ll:
    for ii, meg in enumerate(list_all):
    	print ii
    	fname_cncrt = data_path + meg + 'firstMorphed_ico_imlex_ica_word_Source_Evoked_m500_700'
    	stc_cncrt = mne.read_source_estimate(fname_cncrt)
    	#abslt_cncrt= np.absolute(stc_cncrt.copy().crop(.3,.4).data)
    	fname_abs = data_path + meg + 'firstMorphed_ico_imlex_ica_nonword_Source_Evoked_m500_700'
...
#        return permutation_t_test
    ...
    out_file1=out_path + 'ClusPer_abs_morphedico_cp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_17subj_imlex_ica_HLImag_50_550_100ms_tail'+str(tail)+'_maxstep'+str(max_step)
    ...
    out_file2=out_path + 'ClusPer_abs_sw_morphedico_cp'+str(p_threshold)[2:]+'_p'+str(p_thresh)[2:]+'_17subj_imlex_ica_HLImag_50_550_100ms_tail'+str(tail)+'_maxstep'+str(max_step)
