...
for cnt, meg in enumerate(ll):
    ...
    ltc=scipy.io.loadmat(path_c,mat_dtype=True); newlabels=np.squeeze(ltc['newlabels']).astype(int)
...
#        Matx_normalised[:,cntr]=np.divide(Matx[:,cntr],Matx[:,-1])
    ...
    mne.write_labels_to_annot(balanced_labels, subject=subject_from, parc=subject_from+'_test_finalpostparc_smooth_split_merge', subjects_dir=data_path,colormap='gist_ncar',hemi='both',overwrite=True)
