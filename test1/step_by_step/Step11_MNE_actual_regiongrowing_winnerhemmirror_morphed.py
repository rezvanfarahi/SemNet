...
path_mtx=data_path+'Matx_vert_second_newreg.mat'
...
fname_1col='colour_onlyvertices'+'_actualRG_'+str(len(labelsvv))+'.mat'
make_parcellation_mine(src_avg,labels_original,colors_orig,subject_avg,fname)
scipy.io.savemat(fname_1col,{'colors_orig':colors_orig})
...
#                labels_merged[lblname_cnt].name=labels[newlabels[labcnt,1]].name[:-3]+'_'+labels[newlabels[labcnt,0]].name
    ...
    ## keep only the hemisphere with most labels and mirror
    
    unver1=unver[unver>=0]
    labvernum1=labvernum[1:]
    if np.sum(labvernum1[unver1%2==0])>=np.sum(labvernum1[unver1%2!=0]):#left wins
        unver=unver1[unver1%2==0]
    else:
        unver=unver1[unver1%2!=0]#np.hstack((-100,unver1[unver1>=10242]))
    labels_winhem=list()
    #num_verts=np.zeros(unver[:-1].shape)
...
#                    balanced_labels.append(labels_final[lcnt02])
    ...
    colors_fin=nc(len(balanced_labels)/2,bytes_=True, cmap='hsv')
    ...
    fname='finalpostparc_smooth_actualRG_winnerhem_'+str(whilec)+'_'+str(len(balanced_labels))
    col_fname='colour_actualRG_winnerhem_'+str(whilec)+'_'+str(len(balanced_labels))+'.mat'
    scipy.io.savemat(col_fname,{'colors_fin':colors_fin})
