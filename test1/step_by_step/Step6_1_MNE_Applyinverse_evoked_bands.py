...
inv_fname = 'InvOp_ico5diagreg_fft_1_48_clean_ica_EMEG-inv.fif'#'InverseOperator_newreg_fft_pnt1_48_clean_ica_EMEG-inv.fif'#
...
for ii, meg in enumerate(ll):
    ...
    print "Epoching"
    if subject_inds[0] in np.array([2,3]):#np.array([4,8,9]):
        reject = dict(eeg=150e-6, grad=200e-12, mag=4e-12)
    else:
        reject = dict(eeg=120e-6, grad=200e-12, mag=4e-12)#, eog=150e-6)
    event_ids = {'cncrt_wrd': 1, 'abs_wrd': 2}#{'visual': 1, 'hear': 2, 'hand': 3, 'neutral': 4, 'emotional': 5,'pwordc': 6}
    ...
    for bcnt,b in enumerate(bands):
        ...
        print len(epochs)/184.#    for eegcnt in range(71):
#        if eegcnt<10:
...
#    condition_list=range(len(epochs_list))
    ...
    #        this_stc.resample(200)
            data_out = data_path + meg + 'firstMorphed_ico_SemDec_pnt1_48ica_'+event_names[evcnt]+'_Source_Evoked_m300_600_'+b
