...
Matx=np.zeros((20484,83,17))

print "ll:"
print ll
ii=-1
labelss = mne.read_labels_from_annot(subject='fsaverage', parc='rez_aparc', subjects_dir=subjects_dir)
labelss.remove(labelss[-5])
labelss.remove(labelss[-4])
...
for cnt, meg in enumerate(ll):
    ...
    forward = mne.read_forward_solution(fname_fwd, force_fixed=False, surf_ori=True)
    ...
    snr = np.mean(ediv)#3.0
    lambda2 = 1.0 / snr ** 2
    method = 'sLORETA'  # can be 'MNE' or 'sLORETA'
    mode = 'svd'
    n_svd_comp = 1
    #mne.io.make_eeg_average_ref_proj(forward['info'])
    #inverse_operator_eegmeg['info']['projs']=inverse_operator_eegmeg['projs']
    stc_ctf_mne = cross_talk_function(inverse_operator_eegmeg, forward, labels,method=method, lambda2=lambda2,mode=mode,signed=False, n_svd_comp=n_svd_comp)#
...
#    ff=np.intersect1d(ee,bbv)
    ...
    # for average
    Matx[:,:,cnt]=stc_to.data
Mat_avg=Matx.mean(axis=2)
rat=np.zeros((len(labelss),1))
for kk, lblk in enumerate(labelss):
    bb=stc_to.in_label(labelss[kk])
    cc=np.abs(Mat_avg[:,kk])
    dd=cc.argsort()[::-1]
    if kk%2==0:
        bbv=bb.vertices[0]
    else:
        bbv=bb.vertices[1]+10242
    ee=dd[:bb.shape[0]]
    ff=np.intersect1d(ee,bbv)
    rat[kk]=len(ff)/float(len(bbv))

rat_new=rat.copy()
rat_new[::2]=np.minimum(rat[::2],rat[1::2])
rat_new[1::2]=np.minimum(rat[::2],rat[1::2])
labl_new=[labl for ii,labl in enumerate(labelss) if rat_new[ii]>=rat_new.mean()-rat_new.std()] 
annot_fname=data_path+'fsaverage/label/rezvan_parc_sloreta_ctf.annot'   
mne.label.write_labels_to_annot(labl_new, subject='fsaverage', parc='rezvan_parc_sloreta_ctf', overwrite=True,
