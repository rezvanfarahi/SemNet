...
# Compute inverse solution and for each epoch. By using "return_generator=True"
		...
		# compute the PhaseLocking without having to keep all source estimates in memory.

		snr = 1.0  # use lower SNR for single epochs
		lambda2 = 1.0 / snr ** 2
		stcs = apply_inverse_epochs(epochs, inverse_operator, lambda2, method, pick_ori="normal", return_generator=True)

# Now we are ready to compute the PhaseLocking in the alpha and beta band.
...
# average the PhaseLocking in the alpha and beta band, i.e., we will only
...
#spectral_connectivity(data, method='coh', indices=None, sfreq=6.283185307179586, mode='multitaper', fmin=None, fmax=inf, fskip=0, faverage=False, tmin=None, tmax=None, mt_bandwidth=None, mt_adaptive=False, mt_low_bias=True, cwt_frequencies=None, cwt_n_cycles=7, block_size=1000, n_jobs=1, verbose=None)
		...
		print('Frequencies in Hz over which PhaseLocking was averaged for alpha: ')
		print(freqs[0])
		print('Frequencies in Hz over which PhaseLocking was averaged for beta: ')
		print(freqs[1])

		# Generate a SourceEstimate with the PhaseLocking. This is simple since we
		...
		plv_stc = mne.SourceEstimate(plv, vertices=stc.vertno, tmin=1e-3 * tmin, tstep=1e-3 * tstep, subject=subjects[ii])
		data_out = data_path + meg +  'SemLoc_Concrete_PhaseLocking_AlphaBeta_' + label_name[0:-3]
		plv_stc.save(data_out)

		# Now we c+an visualize the PhaseLocking using the plot method
		#brain = coh_stc.plot(subjects[0], 'inflated', 'lh', fmin=0.25, fmid=0.4,
		#	             fmax=0.65, time_label='PhaseLocking %0.1f Hz',
