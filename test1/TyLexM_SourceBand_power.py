...
## epoching
    ...
    bands = dict(theta=[4,8], alpha=[8,12], beta=[13, 30], gamma=[31, 45])#, beta=[13, 30], gamma=[31, 45])
    snr = ediv.mean()
    lambda2 = 1.0 / snr ** 2
    print "source_band_induced_power concrete"
    stcs_concrete = source_band_induced_power(epochs_concrete, inv_op, bands=bands, label=None, lambda2=lambda2, method='MNE', n_cycles=np.array([2,5,7,9]), baseline=(-400,0), baseline_mode='ratio', use_fft=False, n_jobs=4)
	...
	data_out = data_path + meg + 'preMorphed_SemLoc_icaclean_Concrete_Power_ratio_m500_700_' + b
	stc_to.save(data_out)     

    print "source_band_induced_power abstract"
    stcs_abstract = source_band_induced_power(epochs_abstract, inv_op, bands=bands, label=None, lambda2=lambda2, method='MNE', n_cycles=np.array([2,5,7,9]), baseline=(-400,0), baseline_mode='ratio', use_fft=False, n_jobs=4)
    
    for b, stc in stcs_abstract.iteritems():
        morphmat1=mne.compute_morph_matrix(subject_from, subject_to, stc.vertno, vertices_to, subjects_dir=data_path)
        stc_to=stc.morph_precomputed(subject_to,vertices_to,morphmat1, subject_from)
        data_out = data_path + meg + 'preMorphed_SemLoc_icaclean_Abstract_Power_ratio_m500_700_' + b
        stc_to.save(data_out)
