...
for p_threshold in ll:
    ...
    ### Concrete
    		...
    		fname_abs = data_path + meg + 'firstMorphed_SemLoc_icaclean_Abstract_' + conn_method + '_ThetaAlphaBetaGamma' + trange + label_name[0:-3] 
    	...
    	T_obst, clusterst, cluster_p_valuest, H0t = clut = spatio_temporal_cluster_1samp_test(X_theta, connectivity=connectivity,  n_permutations=n_permutations, n_jobs=4, threshold=t_threshold, tail=0)
    	...
    	T_obsa, clustersa, cluster_p_valuesa, H0a = clua = spatio_temporal_cluster_1samp_test(X_alpha, connectivity=connectivity,  n_permutations=n_permutations, n_jobs=4, threshold=t_threshold, tail=0)
    	#    Now select the clusters that are sig. at p < 0.05 (note that this value
    	#    is multiple-comparisons corrected).
    	...
    	T_obsb, clustersb, cluster_p_valuesb, H0b = club = spatio_temporal_cluster_1samp_test(X_beta, connectivity=connectivity,  n_permutations=n_permutations, n_jobs=4, threshold=t_threshold, tail=0)
    	#    is multiple-comparisons corrected).
    	good_cluster_indsb = np.where(cluster_p_valuesb < 0.05)[0]
    	print cluster_p_valuesb.min()
    	
    	print('Clustering gamma')
    	T_obsg, clustersg, cluster_p_valuesg, H0g = clug = spatio_temporal_cluster_1samp_test(X_gamma, connectivity=connectivity,  n_permutations=n_permutations, n_jobs=4, threshold=t_threshold, tail=0)
    	#    Now select the clusters that are sig. at p < 0.05 (note that this value
    	#    is multiple-comparisons corrected).
    	...
    	tstep=1e-3
    	fsave_vertices = [np.arange(10242), np.arange(10242)]
    	p_thresh=cluster_p_valuest.min()+0.000001
    	print p_thresh
    	if p_thresh<=0.1:
    	...
    	p_thresh=cluster_p_valuesa.min()+0.000001
    	print p_thresh
    	if p_thresh<=0.1:
    	...
    	p_thresh=cluster_p_valuesb.min()+0.000001
    	print p_thresh
    	if p_thresh<=0.1:
    		print "hi"
    	...
    	p_thresh=cluster_p_valuesg.min()+0.000001
    	print p_thresh
    	if p_thresh<=0.1:
