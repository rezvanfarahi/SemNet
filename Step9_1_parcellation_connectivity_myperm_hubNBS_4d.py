...
freq_names=['Allfreqs','Alpha','Beta','Gamma']
...
#        print ii
    ...
    thisX=grand_connmat[:,:,:,:,thisev,:]-grand_connmat[:,:,:,:,refev,:]
    thisX=np.transpose(thisX,(4,0,1,2,3))
    MT=np.zeros((nnodes,nnodes,nfreqs,ntwins))    
    for uvcnt1 in range(thisX.shape[2]):#node
        for uvcnt2 in range(thisX.shape[3]):#freq
            for uvcnt3 in range(thisX.shape[4]):#time
#        print uvcnt
                MT[:,uvcnt1,uvcnt2,uvcnt3]= mne.stats.ttest_1samp_no_p(thisX[:,:,uvcnt1,uvcnt2,uvcnt3], method='relative')
    for ffcnt in range(nfreqs):
        for ttcnt in range(ntwins):
            np.fill_diagonal(MT[:,:,ffcnt,ttcnt], 0)
    MTpos=MT.copy()
    ...
    Tpos_sum=np.sum(np.sum(np.sum(MTpos_1side,3),2),1)
...
#        print pcnt
        ...
        for uvcnt1 in range(thisX.shape[2]):#node
            for uvcnt2 in range(thisX.shape[3]):#freq
                for uvcnt3 in range(thisX.shape[4]):#time
#        print uvcnt
                    MT[:,uvcnt1,uvcnt2,uvcnt3]= mne.stats.ttest_1samp_no_p(signmat*thisX[:,:,uvcnt1,uvcnt2,uvcnt3], method='relative')
        for ffcnt in range(nfreqs):
            for ttcnt in range(ntwins):
                np.fill_diagonal(MT[:,:,ffcnt,ttcnt], 0)
        MTpos_perm=MT.copy()
        ...
        Tpos_sum_perm=np.sum(np.sum(np.sum(MTpos_1side,3),2),1)
...
#        this_MTpos[np.where(comps_pos!=cu_pos[pvalsig])[0],:]=0
        out_fname=out_path+'NBSnetwork_4D_pos'+parc_names[wparc]+'_'+event_names[thisev]+'_'+event_names[refev]+'_'+freq_names[thisfreq]+'_alltimes_'+str(p_threshold)[2:]+'.mat'#'_'+str(thisp)[2:6]+
...
#        print uvcnt
    ...
    Tneg_sum=np.sum(np.sum(np.sum(MTneg_1side,3),2),1)
...
#        print uvcnt
        ...
        Tneg_sum_perm=np.sum(np.sum(np.sum(MTneg_1side,3),2),1)
...
#        this_MTneg[np.where(comps_neg!=cu_neg[pvalsig])[0],:]=0
        out_fname=out_path+'NBSnetwork_4D_neg'+parc_names[wparc]+'_'+event_names[thisev]+'_'+event_names[refev]+'_'+freq_names[thisfreq]+'_alltimes_'+str(p_threshold)[2:]+'.mat'#'_'+str(thisp)[2:6]+
