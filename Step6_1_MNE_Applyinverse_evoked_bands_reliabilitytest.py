...
freqsmin=np.array([0.5,4,8,12,30])#np.array([4,8,30])
freqsmax=np.array([4,8,12,30,48])#np.array([8,12,48])
print "subjects:"
print ll
n_subjects=19
...
for ii, meg in enumerate(ll):
    ...
    event_ids = {'visual': 1, 'hear': 2, 'hand': 3, 'pwordc': 6}
    ...
    for bcnt,b in enumerate(bands):
        print b
        raw=raw_orig.copy()
        freqmin=freqsmin[bcnt];freqmax=freqsmax[bcnt]
...
#                raw.info['bads'].append(u'EEG0'+str(eegcnt))
        epochs_list=range(4)
        all_ids=np.array([1,2,3,6])
        for eid in range(4):
            event_id = all_ids[eid]#eid+1  
...
#    condition_list=range(len(epochs_list))
        for evcnt in range(len(epochs_list)):
            this_epoche=epochs_list[evcnt][::2]
            this_evokede = this_epoche.average()
            snr = 3.0#np.mean(ediv)
            lambda2 = 1.0 / snr ** 2
            this_condition = apply_inverse(this_evokede, inverse_operator, lambda2, method)
    ...
    #        this_stc.resample(200)
            data_out = data_path + meg + 'firstMorphed_ico_SemDec_Even_pnt1_48ica_'+event_names[evcnt]+'_Source_Evoked_m300_600_'+b
            this_stc.save(data_out)
            
            this_epocho=epochs_list[evcnt][1::2]
            this_evokedo = this_epocho.average()
            this_conditiono = apply_inverse(this_evokedo, inverse_operator, lambda2, method)
            this_stco=this_conditiono.morph_precomputed(subject_to,vertices_to,this_morphmat, subject_from)
    #        this_stc.resample(200)
            data_out = data_path + meg + 'firstMorphed_ico_SemDec_Odd_pnt1_48ica_'+event_names[evcnt]+'_Source_Evoked_m300_600_'+b
            this_stco.save(data_out)
