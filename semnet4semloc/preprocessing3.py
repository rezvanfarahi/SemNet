...
def compute_ica(raw, subject, n_components=0.99, picks=None, decim=None,
    ...
    if 'eeg' in ica:
        if 'ecg' in raw:
            ...
            #picks_ = pick_types(raw.info, meg=False, ecg=True)[0]
    ...
    ecg_epochs = create_ecg_epochs(raw, tmin=ecg_tmin, tmax=ecg_tmax, picks=picks_, reject=reject_)#keep_ecg=True,
...
#        [picks_, pick_types(raw.info, meg=False, eeg=False, ecg=False,
#                            eog=True)[1]])
    #picks_eog = pick_types(raw.info, meg=False, eeg=False, ecg=False,eog=True)
    eog_reject={'mag': 5e-12, 'grad': 5000e-13, 'eeg': 800e-6,'eog': 800e-6}
    for ch_type in ['mag', 'grad', 'eeg']:
        if ch_type not in ica:
    ...
    eog_ave = eog_epochs.average()#picks=np.array([0,1])
    ...
    if len(eog_epochs.events) > 0:
        threshold=3.0
        eog_inds, scores = ica.find_bads_eog(eog_epochs, threshold=threshold)#ica.find_bads_eog(eog_epochs, threshold=threshold)
        while not eog_inds:# and threshold>2.0:
           threshold=threshold-0.1
           eog_inds, scores = ica.find_bads_eog(eog_epochs, threshold=threshold)#ica.find_bads_eog(eog_epochs, threshold=threshold)
    else:
        threshold=3.0
        eog_inds, scores = ica.find_bads_eog(raw, threshold=threshold)#ica.find_bads_eog(eog_epochs, threshold=threshold)
        while not eog_inds:# and threshold>2.0:
           threshold=threshold-0.1
           eog_inds, scores = ica.find_bads_eog(raw, threshold=threshold)#ica.find_bads_eog(eog_epochs, threshold=threshold)
        

    if len(eog_inds)>0 and len(eog_epochs.events) > -1:
